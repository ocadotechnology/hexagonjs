...
dx.Set = (function(superClass) {
  extend(Set, superClass);

  function Set(iterable) {
    dx.deprecatedWarning('dx.Set', 'ES6 Set');
...
dx.Map = (function(superClass) {
  extend(Map, superClass);

  function Map(iterable) {
    dx.deprecatedWarning('dx.Map', 'ES6 Map');
...
dx.List = (function(superClass) {
  extend(List, superClass);

  function List(items) {
    dx.deprecatedWarning('dx.List', 'JS Array');
...
(function(){
var dx_extend, dx_merge, dx_parseHTML, scrollbarSize, supportsDate, supportsTouch, vendorPrefixes,
  slice = [].slice;

dx.deprecatedWarning = function() {
...
dx.debounce = function(duration, fn) {
  var timeout;
  timeout = void 0;
  return function() {
    var f, origArgs;
...
dx.clamp = function(min, max, value) {
  return Math.min(max, Math.max(min, value));
};

dx.clampUnit = function(value) {
  return dx.clamp(0, 1, value);
};

dx.randomId = function(size, alphabet) {
...
dx.min = function(values) {
  return Math.min.apply(null, values != null ? values.filter(dx.defined) : void 0);
};

dx.minBy = function(values, f) {
...
dx.max = function(values) {
  return Math.max.apply(null, values != null ? values.filter(dx.defined) : void 0);
};

dx.maxBy = function(values, f) {
...
dx.sum = function(values, f) {
  return values.reduce((function(a, b) {
    return a + b;
  }), 0);
};

dx.flatten = function(arr) {
  return [].concat.apply([], arr);
};

dx.cycle = function(list, i) {
  return list[i % list.length];
};

dx.hashList = function(list, str) {
  return list[dx.hash(str, list.length)];
};

dx.find = function(arr, f) {
  var d, l, len1;
...
dx.isString = function(x) {
  return typeof x === 'string' || x instanceof String;
};

dx.isFunction = function(x) {
...
dx.groupBy = function(arr, f) {
  ...
  for (l = 0, len1 = arr.length; l < len1; l++) {
    ...
    category = f(x);
    if (!map.has(category)) {
      map.set(category, new dx._.List);
    }
...
dx.unique = function(list) {
  return new dx._.Set(list).values();
};

dx.endsWith = function(string, suffix) {
  return string.indexOf(suffix, string.length - suffix.length) !== -1;
};

dx.startsWith = function(string, substring) {
  return string.lastIndexOf(substring, 0) === 0;
};

dx.tween = function(start, end, amount) {
  return start + (end - start) * amount;
};

dx.defined = function(x) {
  return x !== void 0 && x !== null;
};

dx.zip = function(arrays) {
  var i, l, length, ref, results;
  if (arrays) {
    if (arrays.length > 0) {
      length = dx.min(arrays.map(function(d) {
        return d.length || 0;
      }));
      if (length > 0) {
        results = [];
...
dx_extend = function(target, overlay, retainUndefined) {
  var k, results, v;
  results = [];
  for (k in overlay) {
    v = overlay[k];
    if (dx.isPlainObject(v)) {
      if (target[k] == null) {
        target[k] = {};
      }
      results.push(dx_extend(target[k], v, retainUndefined));
    } else {
      if (v !== void 0 || retainUndefined) {
        results.push(target[k] = dx.clone(v));
      } else {
        results.push(void 0);
      }
    }
...
dx_merge = function(deep, retainUndefined, objects) {
  var k, l, len1, len2, m, obj, res, v;
  if (deep) {
    res = {};
    for (l = 0, len1 = objects.length; l < len1; l++) {
      obj = objects[l];
      if (dx.isPlainObject(obj)) {
        dx_extend(res, obj, retainUndefined);
      }
    }
    return res;
  } else {
...
dx.merge = function() {
  ...
  return dx_merge(true, true, objects);
};

dx.merge.defined = function() {
  var objects;
  objects = 1 <= arguments.length ? slice.call(arguments, 0) : [];
  return dx_merge(true, false, objects);
};

dx.shallowMerge = function() {
  var objects;
  objects = 1 <= arguments.length ? slice.call(arguments, 0) : [];
  return dx_merge(false, true, objects);
};

dx.shallowMerge.defined = function() {
  var objects;
  objects = 1 <= arguments.length ? slice.call(arguments, 0) : [];
  return dx_merge(false, false, objects);
};

dx.clone = function(obj) {
  var ref;
  if (dx.isArray(obj)) {
    return obj.map(dx.clone);
  } else if (dx.isPlainObject(obj)) {
    return dx.merge({}, obj);
  } else if (obj instanceof dx._.List) {
    return new dx._.List(obj.entries().map(dx.clone));
  } else if (obj instanceof dx._.Map) {
    return new dx._.Map(obj.entries().map(function(arg) {
      var k, v;
      k = arg[0], v = arg[1];
      return [dx.clone(k), dx.clone(v)];
    }));
  } else if (obj instanceof dx._.Set) {
    return new dx._.Set(obj.keys().map(dx.clone));
  } else if (obj instanceof Date) {
    return new Date(obj.getTime());
  } else if (dx.isObject(obj) && obj !== null) {
    dx.consoleWarning("Trying to clone " + obj + " with constructor " + (obj != null ? (ref = obj.constructor) != null ? ref.name : void 0 : void 0) + ", it isn't really cloneable! Carrying on anyway.");
    return {};
  } else {
    return obj;
  }
...
dx.shallowClone = function(obj) {
  var ref;
  if (dx.isArray(obj)) {
    return obj.slice();
  } else if (dx.isPlainObject(obj)) {
    return dx.shallowMerge({}, obj);
  } else if (obj instanceof dx._.List) {
    return new dx._.List(obj.entries());
  } else if (obj instanceof dx._.Map) {
    return new dx._.Map(obj.entries());
  } else if (obj instanceof dx._.Set) {
    return new dx._.Set(obj.keys());
  } else if (obj instanceof Date) {
    return new Date(obj.getTime());
  } else if (dx.isObject(obj) && obj !== null) {
    dx.consoleWarning("Trying to shallow clone " + obj + " with constructor " + (obj != null ? (ref = obj.constructor) != null ? ref.name : void 0 : void 0) + ", it isn't really cloneable! Carrying on anyway.");
    return {};
  } else {
    return obj;
  }
...
dx.identity = function(d) {
  return d;
};

dx_parseHTML = null;

dx.parseHTML = function(html) {
  var e;
  dx.deprecatedWarning('dx.parseHTML', 'N/A');
  if (!dx_parseHTML) {
  ...
  return dx_parseHTML(html);
};

dx.cleanNode = function(node, recurse) {
  var child, n;
  if (recurse == null) {
    recurse = true;
  }
...
dx.scrollbarSize = function() {
  ...
  if (scrollbarSize == null) {
    ...
    inner = dx.detached('p').style('width', '100%').style('height', '200px');
    outer = dx.detached('div').style('position', 'absolute').style('top', '0').style('left', '0').style('visiblity', 'hidden').style('width', '200px').style('height', '150px').style('overflow', 'hidden');
    outer.append(inner);
    dx.select('body').append(outer);
...
dx.parentZIndex = function(node, findMax) {
  var check, res;
  check = function(node) {
    var index;
    index = Number(dx.select(node).style('z-index'));
    if (!isNaN(index) && index > 0) {
      return index;
    }
  };
  res = dx.checkParents(node, check, findMax);
  if (findMax) {
    return dx.max(res);
  } else {
    return res;
...
dx.checkParents = function(node, check, returnArray) {
...
BasicEventEmitter = (function() {
  function BasicEventEmitter() {
    this.callbacks = new dx._.Map;
    this.allCallbacks = new dx._.List;
  }
  ...
  BasicEventEmitter.prototype.on = function(name, callback) {
    if (name) {
      if (!this.callbacks.has(name)) {
        this.callbacks.set(name, new dx._.List);
      }
...
EventEmitter = (function() {
  ...
  function EventEmitter() {
    this.suppressedMap = new dx._.Map;
    this.emitters = new dx._.List;
    this.emittersMap = new dx._.Map;
  ...
  EventEmitter.prototype.on = function(name, namespace, callback) {
    ...
    if (namespace === 'default') {
      dx.consoleWarning('"default" is a reserved namespace. It can not be used as a namespace name.');
      return this;
    }
    if (dx.isString(namespace)) {
  ...
  EventEmitter.prototype.off = function(name, namespace, callback) {
    ...
    if (dx.isString(namespace)) {
      ...
      if (!callback && !dx.isString(namespace)) {
        callback = namespace;
      }
...
Color = (function() {
    ...
    return function(value) {
      if (arguments.length > 0) {
        if ((value != null) && !isNaN(value)) {
          this._[prop] = dx.clamp(min, max, value);
          if (!isAlphaValue) {
            update(this._, isHSL);
          }
        }
  ...
  Color.prototype.mix = function(color, amount) {
    ...
    if (!dx.color.isColor(color)) {
      color = dx.color(color);
    }
  ...
  Color.prototype.range = function(numLight, numDark, maxRange, outputFormat) {
    ...
    self = this;
    light = dx.range(numLight + 1).map(function(i) {
      return self.clone().lighten(step * i);
    });
    dark = dx.range(numDark).reverse().map(function(i) {
...
dx.color.isColorString = function(str) {
  return fromString(str) !== void 0;
};

dx.color.isColor = function(obj) {
...
getMatches = function(node) {
  return node.matches || dx.vendor(node, 'matchesSelector') || matchPolyfill;
};
...
getHexagonElementDataObject = function(element, createIfNotExists) {
  ...
  if (createIfNotExists) {
    if (element.__dx__ == null) {
      element.__dx__ = {};
    }
    return element.__dx__;
  } else {
    if (element.__dx__) {
      return element.__dx__;
    }
...
ElementSet = (function() {
  function ElementSet() {
    this.elements = new dx._.List;
    this.ids = new dx._.Set;
  }
...
Selection = (function() {
  ...
  Selection.prototype.select = function(selector) {
    var s;
    if (!dx.isString(selector)) {
      dx.consoleWarning('Selection.select was passed the wrong argument type', 'Selection.select only accepts a string argument, you supplied:', selector);
  ...
  Selection.prototype.selectAll = function(selector) {
    if (!dx.isString(selector)) {
      dx.consoleWarning('Selection.selectAll was passed the wrong argument type', 'Selection.selectAll only accepts a string argument, you supplied:', selector);
      return new Selection([]);
    } else {
  ...
  Selection.prototype.closest = function(selector) {
    var s;
    s = !dx.isString(selector) ? (dx.consoleWarning('Selection.closest was passed the wrong argument type', 'Selection.closest only accepts a string argument, you supplied:', selector), new Selection([])) : new Selection(this.nodes.map(function(node) {
  ...
  attachSingle = function(selection, element, attacher) {
    ...
    if (!(dx.isString(element) || (element instanceof Selection) || (element instanceof Element))) {
      dx.consoleWarning('Selection Api error when attaching element', 'Expecting an Element, Selection or string argument, you supplied:', element);
      return [];
    }
    if (!selection.singleSelection && (element instanceof Element || element instanceof Selection)) {
      dx.consoleWarning('Selection Api error when attaching element', 'You can not attach an existing element to a selection with multiple elements');
      return [];
    }
    if (selection.empty()) {
      dx.consoleWarning('Selection Api error when attaching element', 'You can not attach an element to an empty selection');
      return [];
    }
  ...
  attach = function(selection, name, attacher, reverse) {
    ...
    newNodes = dx.isArray(name) ? (singleSelection = false, dir = reverse ? -1 : 1, ns = (function() {
      ...
      return results;
    })(), dx.flatten(reverse ? ns.reverse() : ns)) : attachSingle(selection, name, attacher);
  ...
  Selection.prototype.html = function(html) {
    var j, len, node, ref;
    dx.deprecatedWarning('Selection::html', 'N/A');
    if (arguments.length === 1) {
      ref = this.nodes;
      for (j = 0, len = ref.length; j < len; j++) {
        node = ref[j];
  ...
  Selection.prototype.on = function(name, namespace, f) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      eventEmitter = data.eventEmitter ? data.eventEmitter : data.eventEmitter = new dx.EventEmitter;
      if (data.eventAugmenters == null) {
        data.eventAugmenters = new dx._.Map;
      }
      if (data.listenerNamesRegistered == null) {
        data.listenerNamesRegistered = new dx._.Set;
      }
...
dx.select = function(selector) {
  if (selector instanceof Selection) {
    return selector;
  } else if (!((selector instanceof HTMLElement) || (selector instanceof SVGElement) || dx.isString(selector) || selector === document || selector === window)) {
    dx.consoleWarning('dx.select was passed the wrong argument type', 'dx.select only accepts a HTMLElement, SVGElement or string argument, you supplied:', selector);
    return new Selection([]);
  } else {
...
dx.select.getHexagonElementDataObject = getHexagonElementDataObject;

dx.select.addEventAugmenter = function(augmenter) {
  return augmenters.push(augmenter);
};

dx.selectAll = function(selector) {
  if (!(dx.isString(selector) || dx.isArray(selector))) {
    dx.consoleWarning('dx.selectAll was passed the wrong argument type', 'dx.selectAll only accepts a string argument, you supplied:', selector);
    return new Selection([]);
  } else {
    if (dx.isArray(selector)) {
...
dx.detached = function(name, namespace) {
  namespace = namespaces.hasOwnProperty(name) ? namespaces[name] : namespaces.xhtml;
  return dx.select(document.createElementNS(namespace, name));
};

dx.Selection = Selection;
...
(function(){
var dx_ease, loopUpdate, next;

next = dx.vendor(window, "requestAnimationFrame") || function(f) {
...
dx.transition = function(millis, f, ease, endCallback) {
  var cancelled, start;
  if (ease == null) {
    ease = dx.ease.linear;
  }
...
Animation = (function(superClass) {
  ...
  function Animation(node1, ease1) {
    this.node = node1;
    this.ease = ease1 != null ? ease1 : dx.ease.linear;
  ...
  transition = function(iteration, duration) {
    this.remaining++;
    this.cancelers.push(dx.transition(duration, iteration, this.ease, this.endCallback));
    return this;
  };
  ...
  Animation.prototype.style = function(property, value, duration) {
    ...
    if (this.node) {
      if (arguments.length === 4) {
        interpolator = dx.interpolate(arguments[1], arguments[2]);
        ...
        interpolator = dx.interpolate(window.getComputedStyle(this.node, null).getPropertyValue(property), value);
        iteration = (function(_this) {
          return function(t, cancel) {
            if (!cancel) {
              return _this.node.style.setProperty(property, interpolator(t));
  ...
  Animation.prototype.attr = function(attribute, value, duration) {
    ...
    if (this.node) {
      if (arguments.length === 4) {
        ...
        interpolator = dx.interpolate(this.node.getAttribute(attribute), value);
        iteration = (function(_this) {
          return function(t, cancel) {
            if (!cancel) {
              return _this.node.setAttribute(attribute, interpolator(t));
...
dx.animate = function(node, ease) {
  return new Animation(node, ease);
};

dx.Selection.prototype.animate = function(ease) {
  return dx.animate(this.nodes[0], ease);
};
...
Morph = (function(superClass) {
  ...
  function Morph(node1, trigger, start, cancellers) {
    ...
    this.finished = false;
    if (this.trigger) {
      this.trigger.on('end', 'dx.animate', (function(_this) {
        return function() {
          _this.finished = true;
          if (!_this.cancelled) {
            return perform.call(_this);
    ...
    if (dx.isFunction(f)) {
      ...
      if (this.node) {
        ...
        if (morphFactory) {
          ...
          dx.consoleWarning(f + ' is not a registered morph', 'The available morphs are', dx_morphs.entries());
        }
      }
    }
    return this;
  ...
  Morph.prototype.cancel = function() {
    ...
    if (!this.cancelled) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        canceler = ref[i];
        if (dx.isFunction(canceler.cancel)) {
          canceler.cancel();
        }
      }
    }
  ...
  perform = function() {
    ...
    if (this.actions.length > 0) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        if (res instanceof dx.EventEmitter) {
          results.push(res.on('end', 'dx.animate', onEnd));
  ...
  Morph.prototype.go = function(cancelOngoing) {
    ...
    this.start();
    if (this.node) {
      obj = dx.select.getHexagonElementDataObject(this.node, true);
      if (obj.morphs == null) {
        obj.morphs = [];
      }
  ...
  Morph.prototype.cancelOngoing = function() {
    ...
    if (this.node) {
      obj = dx.select.getHexagonElementDataObject(this.node, false);
      if (obj) {
        if (obj.morphs) {
          ref = obj.morphs;
          for (i = 0, len = ref.length; i < len; i++) {
...
dx.morph = function(node) {
  return new Morph(node);
};

dx.Selection.prototype.morph = function() {
  return new Morph(this.nodes[0]);
};

dx_morphs = new dx._.Map;

dx.morph.register = function(name, morph) {
  return dx_morphs.set(name, morph);
};
...
View = (function() {
  ...
  View.prototype.apply = function(data, key) {
    ...
    if (this.rootSelection.size()) {
      ...
      if (key) {
        nodeByKey = new dx._.Map;
        dataByKey = new dx._.Map(data.map(function(datum) {
          return [key(datum), datum];
        }));
        for (l = 0, len = nodes.length; l < len; l++) {
          node = nodes[l];
          nodeData = dx.select.getHexagonElementDataObject(node);
          if (nodeData.datum) {
            d = nodeData.datum;
            k = key(d);
            if (nodeByKey.has(k)) {
        ...
        for (m = 0, len1 = nodes.length; m < len1; m++) {
          ...
          if (i < data.length) {
            ...
            nodeData = dx.select.getHexagonElementDataObject(node, false);
            exitSet.push({
              element: node,
      ...
      viewEnterWarning = function(element, selector) {
        return dx.consoleWarning("view enter fn returned", element, "! It didn't match selector", selector, ", so you may encounter odd behavior");
      };
      ...
      newNodeSet = enterSet.map((function(_this) {
        return function(d, i) {
          ...
          if (!isChild) {
            ...
            isClassedCorrectly = dx.select(element).classed(classString);
            if (!isClassedCorrectly) {
              viewEnterWarning(element, _this.selector);
            }
          }
          hedo = dx.select.getHexagonElementDataObject(element);
          hedo.datum = datum;
          return ret = {
            element: element,
            datum: d.datum
      ...
      for (i = o = 0, len2 = exitSet.length; o < len2; i = ++o) {
        d = exitSet[i];
        this.old.call(dx.select(d.element), d.datum, d.element, i);
      }
      ref2 = updateSet.concat(newNodeSet);
      for (i = p = 0, len3 = ref2.length; p < len3; i = ++p) {
        d = ref2[i];
        this.each.call(dx.select(d.element), d.datum, d.element, i);
      }
      return this;
    }
  };
...
dx.Selection.prototype.view = function(selector, type) {
  if (type == null) {
    type = 'div';
  }
  if (this.size() === 0) {
    dx.consoleWarning('.view() called on an empty selection');
  }
...
addAugmentor = function(name, mouseName, touchNames, node, addEventAugmenter, mouseEventFactory, touchEventFactory) {
  return dx.select.addEventAugmenter({
    name: name,
      ...
      array = array.sort(function(a, b) {
        ...
        if (aI > bI) {
          ...
          for (i = j = 0, ref = aArr.length; j < ref; i = j += 1) {
            r = dx.sort.compare(aArr[i], bArr[i]);
            if (r !== 0) {
              break;
            }
          }
...
dx.filter.stringTypes = function() {
  return ['contains', 'exact', 'excludes', 'startsWith', 'regex', 'fuzzy'];
};

dx.filter.numberTypes = function() {
  return ['exact', 'greater', 'less'];
};

dx.filter.types = function() {
...
getValidationMessage = function(message, type) {
  switch (message.toLowerCase()) {
    case 'value missing':
      if (type === 'radio') {
        return dx.userFacingText('form', 'missingRadioValue');
      } else {
        return dx.userFacingText('form', 'missingValue');
      }
      break;
    case 'type mismatch':
      return dx.userFacingText('form', 'typeMismatch');
    default:
      return message;
  }
};
...
validateForm = function(form, options) {
  ...
  form = dx.select(form).node();
  options = dx.merge.defined({
    showMessage: true
  }, options);
  dx.select(form).selectAll('.dx-form-error').remove();
  errors = [];
  ...
  if (options.showMessage && errors.length > 0) {
    ...
    dx.select(error.node.parentNode).insertAfter('div')["class"]('dx-form-error').append('div').insertAfter('div')["class"]('dx-form-error-text-container').append('div')["class"]('dx-form-error-text').text(error.message);
    dx.select(error.node).on('click', 'dx.form', function(e) {
      var next;
      next = dx.select(error.node.parentNode.nextElementSibling);
      if (next.classed('dx-form-error')) {
        next.remove();
      }
      return dx.select(error.node).off('click', 'dx.form');
    });
  }
  return {
...
Modal = (function(superClass) {
  ...
  closeModal = function(modal, event) {
    var body;
    body = dx.select('body').classed('dx-modal-open', false);
    body.select('.dx-modal-container').remove();
    body.select('.dx-modal-shade').remove();
  ...
  Modal.prototype.show = function(cb) {
    ...
    body = dx.select('body').classed('dx-modal-open', true);
    shade = body.select('.dx-modal-shade');
    if (shade.empty()) {
      shade = body.append('div').attr('class', 'dx-modal-shade');
      shade.style('opacity', 0).morph()["with"]('fadein', 150).go();
    }
    body.select('.dx-modal-container').remove();
    modalContainer = body.append('div').attr('class', 'dx-modal-container');
    modal = modalContainer.append('div').attr('class', 'dx-modal');
    titleContainer = modal.append('div')["class"]('dx-modal-title-container dx-group dx-horizontal dx-header');
    title = dx.detached('div')["class"]('dx-modal-title');
    if (this.options.closeButtonEnabled) {
      closeButton = dx.detached('div').add(dx.detached('i')["class"]('dx-icon dx-icon-close'))["class"]('dx-modal-close dx-fixed').on('click', 'dx.modal', (function(_this) {
        return function() {
      ...
      if ((this.title == null) || this.title.length === 0) {
        titleContainer.classed('dx-modal-title-empty', true);
      }
    }
    if (this.options.closeWithShadeEnabled) {
      modalContainer.on('click', 'dx.modal', (function(_this) {
        return function(e) {
          if (!modal.contains(e.target) && dx.select('body').contains(e.target)) {
            return closeModal(_this, {
              cause: 'shade'
            });
          }
    ...
    this.contentContainer = modal.append('div').attr('class', 'dx-modal-content');
    if (this.setup) {
...
makeButtons = function(container, buttons, modal, callback) {
  buttons.forEach(function(d) {
    return container.append('button').attr('type', 'button')["class"](d.classes).add(dx.detached('i')["class"](d.icon)).add(dx.detached('span').text(' ' + d.text)).on('click', 'dx.modal', function() {
...
getHeaderRender = function(titleClass) {
  return function(elem, title, button, modal) {
    return dx.select(elem).classed('dx-background-' + titleClass, true).add(title).add(button);
  };
};
...
modalDialog = function(title, message, callback, options) {
    ...
    buttons: [
      {
        ...
        value: false,
        classes: 'dx-btn dx-negative'
      }, {
        text: 'Confirm',
        icon: 'dx-icon dx-icon-check',
        value: true,
        classes: 'dx-btn dx-positive'
      }
    ],
    titleClass: void 0,
    icon: void 0
  ...
  setup = function(element) {
    ...
    container = dx.select(element);
    message = container.append('div')["class"]('dx-modal-message').text(message);
    buttonContainer = container.append('div')["class"]('dx-modal-buttons');
  ...
  modal.on('hide', 'dx.modal', function(d) {
    if (d.cause !== 'api') {
      return callback();
    }
  });
...
modalInput = function(title, message, callback, options) {
  ...
  setup = function(element) {
    ...
    container = dx.select(element);
    message = container.append('span')["class"]('dx-modal-message').text(message);
    input = container.append('input')["class"]('dx-modal-input').text(this.options.value);
    buttonContainer = container.append('div')["class"]('dx-modal-buttons');
  ...
  modal.on('close', 'dx.modal', function(d) {
    if (d.cause !== 'api') {
      return callback();
    }
  });
...
setupNotification = function(notification, selection) {
  ...
  icon = (notification.options.icon != null) && notification.options.icon.length > 0 ? dx.detached('div')["class"]('dx-notification-icon-container').add(dx.detached('i')["class"]('dx-notification-icon ' + notification.options.icon)) : void 0;
  content = dx.detached('div')["class"]('dx-notification-content');
  msg = notification.message;
  msgIsString = dx.isString(msg) || dx.isNumber(msg);
  msgIsNode = !msgIsString && ((msg instanceof dx.Selection) || (msg instanceof HTMLElement));
  msgIsArrayOfNodes = msgIsNode || dx.isArray(msg) && msg.every(function(item) {
    return (item instanceof dx.Selection) || (item instanceof HTMLElement);
  });
  msgIsObject = !msgIsString && !msgIsNode && !msgIsArrayOfNodes && dx.isObject(msg);
  if (msgIsString) {
    content.text(msg);
  ...
  if (notification.options.pinnable) {
    pin = dx.detached('div')["class"]('dx-notification-icon-container dx-notification-pin').on('click', 'dx.notify', function() {
      return togglePin(notification);
    }).add(dx.detached('i').attr('class', 'dx-icon dx-icon-thumb-tack'));
    notification.domPin = pin;
    updatePinnedStatus(notification);
  }
  close = dx.detached('div')["class"]('dx-notification-icon-container dx-notification-close').on('click', 'dx.notify', function() {
    return notification.close();
  }).add(dx.detached('i')["class"]('dx-icon dx-icon-close'));
...
redraw = function(manager) {
  ...
  selection = dx.select(manager.selector);
  container = selection.select('#' + manager.uniqueId);
  if (container.empty()) {
    container = selection.append('div')["class"]('dx-notification-container').attr('id', manager.uniqueId);
  }
  view = container.view('.dx-notification');
  view.enter(function(d) {
    var optionalClass;
    selection = this.append('div');
    optionalClass = d.options.cssclass ? " " + d.options.cssclass : '';
    selection["class"]("dx-notification" + optionalClass).forEach(function(node) {
...
updatePinnedStatus = function(notification) {
  return notification.domPin.classed('dx-notification-pin-pinned', notification.pinned);
};

defaultRenderer = function(node, message) {
  return dx.select(node).text(message);
};
...
NotificationManager = (function() {
  ...
  function NotificationManager(selector) {
    ...
    this.currentId = 0;
    this.notifications = [];
    this.uniqueId = 'dx-notify-' + dx.randomId();
    this._ = {
      defaultTimeout: 5
    };
  }
  ...
  themedNotification = function(manager, contextClass, iconClass, message, options) {
    ...
    mergedOptions = dx.merge({
      icon: 'dx-icon ' + iconClass
    }, options);
  ...
  NotificationManager.prototype.info = function(message, options) {
    ...
    return themedNotification(this, 'dx-info', 'dx-icon-info', message, options);
  };
  ...
  NotificationManager.prototype.warning = function(message, options) {
    ...
    return themedNotification(this, 'dx-warning', 'dx-icon-warning', message, options);
  };
  ...
  NotificationManager.prototype.negative = function(message, options) {
    ...
    return themedNotification(this, 'dx-negative', 'dx-icon-error', message, options);
  };
  ...
  NotificationManager.prototype.positive = function(message, options) {
    ...
    return themedNotification(this, 'dx-positive', 'dx-icon-check', message, options);
  };
...
dx.round = function(number, options) {
  return strictCheck(number, formatRound, options);
};

dx.si = function(number, options) {
  return strictCheck(number, formatSI, options);
};

dx.exp = function(number, options) {
  return strictCheck(number, formatExp, options);
};

dx.fixed = function(number, options) {
  return strictCheck(number, formatFixed, options);
};

dx.zeroPad = function(number, options) {
  return strictCheck(number, zeroPad, options);
};

deprecatedWarning = function(which) {
  return dx.deprecatedWarning("dx.format." + which + ": The formatter factory pattern is deprecated and will be removed in the next major release, please use dx." + which + "(number, options)");
};
...
ClickDetector = (function(superClass) {
  ...
  function ClickDetector() {
    ...
    this.eventId = dx.randomId();
    this.exceptions = new dx._.List;
    container = void 0;
    ...
    dx.select(document).on('pointerdown', 'dx.click-detector.' + this.eventId, this.downAction);
    dx.select(document).on('pointerup', 'dx.click-detector.' + this.eventId, this.upAction);
  }
  ...
  ClickDetector.prototype.cleanUp = function() {
    dx.select(document).off('pointerdown', 'dx.click-detector.' + this.eventId, this.downAction);
    dx.select(document).off('pointerup', 'dx.click-detector.' + this.eventId, this.upAction);
    return this;
  };
...
dx.ClickDetector = ClickDetector;
...
LocalStoragePreferencesStore = {
  save: function(prefs, cb) {
    localStorage.setItem('dx_preferences', prefs);
    return cb();
  },
  load: function(cb) {
    return cb(void 0, localStorage.getItem('dx_preferences'));
  }
};
...
defaultTimezoneLookup = function(offset) {
  ...
  return "UTC" + modifier + (dx.zeroPad(hours)) + ":" + (dx.zeroPad(minutes));
};
...
Preferences = (function(superClass) {
  ...
  function Preferences() {
    ...
    setupModal = (function(_this) {
      return function(element) {
        ...
        localeAutocompleteElement = dx.detached('input');
        ...
        new dx.AutoComplete(localeAutocompleteElement.node(), supportedLocales, {
          renderer: function(element, datum) {
            return dx.select(element).text(datum.full);
          },
        ...
        localeSection = dx.detached('div').add(dx.detached('label').text(dx.userFacingText('preferences', 'locale'))).add(localeAutocompleteElement);
        timezoneAutocompleteElement = dx.detached('input');
        timezoneAutocompleteElement.value(timezone);
        new dx.AutoComplete(timezoneAutocompleteElement.node(), _this._.supportedTimezones, {
          showOtherResults: true,
          mustMatch: true
        }).on('change', function(value) {
          return timezone = value;
        });
        timezoneSection = dx.detached('div').add(dx.detached('label').text(dx.userFacingText('preferences', 'timezone'))).add(timezoneAutocompleteElement);
        saveButton = dx.detached('button')["class"]('dx-btn dx-positive').add(dx.detached('i')["class"]('dx-icon dx-icon-check')).add(dx.detached('span').text(' ' + dx.userFacingText('preferences', 'save'))).on('click', function() {
          _this.locale(locale);
          _this.timezone(timezone);
          return _this.save(function(err) {
            if (err) {
              return dx.notify.negative(err);
            } else {
              dx.notify.positive(dx.userFacingText('preferences', 'preferencesSaved'));
              return modal.hide();
            }
          });
        });
        return dx.select(element).append('div')["class"]('dx-form').add(localeSection).add(timezoneSection).add(saveButton);
      };
    })(this);
    modal = new dx.Modal(dx.userFacingText('preferences', 'preferences'), setupModal);
    this._ = {
    ...
    if (!(dx.isString(defaultLocaleId) && lookupLocale(defaultLocaleId))) {
      defaultLocaleId = 'en';
    }
  ...
  Preferences.prototype.timezone = function(timezone) {
    if (arguments.length > 0) {
      if (this.isTimezoneSupported(timezone)) {
        ...
        dx.consoleWarning('preferences.timezone:', timezone + ' is not a valid timezone');
      }
      return this;
    } else {
  ...
  Preferences.prototype.locale = function(locale) {
    ...
    if (arguments.length > 0) {
      if (this.isLocaleSupported(locale)) {
        ...
        dx.consoleWarning('preferences.locale', locale + ' is not a valid locale. If you think the locale should be added to the list contact the maintainers of hexagon');
      }
      return this;
    } else {
  ...
  Preferences.prototype.isTimezoneSupported = function(timezone) {
    return dx.isString(timezone) && this._.supportedTimezones.indexOf(timezone) !== -1;
  };

  Preferences.prototype.isLocaleSupported = function(locale) {
    return dx.isString(locale) && lookupLocale(locale);
  };
...
dx.preferences = new Preferences;

dx.preferences.localStorageStore = LocalStoragePreferencesStore;
...
checkFixedPos = function(node) {
  if (dx.select(node).style('position') === 'fixed') {
    return true;
  }
};
...
calculateDropdownPosition = function(alignments, selectionRect, dropdownRect, windowRect, ddMaxHeight, scrollbarWidth) {
  ...
  if (direction === 'down' || direction === 'up') {
    x = dx.clamp(0, windowRect.width - dropdownRect.width, x);
  } else {
    y = dx.clamp(0, windowRect.height - dropdownRect.height, y);
  }
...
dropdownContentToSetupDropdown = function(dropdownContent) {
  ...
  return setupDropdown = (function() {
    switch (false) {
      case !dx.isString(dropdownContent):
        return function(node) {
          return dx.select(node).node().innerHTML = dropdownContent;
        };
      case !dx.isFunction(dropdownContent):
        return dropdownContent;
      default:
        dx.consoleWarning('dropdown: dropdownContent is not a valid type. dropdownContent: ', dropdownContent);
        return function() {
          return void 0;
        };
    }
...
Dropdown = (function(superClass) {
  ...
  function Dropdown(selector, dropdownContent, options) {
    ...
    clickDetector = new dx.ClickDetector;
    clickDetector.on('click', 'dx.dropdown', (function(_this) {
      return function() {
        return _this.hide();
      };
    })(this));
    ...
    if (this.options.mode === 'click' || this.options.mode === 'hover') {
      selection.on('click', 'dx.dropdown', onclick);
    }
    if (this.options.mode === 'hover') {
      selection.on('mouseover', 'dx.dropdown', onmouseover);
      selection.on('mouseout', 'dx.dropdown', onmouseout);
    }
  }
  ...
  Dropdown.prototype.show = function(cb) {
    ...
    if (_.visible) {
      ...
      _.visible = true;
      _.dropdown = dx.select(dx._.dropdown.attachToSelector).append('div').attr('class', 'dx-dropdown');
      ...
      parentFixed = dx.checkParents(_.selection.node(), checkFixedPos);
      parentZIndex = dx.parentZIndex(_.selection.node(), true);
      ...
      }, ddMaxHeight, dx.scrollbarSize()), x = ref.x, y = ref.y;
      if (!parentFixed) {
  ...
  Dropdown.prototype.cleanUp = function() {
    ...
    if (this.options.mode === 'hover') {
      _.selection.off('mouseover', 'dx.dropdown', _.onmouseover);
      _.selection.off('mouseout', 'dx.dropdown', _.onmouseout);
    }
    return this;
  };
...
Collapsible = (function(superClass) {
  ...
  function Collapsible(selector, options) {
    ...
    this.selection = dx.select(selector).classed('dx-openable', true).classed('dx-collapsible', true);
    header = this.selection.select('.dx-collapsible-heading');
    if (!(toggleBtn = header.select('.dx-collapsible-toggle')).empty()) {
      header.classed('dx-collapsible-heading-no-hover', true);
      toggleBtn.on('click', 'dx.collapsible', (function(_this) {
        return function() {
          return _this.toggle();
        };
      })(this));
    } else {
      header.on('click', 'dx.collapsible', (function(_this) {
        return function() {
          return _this.toggle();
        };
      })(this));
    }
    content = this.selection.select('.dx-collapsible-content').style('height', 0).style('opacity', 0);
    if (this.options.addIcon) {
      if (toggleBtn.empty()) {
        header.select('i').remove();
        header.prepend('i')["class"]('dx-icon dx-icon-chevron-right dx-collapsible-icon');
      } else {
        toggleBtn.select('i').remove();
        toggleBtn.prepend('i')["class"]('dx-icon dx-icon-chevron-right dx-collapsible-icon');
      }
    }
    this.visible = void 0;
  ...
  Collapsible.prototype.show = function(animate, cb) {
    ...
    if (!this.lazyContentCreated) {
      ...
      if (this.options.lazyContent) {
        this.options.lazyContent(this.selection.select('.dx-collapsible-content').node());
      }
    }
    if (this.visible !== true) {
      this.selection.classed('dx-collapsible-expanded', true).classed('dx-opened', true);
      content = this.selection.select('.dx-collapsible-content');
      if (animate) {
        self = this;
        ...
        this.selection.select('.dx-collapsible-heading').select('.dx-collapsible-icon').morph()["with"]('rotate-90', 200).go(true);
      } else {
  ...
  Collapsible.prototype.hide = function(animate, cb) {
    ...
    if (this.visible !== false) {
      this.selection.classed('dx-collapsible-expanded', false).classed('dx-opened', false);
      if (animate) {
        self = this;
        this.selection.select('.dx-collapsible-content').morph()["with"]('fadeout', 100).and('collapsev', 100).then(function() {
          self.emit('hideend');
          return typeof cb === "function" ? cb() : void 0;
        }).go(true);
        this.selection.select('.dx-collapsible-heading').select('.dx-collapsible-icon').morph()["with"]('rotate-0', 200).go(true);
      } else {
        this.selection.select('.dx-collapsible-content').style('display', 'none');
        this.emit('hideend');
      }
      this.visible = false;
      this.emit('hidestart');
...
dx.Collapsible = Collapsible;

dx.initializeCollapsibles = function(selector, options) {
  return dx.selectAll(selector).nodes.map(function(d) {
...
flatSelect = function(selector) {
  if (selector instanceof dx.Selection) {
    return selector;
  } else {
    return dx.select(selector);
  }
};
  ...
  return function(selector, context) {
    ...
    if (arguments.length > 1) {
      ...
      if (contextArray.indexOf(context) !== -1) {
        ...
        dx.consoleWarning(context + " is not a known context! Accepted values are " + (contextArray.join(', ')));
      }
      return selection;
    } else {
...
dx.palette.context = context(contexts, 'dx');

dx.palette.textContext = context(paletteContexts, 'dx-text');

dx.palette.backgroundContext = context(paletteContexts, 'dx-background');

dx.palette.borderContext = context(paletteContexts, 'dx-border');
...
PreferencesHandler = (function(superClass) {
  ...
  function PreferencesHandler() {
    ...
    this._ = {
      uniqueId: dx.randomId()
    };
    dx.preferences.on('localechange', 'dx.date-time-localizer' + this._.uniqueId, (function(_this) {
      return function() {
        if (!_this._.instanceLocale) {
          return _this.emit('localechange', {
            cause: 'api',
            value: dx.preferences.locale()
          });
        }
      };
    })(this));
    dx.preferences.on('timezonechange', 'dx.date-time-localizer' + this._.uniqueId, (function(_this) {
      return function() {
        if (!_this._.instanceTimezone) {
          return _this.emit('timezonechange', {
            cause: 'api',
            value: dx.preferences.timezone()
          });
        }
      };
    })(this));
  ...
  PreferencesHandler.prototype.locale = function(locale) {
    if (arguments.length) {
      if ((locale == null) || dx.preferences.isLocaleSupported(locale)) {
        this._.instanceLocale = locale ? true : false;
        this._.locale = locale;
        this.emit('localechange', {
          cause: 'api',
          value: locale || dx.preferences.locale()
        });
      } else {
        dx.consoleWarning(locale + ' is not a valid locale. If you think the locale should be added to the list contact the maintainers of hexagon');
      }
      return this;
    } else {
      return this._.locale || dx.preferences.locale();
    }
  };

  PreferencesHandler.prototype.timezone = function(timezone) {
    if (arguments.length) {
      if ((timezone == null) || dx.preferences.isTimezoneSupported(timezone)) {
        this._.instanceTimezone = timezone ? true : false;
        this._.timezone = timezone;
        this.emit('timezonechange', {
          cause: 'api',
          value: timezone || dx.preferences.timezone()
        });
      } else {
        dx.consoleWarning(timezone + ' is not a valid timezone');
      }
      return this;
    } else {
      return this._.timezone || dx.preferences.timezone();
    }
  };
...
DateTimeLocalizer = (function(superClass) {
  ...
  DateTimeLocalizer.prototype.date = function(date, useInbuilt) {
    if (useInbuilt) {
      return date.getFullYear() + '-' + dx.zeroPad(date.getMonth() + 1) + '-' + dx.zeroPad(date.getDate());
    } else {
      return dx.zeroPad(date.getDate()) + '/' + dx.zeroPad(date.getMonth() + 1) + '/' + date.getFullYear();
    }
  };

  DateTimeLocalizer.prototype.time = function(date, showSeconds) {
    var timeString;
    date = dx.preferences.applyTimezoneOffset(date, this.timezone());
    timeString = date.getHours() + ':' + dx.zeroPad(date.getMinutes());
    if (showSeconds) {
      timeString += ':' + dx.zeroPad(date.getSeconds());
    }
    return timeString;
  };
...
dx.dateTimeLocalizer = dateTimeLocalizer;
...
addHoldHandler = function(incrementOnHold, incrementDelay, selection, incrementFn) {
  ...
  if (incrementOnHold) {
    ...
    selection.on('pointerdown', 'dx.number-picker', function(e) {
      var fn;
      holdStart = Date.now();
    ...
    selection.on('pointerup', 'dx.number-picker', function() {
      clearTimers();
      if ((Date.now() - holdStart) < 200) {
        return incrementFn();
      }
    });
    return selection.on('pointerleave', 'dx.number-picker', clearTimers);
  } else {
    return selection.on('click', 'dx.number-picker', function() {
      return incrementFn();
    });
  }
};
...
NumberPicker = (function(superClass) {
  ...
  function NumberPicker(selector, options) {
    ...
    dx.component.register(this.selector, this);
    this.options = dx.merge.defined({
      buttonClass: '',
      min: void 0,
      max: void 0,
    ...
    selection = container["class"]('dx-number-picker');
    incrementButton = selection.append('button').attr('type', 'button')["class"]('dx-number-picker-increment dx-btn ' + this.options.buttonClass);
    incrementButton.append('i')["class"]('dx-icon dx-icon-chevron-up');
    ...
    this.selectInput.on('blur', 'dx.number-picker', (function(_this) {
      return function() {
    ...
    decrementButton = selection.append('button').attr('type', 'button')["class"]('dx-number-picker-decrement dx-btn ' + this.options.buttonClass);
    decrementButton.append('i')["class"]('dx-icon dx-icon-chevron-down');
  ...
  NumberPicker.prototype.value = function(value, screenValue) {
    ...
    if (arguments.length > 0) {
      ...
      selection = dx.select(this.selector);
      selection.select('.dx-number-picker-decrement').attr('disabled', getDisabled(this.options.disabled, newVal, this.min()));
      selection.select('.dx-number-picker-increment').attr('disabled', getDisabled(this.options.disabled, newVal, this.max()));
      if (prevValue !== newVal) {
        this.emit('change', {
          value: newVal
        });
  ...
  NumberPicker.prototype.disabled = function(disable) {
    ...
    if (disable != null) {
      ...
      dx.select(this.selector).selectAll('button').forEach(function(e) {
...
dx.numberPicker = function(options) {
  var selection;
  selection = dx.detached('div');
  new NumberPicker(selection.node(), options);
  return selection;
};

dx.NumberPicker = NumberPicker;
...
populateNode = function(node, items) {
  return dx.select(node).view('.dx-menu-item').update(function(d, element) {
...
setActive = function(menu, pos, up, click) {
  ...
  if ((ref = menu.dropdown._.dropdown) != null) {
    ref.selectAll('.dx-menu-item').classed('dx-menu-active', false);
  }
  if (pos >= 0) {
    ...
    if (goUp && !up) {
      ...
      if (((dNode = (ref3 = menu.dropdown._.dropdown) != null ? ref3.node() : void 0) != null) && !click) {
        menuNode = dx.select(node).classed('dx-menu-active', true);
        mNode = menuNode.node();
        offset = mNode.offsetTop;
        collapsibleHeading = menuNode.select('.dx-menu-collapsible');
        itemHeight = collapsibleHeading.size() > 0 ? collapsibleHeading.node().clientHeight : mNode.clientHeight;
        parentNode = dx.select(mNode.parentNode);
        parentOffset = parentNode.classed('dx-collapsible-content') ? parentNode.node().offsetTop : 0;
...
MenuItem = (function() {
  ...
  MenuItem.prototype.build = function(container) {
    ...
    this.node = container;
    container = dx.select(container);
    if (((ref = this._.menuItems) != null ? ref.length : void 0) > 0) {
      container.view('.dx-collapsible').apply(this);
      collapsibleNode = container.select('.dx-collapsible');
      collapsibleNode.view('.dx-collapsible-heading').apply(this);
      collapsibleNode.view('.dx-collapsible-content').update(function() {
        return this.style('display', 'none');
      }).apply(this);
      headerNode = collapsibleNode.select('.dx-collapsible-heading').classed('dx-menu-collapsible', true).node();
      contentNode = container.select('.dx-collapsible-content').node();
      this.menu.options.renderer(headerNode, this.content);
      this.collapsible = new dx.Collapsible(collapsibleNode.node());
      return populateNode(contentNode, this._.menuItems);
    } else {
      linkEnabled = !this.content.unselectable && !this.content.disabled;
      container.classed('dx-menu-link', linkEnabled).classed('dx-menu-item-disabled', this.content.disabled).classed('dx-menu-unselectable', this.content.unselectable);
...
Menu = (function(superClass) {
  ...
  function Menu(selector, options) {
    ...
    this.options = dx.merge.defined({
      ...
      renderer: function(node, data) {
        return dx.select(node).text(data.text ? data.text : data);
      },
      items: []
    }, options);
    self = this;
    ...
    if ((this.options.dropdownOptions.ddClass != null) && this.options.dropdownOptions.ddClass.length === 0) {
      colorClass = dx.palette.context(this.selector);
    }
    this.options.dropdownOptions.ddClass = 'dx-menu ' + (colorClass != null ? 'dx-' + colorClass : this.options.dropdownOptions.ddClass);
    dropdownContent = function(node) {
      var doneFn, elem, menuItems, rawItems;
      elem = dx.select(node);
      menuItems = elem.select('.dx-menu-items');
      if (menuItems.empty()) {
        menuItems = elem.append('div')["class"]('dx-menu-items');
      }
    ...
    this.dropdown = new dx.Dropdown(this.selector, dropdownContent, this.options.dropdownOptions);
    this.dropdown.on('showend', (function(_this) {
      return function() {
        var ddNode, node;
        if (_this.dropdown._.dropdown != null) {
          node = _this.dropdown._.dropdown.node();
          ddNode = dx.select(node);
          if (node.scrollTop < node.scrollHeight - node.clientHeight) {
            ddNode.style('width', ddNode.width() + dx.scrollbarSize() + 'px');
            if (_this.dropdown._.alignments[2] === 'r') {
              return ddNode.style('left', Math.max(0, ddNode.box().left - dx.scrollbarSize()) + 'px');
            }
          }
        }
      };
    ...
    selection = dx.select(this.selector);
    selection.off('click', 'dx.dropdown');
    selection.on('click', 'dx.menu', (function(_this) {
      return function() {
        var loading;
        if (!_this.options.disabled) {
          if (_this.dropdown.isOpen()) {
            ...
            if (!_this.loading) {
              if ((_this.data != null) && dx.isFunction(_this.data)) {
                _this.loading = true;
                loading = selection.prepend('span');
                loading.append('i')["class"]('dx-menu-loading dx-icon dx-icon-spin dx-icon-spinner');
    ...
    this.dropdown.on('showstart', 'dx.menu', function() {
      var ref;
      self.cursorPos = -1;
      if (!isInput) {
      ...
      targetElem.on('keydown', 'dx.menu', function(e) {
        if (self.dropdown.isOpen()) {
          checkEvent(e, self);
        }
        return self.emit('keydown', e);
      });
      return (ref = self.dropdown._.dropdown) != null ? ref.on('click', 'dx.menu', function(e) {
        var allItems, i, index, t, target;
        target = dx.select(e.target).classed('dx-menu-link') ? e.target : dx.select(e.target).closest('.dx-menu-item').node();
        if (target) {
          index = -1;
          t = dx.select(target);
          if (t.classed('dx-menu-link')) {
  ...
  Menu.prototype.disabled = function(disabled) {
    if (disabled != null) {
      ...
      dx.select(this.selector).attr('disabled', disabled ? true : void 0).classed('dx-disabled', disabled);
...
sortItems = function(valueLookup) {
  if (valueLookup == null) {
    valueLookup = dx.identity;
  }
  return function(a, b) {
    return dx.sort.compare(valueLookup(a), valueLookup(b));
  };
};
...
sortActive = function(items) {
  ...
  groupedActive = new dx._.Map(dx.groupBy(items, function(i) {
    return !i.disabled;
  }));
...
AutocompleteFeed = (function() {
  function AutocompleteFeed(options) {
    ...
    resolvedOptions = dx.merge.defined(defaults, options);
    if (resolvedOptions.filter == null) {
      resolvedOptions.filter = function(items, term) {
        var active, filtered, inactive, ref;
        filtered = dx.filter[resolvedOptions.matchType](items, term, resolvedOptions.filterOptions);
    ...
    this._ = {
      ...
      resultsCache: new dx._.Map
    };
  }

  AutocompleteFeed.prototype.clearCache = function() {
    this._.resultsCache = new dx._.Map;
    return this;
  };
  ...
  AutocompleteFeed.prototype.filter = function(term, callback) {
    ...
    if (term === void 0) {
      ...
      } else if (_.options.matchType === 'external' && dx.isFunction(_.items)) {
  ...
  AutocompleteFeed.prototype.validateItems = function(items) {
    return dx.isArray(items) || dx.isFunction(items);
  };
...
dx.AutocompleteFeed = AutocompleteFeed;
...
buildCalendar = function(datepicker, mode) {
  ...
  _ = datepicker._;
  _.calendarGrid.selectAll('.dx-grid-row').remove();
  _.calendarView = _.calendarGrid.view('.dx-grid-row', 'div').update(function(d, e, i) {
...
calendarGridUpdate = function(datepicker, data, elem, index, mode) {
  var _, element;
  _ = datepicker._;
  element = dx.select(elem);
  if (data === 'days') {
    return element.classed('dx-grid-row-heading', true).view('.dx-grid').update(function(d) {
      return this.text(d).node();
    }).apply(datepicker.localizer.weekDays());
  } else {
    return element.view('.dx-grid').enter(function(d) {
      var node;
      node = this.append('div')["class"]('dx-grid');
      if (datepicker.options.selectRange) {
        node.append('div')["class"]('dx-grid-range-bg');
      }
      node.append('div')["class"]('dx-grid-text');
      return node.node();
...
calendarGridRowUpdate = function(datepicker, data, elem, index, rowIndex, mode) {
  ...
  element = dx.select(elem);
  if (data == null) {
    return element.classed('dx-grid-out-of-range', true).classed('dx-grid-selected', false).classed('dx-grid-selected-start', false).classed('dx-grid-selected-end', false).classed('dx-grid-selected-range', false).classed('dx-grid-today', false);
  } else {
    year = null;
    month = null;
    day = null;
    ...
    element.classed('dx-grid-out-of-range', !isValid).classed('dx-grid-selected', selected).classed('dx-grid-selected-start', selectedS).classed('dx-grid-selected-end', selectedE).classed('dx-grid-selected-range', betweenDates).classed('dx-grid-today', today);
    element.select('.dx-grid-text').text(screenVal);
    if (isValid) {
      return element.on('click', 'dx-date-picker', function() {
        var date, nMode;
        _.userEvent = true;
        if (mode !== 'd' && mode !== 'y') {
          if (day != null) {
...
DatePicker = (function(superClass) {
  ...
  function DatePicker(selector, options) {
    ...
    this.localizer.on('timezonechange', 'dx.date-picker', (function(_this) {
      return function() {
        return updateDatepicker(_this, true);
      };
    })(this));
    ...
    if (this.options.selectRange) {
      ...
      inputUpdate = function(which) {
        ...
        return timeout = setTimeout(function() {
          ...
          _.inputStart.classed('dx-date-error', !startValid);
          return _.inputEnd.classed('dx-date-error', !endValid);
        }, 500);
      };
      _.inputStart = inputContainer.append('input')["class"]('dx-date-input').on('input', 'dx.date-picker', function() {
        return inputUpdate(false);
      });
      inputContainer.append('i')["class"]('dx-date-to-icon dx-icon dx-icon-angle-double-right');
      _.inputEnd = inputContainer.append('input')["class"]('dx-date-input').on('input', 'dx.date-picker', function() {
      ...
      if (_.useInbuilt) {
        _.input.attr('type', 'date').on('focus', 'dx.date-picker', function() {
    ...
    if (this.options.type === 'calendar') {
      ...
      _.calendarGrid = calendarElem.append('div')["class"]('dx-calendar-grid');
      if (this.options.showTodayButton) {
        _.calendarTodayButton = calendarElem.append('div')["class"]('dx-calendar-today-btn').append('button')["class"]('dx-btn dx-btn-invert').on('click', 'dx.date-picker', function() {
          var date;
          date = new Date();
          date.setHours(0, 0, 0, 0);
          self.date(date);
      ...
      dayNode = dx.detached('div').node();
      monthNode = dx.detached('div').node();
      yearNode = dx.detached('div').node();
      _.dayPicker = new dx.NumberPicker(dayNode, {
        buttonClass: 'dx-btn-invert'
      }).on('change', 'dx.date-picker', function(e) {
        _.userEvent = true;
        return self.day(e.value);
      });
      _.monthPicker = new dx.NumberPicker(monthNode, {
        buttonClass: 'dx-btn-invert'
      }).on('change', 'dx.date-picker', function(e) {
        _.userEvent = true;
        return self.month(e.value);
      });
      _.yearPicker = new dx.NumberPicker(yearNode, {
        buttonClass: 'dx-btn-invert'
      }).on('change', 'dx.date-picker', function(e) {
        _.userEvent = true;
    ...
    if (!_.useInbuilt) {
      this.dropdown = new dx.Dropdown(this.selector, setupDropdown, {
        matchWidth: false
      });
  ...
  DatePicker.prototype.visibleMonth = function(month, year) {
    ...
    if (this.options.type === 'calendar') {
      ...
      dx.consoleWarning('Setting the visible month only applies to date pickers of type \'calendar\'');
      return this;
    }
  };
  ...
  DatePicker.prototype.range = function(range) {
    ...
    if (this.options.selectRange) {
      ...
      dx.consoleWarning('datePicker.range can only be used for datepickers with \'selectRange\' of true');
      return this;
    }
  };
...
dx.datePicker = function(options) {
  var selection;
  selection = dx.detached('div');
...
setupInput = function(timepicker) {
  timepicker.input.classed('dx-short-time', !timepicker.options.showSeconds);
...
updateTimePicker = function(timepicker, suppress) {
  ...
  if (!timepicker.preventFeedback) {
    ...
    timepicker.input.classed('dx-time-error', false);
...
TimePicker = (function(superClass) {
  ...
  function TimePicker(selector, options) {
    ...
    this.localizer = dx.dateTimeLocalizer();
    this.localizer.on('localechange', 'dx.time-picker', (function(_this) {
      return function() {
        return updateTimePicker(_this, true);
      };
    })(this));
    this.localizer.on('timezonechange', 'dx.time-picker', (function(_this) {
      return function() {
        return updateTimePicker(_this, true);
      };
    })(this));
    ...
    this.selection = dx.select(this.selector).classed('dx-time-picker', true);
    inputContainer = this.selection.append('div')["class"]('dx-time-input-container');
    icon = inputContainer.append('i')["class"]('dx-icon dx-icon-clock-o');
    this.input = inputContainer.append('input')["class"]('dx-time-input');
    timeout = void 0;
    if (dx.supports('date') && dx.supports('touch')) {
      this.input.attr('readonly', true);
      this.input.on('click', 'dx.time-picker', function(event) {
        return event.preventDefault();
      });
    } else {
      this.input.on('input', 'dx.time-picker', (function(_this) {
        return function(event) {
          ...
          return timeout = setTimeout(function() {
            ...
            if (_this.localizer.checkTime(time)) {
              ...
              return _this.input.classed('dx-time-error', true);
            }
          }, 200);
        };
      })(this));
    ...
    setupInput(this);
    hourNode = dx.detached('div').node();
    minuteNode = dx.detached('div').node();
    second = dx.detached('div').node();
    this.hourPicker = new dx.NumberPicker(hourNode, {
      buttonClass: this.options.buttonClass
    }).on('change', 'dx.time-picker', (function(_this) {
      return function(e) {
        _.userEvent = true;
    ...
    this.minutePicker = new dx.NumberPicker(minuteNode, {
      buttonClass: this.options.buttonClass
    }).on('change', 'dx.time-picker', (function(_this) {
      return function(e) {
        _.userEvent = true;
    ...
    this.secondPicker = new dx.NumberPicker(second, {
      buttonClass: this.options.buttonClass
    }).on('change', 'dx.time-picker', (function(_this) {
      return function(e) {
        _.userEvent = true;
    ...
    this.dropdown = new dx.Dropdown(this.selector, setupDropdown, {
      matchWidth: false,
      ddClass: 'dx-time-picker-dropdown'
    });
    this.dropdown.on('hidestart', (function(_this) {
      return function() {
        return _this.emit('hide');
...
dx.timePicker = function(options) {
  var selection;
  selection = dx.detached('div');
...
findTerm = function(term, forceMatch) {
  ...
  if (this.options.showOtherResults && !forceMatch && !dataMatches) {
    ...
    remainingResults = filteredData.length === 0 ? allData : (data = allData.filter(function(d) {
      ...
      return dx.sort.compare(a, b);
    }) : data = data.sort(dx.sort.compare) : void 0, data);
...
AutoComplete = (function(superClass) {
  ...
  function AutoComplete(selector, data1, opts) {
    ...
    if (!dx.isArray(this.data) && !dx.isFunction(this.data)) {
      dx.consoleWarning('AutoComplete - ', this.selector, ': data set incorrectly - you supplied: ', this.data, ' but should have been an array of items or a function');
    } else {
      dx.component.register(this.selector, this);
      this.options = dx.merge.defined({
        minLength: 0,
      ...
      this.options.filterOptions = dx.merge({}, _filterOpts, opts.filterOptions);
      if ((base = this.options).filter == null) {
        base.filter = (function(_this) {
          return function(arr, term) {
            var active, filtered, inactive, ref;
            filtered = dx.filter[self.options.matchType](arr, term, self.options.filterOptions);
            ref = sortActive(filtered), active = ref.active, inactive = ref.inactive;
            return slice.call(active).concat(slice.call(inactive));
          };
        })(this);
      ...
      if ((base1 = this.options).renderer == null) {
        base1.renderer = this.options.inputMap != null ? function(elem, item) {
          return dx.select(elem).text(self.options.inputMap(item));
        } : function(elem, item) {
          return dx.select(elem).text(item);
        };
      }
      ...
      menu = new dx.Menu(this.selector, {
        dropdownOptions: {
          ddClass: 'dx-autocomplete-dropdown'
        }
      });
      menu.pipe(this, '', ['highlight']);
      menu.dropdown.pipe(this, 'dropdown');
      dx.select(this.selector).off('click', 'dx.menu');
      menu.on('input', 'dx.autocomplete', function(e) {
      ...
      input.on('focus', 'dx.autocomplete', function(e) {
        if (!_.ignoreNextFocus) {
          _.cleanUp = false;
          return self.show();
        }
      });
      input.on('blur', 'dx.autocomplete', function(e) {
      ...
      timeout = void 0;
      input.on('input', 'dx.autocomplete', function() {
        _.cleanUp = false;
        clearTimeout(timeout);
      ...
      menu.on('change', 'dx.autocomplete', function(d) {
        var content;
      ...
      menu.on('dropdown.change', 'dx.autocomplete', function(visible) {
        if (!!visible) {
          _.initialValue = input.value();
      ...
      menu.on('click', 'dx.autocomplete', function() {
  ...
  AutoComplete.prototype.clearCache = function() {
    this._.data = new dx._.Map();
    if ((this.data != null) && !dx.isFunction(this.data)) {
...
dx.autoComplete = function(data, options) {
  var selection;
  selection = dx.detached('input');
...
startDrag = function(container, elem, controlElem, e) {
  ...
  if (_.dragging) {
    ...
    dx.select(document).on('pointermove', 'dx.drag-container', function(e) {
      return drag(container, elem, controlElem, e);
    }).on('pointerup', 'dx.drag-container', function(e) {
    ...
    _.placeholder = elem.insertAfter(elem.clone(true).clear()).classed('dx-drag-placeholder', true);
    ...
    elem.classed('dx-drag-current', true).style('width', _.origWidth + 'px');
...
endDrag = function(container, elem) {
  ...
  if (_.dragging) {
    _.dragging = false;
    dx.select(document).off('pointermove', 'dx.drag-container').off('pointerup', 'dx.drag-container');
    _.placeholder.insertAfter(elem);
    _.placeholder.remove();
    elem.style('top', '').style('left', '').style('width', '').classed('dx-drag-current', false);
...
containerChildren = function(container) {
  ...
  return items.filter(dx.defined);
};
...
DragContainer = (function(superClass) {
  ...
  function DragContainer(selector, options) {
    ...
    dx.component.register(selector, this);
    this.selection = dx.select(selector).classed('dx-drag-container', true);
    options = dx.merge({
      lookup: function(node) {
        return dx.select(node).attr('data-id');
      },
      resizeOnDrag: false,
      order: void 0
    }, options);
  ...
  DragContainer.prototype.setup = function() {
    this.selection.selectAll('.dx-drag-control').off('pointerdown', 'dx.drag-container');
    containerChildren(this).forEach((function(_this) {
      return function(elem) {
        var controlElem;
        elem = dx.select(elem);
        if (elem.classed('dx-drag-element')) {
          controlElem = dx.select(elem.select('.dx-drag-control').node() || elem.node());
          return controlElem.classed('dx-drag-control', true).on('pointerdown', 'dx.drag-container', function(evt) {
            if (!controlElem.classed('dx-drag-disabled')) {
  ...
  DragContainer.prototype.order = function(order) {
    ...
    if (arguments.length > 0) {
      ...
      return containerChildren(this).map(this.lookup()).filter(dx.defined);
    }
  };
...
dx.dragContainer = function(options) {
  var selection;
  selection = dx.detached('div');
...
dx.DragContainer = DragContainer;
...
factory = function(type, clasz) {
  return function() {
    return dx.detached(type)["class"](clasz);
  };
};

dx.div = function(cls) {
  if (cls) {
    return dx.detached('div')["class"](cls);
  } else {
    return dx.detached('div');
  }
};

dx.span = function(cls) {
  if (cls) {
    return dx.detached('span')["class"](cls);
  } else {
    return dx.detached('span');
  }
};

dx.inputGroup = factory('div', 'dx-input-group');

dx.notice = factory('div', 'dx-notice');

dx.notice.head = factory('div', 'dx-notice-head');

dx.notice.body = factory('div', 'dx-notice-body');

dx.spinner = factory('span', 'dx-spinner');

dx.spinner.wide = factory('div', 'dx-spinner-wide');

dx.icon = function(options) {
  return dx.detached('i')["class"](options != null ? options["class"] : void 0);
};

dx.button = function(options) {
  return dx.palette.context(dx.detached('button').attr('type', 'button')["class"]('dx-btn'), options != null ? options.context : void 0);
};

dx.label = function(options) {
  return dx.palette.context(dx.detached('span')["class"]('dx-label'), options != null ? options.context : void 0);
};

dx.group = factory('div', 'dx-group dx-horizontal');

dx.group.vertical = factory('div', 'dx-group dx-vertical');

dx.group.fixed = factory('div', 'dx-group dx-horizontal dx-fixed');

dx.group.vertical.fixed = factory('div', 'dx-group dx-vertical dx-fixed');

dx.section = factory('div', 'dx-section');

dx.section.fixed = factory('div', 'dx-section dx-fixed');

dx.checkbox = function() {
  return dx.detached('input').attr('type', 'checkbox');
};
...
replaceWithDiv = function(sel) {
  return sel.replace(dx.div('dx-logo'));
};

replaceLogos = function() {
  var logos;
  logos = dx.selectAll('img.dx-logo');
  if (logos.size()) {
    dx.consoleWarning('Logo:', 'The .dx-logo class should only be applied to <div> elements.', 'You supplied: ', logos.nodes);
...
ProgressBar = (function() {
  function ProgressBar(selector, options) {
    ...
    this.selection = dx.select(this.selector).classed('dx-progress-bar', true);
    this.innerBars = this.selection.append('div').attr('class', 'dx-progress-bar-inner');
    this.value(options.value);
    if (options.segments != null) {
      this.segments(options.segments, true);
    }
    if (options.animate) {
      this.selection.classed('dx-animate', true);
    }
  }
    ...
    if (arguments.length > 0) {
      if (segments != null) {
        ...
        if (this.progressSegments.length > 0) {
          this.selection.selectAll('.dx-progress-bar-inner').remove();
          ...
          this.progressSegments.forEach((function(_this) {
            return function(segment, i) {
              var bar, maxWidth;
              bar = _this.selection.append('div').attr('class', 'dx-progress-bar-inner');
        ...
        this.selection.selectAll('.dx-progress-bar-inner').remove();
        this.selection.append('div').attr('class', 'dx-progress-bar-inner');
      }
      this.innerBars = this.selection.selectAll('.dx-progress-bar-inner');
      if (retainProgress) {
...
dx.progressBar = function(options) {
  var selection;
  selection = dx.detached('div');
...
doCollisionDetection = function(nodesRaw) {
  ...
  reductor = function(oldDistance, arg) {
    ...
    tuple = dx.find(previousNodes, function(arg1) {
...
arcCurve = function(x, y, innerRadius, outerRadius, startRadians, endRadians, padding, dontCurveCenter) {
  ...
  pushPoints = function(startPoint, endPoint, radius) {
    ...
    for (i = j = ref = startPoint, ref1 = endPoint; ref <= ref1 ? j <= ref1 : j >= ref1; i = ref <= ref1 ? ++j : --j) {
      ...
      theta = dx.clamp(startRadians, endRadians, theta);
...
extent2 = function(data, f, g) {
  ...
  if (data.length > 0) {
    ...
    for (j = 0, len = data.length; j < len; j++) {
      d = data[j];
      min = dx.min([min, f(d), g(d)]);
      max = dx.max([max, f(d), g(d)]);
    }
    return [min, max];
  } else {
    return void 0;
...
createLinearGradient = function(parent, values, series) {
  ...
  gradientId = dx.randomId();
  dx.select(parent).select('.dx-linear-gradient').remove();
  linearGradient = dx.select(parent).append('linearGradient').attr('class', 'dx-linear-gradient').attr('id', gradientId).attr('gradientUnits', "userSpaceOnUse").attr('x1', 0).attr('x2', 0).attr('y1', series.axis.yScale.rangeMin).attr('y2', series.axis.yScale.rangeMax);
  values.forEach(function(value) {
    return linearGradient.append('stop').attr('offset', ((value.yValue - series.axis.yScale.domainMin) / (series.axis.yScale.domainMax - series.axis.yScale.domainMin) * 100) + '%').attr('stop-color', dx.color(value.color).alpha(1).toString()).attr('stop-opacity', dx.color(value.color).alpha());
  });
  return gradientId;
};
...
populateLegendSeries = function(selection, series) {
  ...
  background = selection.select('.dx-legend-box');
  if (background.size() === 0) {
    background = selection.append('rect')["class"]('dx-legend-box');
  }
  selection.view('.dx-legend-entry', 'g').enter(function() {
    selection = this.append('g')["class"]('dx-legend-entry');
    selection.append('text');
    selection.append('rect');
    return selection.node();
  }).update(function(s, e, i) {
    this.select('text').text(dx.isFunction(s.title) ? s.title() : s.name).attr('y', i * 20 + 10).attr('x', 15);
    return this.select('rect').text(dx.isFunction(s.title) ? s.title() : s.name).attr('y', i * 20).attr('x', 0).attr('width', 10).attr('height', 10).attr('fill', s.legendColor ? s.legendColor() : s.fillColor);
  }).apply(series);
  width = dx.max(selection.selectAll('text').nodes.map(function(node) {
  ...
  function Series(options) {
    ...
    this._ = {
      options: dx.merge({
        ...
        data: void 0,
        labelsEnabled: true,
        labelRenderer: dx.plot.label.standard,
...
Axis = (function() {
  ...
  function Axis(options) {
    ...
    this._ = {
      series: new dx._.List
    };
    this.x = dimension(this, dx.merge({
      axisTickLabelPosition: 'bottom'
    }, options != null ? options.x : void 0));
    this.y = dimension(this, dx.merge({
      axisTickLabelPosition: 'left'
    ...
    if (dx.isString(series)) {
      series = (function() {
        switch (series) {
          ...
          default:
            dx.consoleWarning(series + ' is not a valid series type');
            return void 0;
        }
      })();
  ...
  Axis.prototype.series = function(series) {
    var k, len, s;
    if (arguments.length > 0) {
      this._.series = new dx._.List(series);
  ...
  Axis.prototype.setupAxisSvg = function(element) {
    ...
    gridGroup = dx.select(element).append('g')["class"]('dx-axis-grid');
    xAxisGroup = dx.select(element).append('g')["class"]('dx-x-axis');
    xAxisGroup.append('g')["class"]('dx-axis-scale');
    yAxisGroup = dx.select(element).append('g')["class"]('dx-y-axis');
    return yAxisGroup.append('g')["class"]('dx-axis-scale');
  };
  ...
  Axis.prototype.tagSeries = function() {
    ...
    groupTypeEntries = function(data) {
      ...
      for (k = 0, len = data.length; k < len; k++) {
        ...
        if (!groups.has(group)) {
          groups.set(group, new dx._.List);
        }
    ...
    for (k = 0, len = ref.length; k < len; k++) {
      ...
      if (!types.has(series._.type)) {
        types.set(series._.type, new dx._.List);
      }
  ...
  Axis.prototype.preupdateXAxisSvg = function(element) {
    ...
    if (this.x.scaleType() === 'discrete') {
      ...
      xs = xs.filter(dx.identity);
      xmin = dx.min(xs.map(function(d) {
        return d[0];
      }));
      xmax = dx.max(xs.map(function(d) {
        return d[1];
      }));
    ...
    xLabelTickSize = 0;
    if (!this.x.visible()) {
      dx.select(element).select('.dx-x-axis').remove();
    } else {
      axisGroupSelection = dx.select(element).select('.dx-x-axis');
      ...
      axisGroupSelection.select('.dx-axis-scale').view('.dx-axis-view', 'g').update(function(scale) {
        return this.view('.dx-tick', 'g').update(function(tick) {
          return this.view('.dx-tick-text-x', 'text').update(function(t) {
            var bbox, size;
      ...
      if (this.x.title()) {
        axisGroupSelection.view('.dx-axis-title', 'text').update(function(d) {
  ...
  Axis.prototype.preupdateYAxisSvg = function(element, totalXAxisSize) {
    ...
    yLabelTickSize = 0;
    if (!this.y.visible()) {
      dx.select(element).select('.dx-y-axis').remove();
    } else {
      axisGroupSelection = dx.select(element).select('.dx-y-axis');
      axisGroupSelection.select('.dx-axis-scale').view('.dx-axis-view', 'g').update(function(scale) {
        return this.view('.dx-tick', 'g').update(function(tick) {
          return this.view('.dx-tick-text-y', 'text').update(function(t) {
            var size;
  ...
  Axis.prototype.updateAxisSvg = function(element, xOffset, yOffset, totalXOffset, totalYOffset) {
    ...
    gridSelection = dx.select(element).select('.dx-axis-grid');
    if (!this.x.visible()) {
      dx.select(element).select('.dx-x-axis').remove();
    } else {
      if (this.x.gridLines()) {
        gridSelection.view('.dx-vertical-grid-line', 'line').update(function(tick) {
      ...
      axisUpdateFunc = function(scale) {
        var nodes;
        this.view('.dx-axis-line', 'line').update(function(s) {
          this.attr('x1', s.rangeMin).attr('x2', s.rangeMax);
          return this.attr('y1', axisY).attr('y2', axisY);
        }).apply(scale);
        this.view('.dx-tick', 'g').update(function(tick, e, i) {
          this.attr("transform", "translate(" + tick[1] + "," + markerY + ")");
          this.view('.dx-tick-line', 'line').update(function(t) {
        ...
        if (self.x.showTicks() && self.x.doCollisionDetection()) {
          nodes = this.selectAll('.dx-tick-text-x').filter(function(x) {
            return x.text();
          }).nodes;
          if (nodes.length) {
      ...
      axisGroupSelection.select('.dx-axis-scale').view('.dx-axis-view', 'g').update(axisUpdateFunc).apply(this.xScale);
      if (this.x.title()) {
        axisGroupSelection.view('.dx-axis-title', 'text').update(function(d) {
    ...
    if (!this.y.visible()) {
      return dx.select(element).select('.dx-y-axis').remove();
    } else {
      if (this.y.gridLines()) {
        gridSelection.view('.dx-horizontal-grid-line', 'line').update(function(tick) {
      ...
      axisGroupSelection.select('.dx-axis-scale').view('.dx-axis-view', 'g').update(function(scale) {
        ...
        return this.view('.dx-tick', 'g').update(function(tick, e, i) {
      ...
      if (this.y.title()) {
        return axisGroupSelection.view('.dx-axis-title', 'text').update(function(d) {
          var translateX, translateY;
  ...
  Axis.prototype.updateDataSvg = function(fillLayer, sparseLayer) {
    ...
    fill = [];
    sparse = [];
    dx.select(fillLayer).view('.dx-series', 'g').update(function(d, e) {
      return fill.push(e);
    }).apply(this.series());
    dx.select(sparseLayer).view('.dx-series', 'g').update(function(d, e) {
  ...
  Axis.prototype.getLabelDetails = function(x, y) {
    var labels;
    labels = dx.flatten(this.series().map(function(series) {
  ...
  Axis.prototype.getYStack = function(type, group, x, seriesId, start) {
    ...
    if (group) {
      ...
      return yStack + dx.sum(maybeys.filter(dx.identity));
    } else {
      return Math.max(start, 0);
    }
  };
  ...
  Axis.prototype.calculateYBounds = function(yMinMightBeAuto, yMaxMightBeAuto) {
    ...
    if ('auto' === yMaxMightBeAuto || 'auto' === yMinMightBeAuto) {
      ...
      types = dx.groupBy(allSeries, function(d) {
        return d._.type;
      });
      stackGroups = types.map(function(arg) {
        ...
        return {
          type: type,
          group: dx.groupBy(series, function(s) {
            if (supportsGroup(s)) {
              return s.group();
            } else {
              return void 0;
      ...
      typeGroupReductor = function(type) {
        return function(arg, arg1) {
          ...
          ref = seriesGroup === void 0 ? (maybeys = allSeries.map(function(s) {
            ...
            yymin: dx.min(ys.map(function(d) {
              return d[0];
            })),
            yymax: dx.max(ys.map(function(d) {
              return d[1];
            }))
          }) : (allX = dx.unique(dx.flatten(series.map(function(s) {
            ...
            return dx.sum(maybeys.filter(dx.identity));
          }), {
            yymin: dx.min(stackHeights),
            yymax: dx.max(stackHeights)
...
Graph = (function(superClass) {
  ...
  function Graph(selector, options) {
    ...
    dx.component.register(this.selector, this);
    this._ = {
      options: dx.shallowMerge({
        zoomRangeStart: 0,
        zoomRangeEnd: 1,
        labelsEnabled: true,
        legendsEnabled: false,
        legendLocation: 'auto',
        noDataText: dx.userFacingText('plot', 'noData'),
        redrawOnResize: true
      }, options),
      axes: new dx._.List
    };
    id = dx.randomId();
    selection = dx.select(this.selector);
    selection.on('resize', 'dx.plot', (function(_this) {
      return function() {
    ...
    this.svgTarget = selection.append("svg").attr('class', 'dx-graph');
    defs = this.svgTarget.append('defs');
    this.axesTarget = this.svgTarget.append('g').attr('class', 'dx-axes');
    this.plotTarget = this.svgTarget.append('g').attr('class', 'dx-plot');
    ...
    savedZoomStart = 0;
    savedZoomEnd = 1;
    this.svgTarget.on('pointerdown', 'dx.plot', (function(_this) {
      return function(p) {
        var x, y;
    ...
    threshold = 0.01;
    this.svgTarget.on('touchmove', 'dx.plot', (function(_this) {
      return function(e) {
        ...
        if (e.targetTouches.length > 1 && _this.zoomEnabled()) {
          ...
          _this.zoomRangeStart(dx.clampUnit(xhat + z * startFactor));
          _this.zoomRangeEnd(dx.clampUnit(xhat + z * endFactor));
          _this.emit('zoom', {
    ...
    this.svgTarget.on('mousemove', 'dx.plot', (function(_this) {
      return function(p) {
        ...
        if (_this.legendEnabled()) {
          legendContainer = _this.svgTarget.select('.dx-legend-container');
    ...
    this.svgTarget.on('mouseleave', 'dx.plot', (function(_this) {
      return function() {
        if (_this.legendEnabled() && _this.legendLocation() === 'hover') {
          return _this.svgTarget.select('.dx-legend-container').style('display', 'none');
        }
      };
    })(this));
    this.svgTarget.on('pointerleave', 'dx.plot', function(p) {
      return clearLabels();
    });
    this.svgTarget.on('click', 'dx.plot', (function(_this) {
      return function(p) {
    ...
    this.svgTarget.on('wheel', 'dx.plot', (function(_this) {
      return function(e) {
  ...
  Graph.prototype.axes = function(axes) {
    if (arguments.length > 0) {
      this._.axes = new dx._.List(axes);
  ...
  Graph.prototype.render = function() {
    ...
    hasData = this.axes().some(function(axis) {
      return axis.series().some(function(series) {
        var data;
        data = series.data();
        return dx.isObject(data) || data.length > 0;
      });
    });
    self = this;
    this.svgTarget.view('.dx-plot-no-data', 'text').update(function() {
    ...
    enter = function(d) {
      var node;
      node = this.append('g')["class"]('dx-axis').node();
      d.setupAxisSvg(node);
      return node;
    };
    totalX = 0;
    this.axesTarget.view('.dx-axis', 'g').enter(enter).update(function(d, element) {
    ...
    this.plotTarget.view('.dx-axis-data', 'g').enter(function() {
      var g;
      g = this.append('g')["class"]('dx-axis-data');
      g.append('g')["class"]('dx-graph-fill-layer');
      g.append('g')["class"]('dx-graph-sparse-layer');
      return g.node();
    }).update(function(d, element) {
      return d.updateDataSvg(this.select('.dx-graph-fill-layer').node(), this.select('.dx-graph-sparse-layer').node());
    }).apply(this.axes());
    if (this.legendEnabled()) {
      legendContainer = this.svgTarget.select('.dx-legend-container');
      if (legendContainer.size() === 0) {
        legendContainer = this.svgTarget.append('g')["class"]('dx-legend-container');
      }
      populateLegendSeries(legendContainer, dx.flatten(this.axes().map(function(axis) {
        return axis.series();
      })));
      ...
      this.svgTarget.select('.dx-legend-container').remove();
    }
  ...
  getClosestMeta = function(graph, x, y) {
    ...
    x = dx.clamp(graph.plotArea.x1, graph.plotArea.x2, x);
    y = dx.clamp(graph.plotArea.y1, graph.plotArea.y2, y);
    labels = dx.flatten(graph.axes().map(function(axis) {
  ...
  clearLabels = function() {
    return dx.select('body').select('.dx-plot-label-container').clear();
  };

  updateLabels = function(graph, x, y) {
    var bestMeta, updateLabel;
    updateLabel = function(data, element) {
      dx.select(element).style('left', Math.round(window.pageXOffset + graph.svgTarget.box().left + data.x) + 'px').style('top', Math.round(window.pageYOffset + graph.svgTarget.box().top + data.y) + 'px');
      return data.series.labelRenderer()(element, data);
    };
    bestMeta = getClosestMeta(graph, x, y);
    if (dx.select('body').select('.dx-plot-label-container').empty()) {
      dx.select('body').append('div')["class"]('dx-plot-label-container');
    }
    return dx.select('body').select('.dx-plot-label-container').view('.dx-plot-label', 'div').update(updateLabel).apply(bestMeta ? boundLabel(bestMeta, graph) : []);
  };
...
BandSeries = (function(superClass) {
  ...
  function BandSeries(options) {
    BandSeries.__super__.constructor.call(this, dx.merge({
      fillColor: dx.color(dx.theme.plot.colors[2]).alpha(0.2).toString(),
      sampleThreshold: 200
    }, options));
    this._.type = 'band';
  }
  ...
  BandSeries.prototype.updateSvg = function(fillLayer) {
    ...
    return dx.select(fillLayer).view('.dx-series-data', 'path', 'dx-series-area').update(function(d) {
      return this.attr('d', d)["class"]('dx-series-data ' + self["class"]()).attr('fill', fillCol);
    }).apply(areas);
  };
...
BarSeries = (function(superClass) {
  ...
  function BarSeries(options) {
    BarSeries.__super__.constructor.call(this, dx.merge({
      fillColor: dx.theme.plot.colors[1],
      group: void 0
    }, options));
    this._.type = 'bar';
  }
  ...
  BarSeries.prototype.updateSvg = function(fillLayer) {
    ...
    self = this;
    axis = this.axis;
    return dx.select(fillLayer).view('.dx-series-data', 'rect').update(function(d) {
      ...
      return this["class"]('dx-series-data dx-series-bar ' + self["class"]()).attr("y", y).attr("x", x).attr("height", Math.max(height, 0)).attr("width", Math.max(width, 0)).style("fill", d.color || self.fillColor());
    }).apply(this.data());
  };
  ...
  BarSeries.prototype.getLabelDetails = function(x, y) {
    ...
    if (this.labelsEnabled()) {
      ...
      barData = dx.find(this.data(), function(d) {
        return d.x === xx;
      });
      if (barData) {
        ...
        if ((xx != null) && (yy != null) && (barData != null)) {
          ...
          yy = dx.clamp(min, max, this.axis.yScale.apply(yy));
          meta = {
            series: this,
            title: this.title(),
            x: barX + width / 2,
...
updatePath = function(series, element, _class, data, type, update) {
  return dx.select(element).view('.dx-series-data', 'g').update(function(d) {
    return this["class"]('dx-series-data dx-series-line ' + series["class"]()).view(_class, type).update(update).apply(d);
  }).apply(data);
};
...
LineSeries = (function(superClass) {
  ...
  function LineSeries(options) {
    LineSeries.__super__.constructor.call(this, dx.merge({
      strokeEnabled: true,
      strokeColor: dx.theme.plot.colors[0],
      fillEnabled: false,
      fillColor: void 0,
  ...
  LineSeries.prototype.updateSvg = function(fillLayer, sparseLayer) {
    ...
    if (this.fillEnabled()) {
      if (Array.isArray(self.fillColor() || self.strokeColor())) {
        gradientCols = self.fillColor() || self.strokeColor().map(function(d) {
          return {
            value: d.value,
            color: dx.color(d.color).alpha(0.1).toString('rgba')
          };
        });
        gradientId = createLinearGradient(fillLayer, gradientCols, self);
        fillCol = 'url(#' + gradientId + ')';
      } else {
        fillCol = self.fillColor() || dx.color(self.strokeColor()).alpha(0.1).toString();
      }
      ...
      updatePath(this, fillLayer, '.dx-series-line-fill', areas, 'path', function(d) {
    ...
    if (this.strokeEnabled()) {
      ...
      updatePath(this, sparseLayer, '.dx-series-line-stroke', curves, 'path', function(d) {
        return this.attr('d', d).attr('stroke', strokeCol);
      });
    }
    if (this.markersEnabled()) {
      preparedData = this.axis.y.scaleType() !== 'discrete' ? applyStack(dx.flatten(featheredData)) : dx.flatten(featheredData);
      return updatePath(this, sparseLayer, '.dx-series-line-markers', scale(preparedData, this.axis), 'circle', function(d) {
...
ScatterSeries = (function(superClass) {
  ...
  function ScatterSeries(options) {
    ScatterSeries.__super__.constructor.call(this, dx.merge({
      fillColor: dx.theme.plot.colors[3],
      radius: 2
    }, options));
    this._.type = 'scatter';
  }
  ...
  ScatterSeries.prototype.updateSvg = function(fillLayer, sparseLayer) {
    var self;
    self = this;
    return dx.select(sparseLayer).view('.dx-series-data', 'circle').update(function(d) {
      return this["class"]('dx-series-data dx-series-scatter ' + self["class"]()).attr('cx', d.x).attr('cy', d.y).attr('r', Math.max(d.radius || self.radius(), 0)).style('fill', d.fillColor || self.fillColor());
...
StraightLineSeries = (function(superClass) {
  ...
  function StraightLineSeries(options) {
    StraightLineSeries.__super__.constructor.call(this, dx.merge({
      strokeColor: dx.theme.plot.colors[4],
      data: {}
    }, options));
  ...
  StraightLineSeries.prototype.updateSvg = function(fillLayer, sparseLayer) {
    ...
    if (data) {
      self = this;
      return dx.select(sparseLayer).view('.dx-series-data', 'line').update(function(d) {
        return this["class"]('dx-series-data dx-series-constant ' + self["class"]()).attr('x1', self.axis.xScale.apply(d[0].x)).attr('y1', self.axis.yScale.apply(d[0].y)).attr('x2', self.axis.xScale.apply(d[1].x)).attr('y2', self.axis.yScale.apply(d[1].y)).attr('d', d).attr('stroke', self.strokeColor());
      }).apply([data]);
    }
  };
...
PieChart = (function(superClass) {
  ...
  defaultSegmentTextFormatter = function(segment, segments) {
    if (segment.size / dx.sum(segments.map(function(s) {
      return s.size;
    })) > 0.05) {
      return segment.size;
    } else {
  ...
  function PieChart(selector, options) {
    ...
    dx.select(this.selector).classed('dx-pie-chart', true).on('resize', 'dx.plot', (function(_this) {
      return function() {
        return _this.render();
      };
    })(this));
  ...
  calculateTotal = function(segments) {
    ...
    allZero = false;
    preTotal = dx.sum(segments.map(function(x) {
      return x.size;
    }));
    total = preTotal === 0 ? (allZero = true, segments.length) : dx.sum(segments.map(function(x) {
  ...
  PieChart.prototype.render = function() {
      ...
      case !(totalAngle === P && startAngle % (P / 2) === 0):
        switch (false) {
          case !(startAngle === 0 || startAngle % (2 * P) === 0 || startAngle % P === 0):
            r = dx.clamp(0, height / 2, r * 2);
            break;
          case !(startAngle % (P * (3 / 2)) === 0 || startAngle % (P / 2) === 0):
            r = dx.clamp(0, width / 2, r * 2);
        }
        switch (false) {
    ...
    updateRing = function(d, e, i) {
      ...
      runningTotal = 0;
      return this.view('.dx-pie-segment', 'path').update(function(s) {
    ...
    updateText = function(d, e, i) {
      ...
      runningTotal = 0;
      ref = calculateTotal(segments), total = ref.total, allZero = ref.allZero;
      return this.view('.dx-pie-segment-text', 'text').update(function(s) {
        var ref1, size, x, y;
        size = getSegmentSize(s, total, allZero);
    ...
    enterChart = function(d) {
      var labelGroup;
      self.svgTarget = this.append('svg')["class"]('dx-graph');
      self.plotTarget = self.svgTarget.append('g')["class"]('dx-plot');
      labelGroup = self.svgTarget.append('g')["class"]('dx-label');
      self.svgTarget.on('pointermove', 'dx.plot', function(p) {
        var legendContainer, x, y;
        ...
        if (self.legendEnabled()) {
          legendContainer = self.svgTarget.select('.dx-legend-container');
      ...
      self.svgTarget.on('pointerleave', 'dx.plot', function() {
        if (self.legendEnabled() && self.legendLocation() === 'hover') {
          return self.svgTarget.select('.dx-legend-container').style('display', 'none');
        }
      });
      self.svgTarget.on('click', 'dx.plot', function(p) {
    ...
    updateChart = function(d) {
      ...
      this.select('.dx-plot').view('.dx-pie-ring', 'g').update(updateRing).apply(data);
      return this.select('.dx-plot').view('.dx-pie-text', 'g').update(updateText).apply(self.segmentTextEnabled() ? data : []);
    };
    selection.view('.dx-graph', 'svg').enter(enterChart).update(updateChart).apply(this);
    ...
    if (this.legendEnabled()) {
      ...
      populateLegendSeries(legendContainer, dx.flatten(data.map(function(d) {
        return d.segments;
      })));
      switch (this.legendLocation()) {
        case 'top-left':
      ...
      return this.svgTarget.select('.dx-legend-container').remove();
    }
  };
  ...
  getClosestMeta = function(pie, x, y) {
    ...
    selection = dx.select(pie.selector);
    width = selection.width();
    height = selection.height();
    r = pie.circle.r;
    ...
    whichRing = dx.clamp(0, data.length - 1, Math.floor((r + pie.ringPadding() * ringSize / 2 - innerRadius) / ringSize));
    ring = data[whichRing];
  ...
  updateLabels = function(pie, x, y) {
    var bestMeta, updateLabel;
    updateLabel = function(data, element) {
      dx.select(element).style('left', Math.round(window.pageXOffset + pie.svgTarget.box().left + data.x) + 'px').style('top', Math.round(window.pageYOffset + pie.svgTarget.box().top + data.y) + 'px');
    ...
    return dx.select('body').select('.dx-plot-label-container').view('.dx-plot-label', 'div').update(updateLabel).apply(bestMeta ? bestMeta : []);
  };
...
dx_plot_label.standard = function(element, meta) {
  var createEntry, details, header, midX, midY;
  createEntry = function(value) {
    return dx.detached('div')["class"]('dx-plot-label-details-entry').add(dx.detached('span')["class"]('dx-plot-label-details-entry-key').text(value.name)).add(dx.detached('span')["class"]('dx-plot-label-details-entry-value').text(value.formatter(value.value)));
  };
  header = dx.detached('div')["class"]('dx-plot-label-marker').style('background', meta.color);
  details = dx.detached('div')["class"]('dx-plot-label-details').add(dx.detached('div')["class"]('dx-plot-label-details-header').text(meta.title)).add(meta.values.map(createEntry));
  midX = (meta.bounding.x1 + meta.bounding.x2) / 2;
  midY = (meta.bounding.y1 + meta.bounding.y2) / 2;
  details.classed('dx-plot-label-details-left', meta.x >= midX);
  details.classed('dx-plot-label-details-bottom', meta.y >= midY);
  return dx.select(element).clear().add(header).add(details);
};

dx_plot_label.basic = function(element, meta) {
  var details, marker, midX, midY, value;
  marker = dx.detached('div')["class"]('dx-plot-label-marker').style('background', meta.color);
  value = meta.values[1];
  midX = (meta.bounding.x1 + meta.bounding.x2) / 2;
  midY = (meta.bounding.y1 + meta.bounding.y2) / 2;
  details = dx.detached('div')["class"]('dx-plot-label-details-basic').classed('dx-plot-label-details-left', meta.x >= midX).classed('dx-plot-label-details-bottom', meta.y >= midY).text(value.formatter(value.value));
  return dx.select(element).clear().add(marker).add(details);
};
  ...
  function Sparkline(selector, options) {
    ...
    opts = dx.merge.defined({
      ...
      labelRenderer: function(element, obj) {
        return dx.select(element).text(obj.y + ' (' + obj.x + ')');
      },
      redrawOnResize: true
    }, options);
    innerLabelRenderer = function(element, meta) {
      ...
      midX = (meta.bounding.x1 + meta.bounding.x2) / 2;
      midY = (meta.bounding.y1 + meta.bounding.y2) / 2;
      labelNode = dx.detached('div').node();
      opts.labelRenderer(labelNode, {
        x: xValue.value,
        y: yValue.value
      });
      details = dx.detached('div')["class"]('dx-plot-label-details-basic').classed('dx-plot-label-details-left', meta.x >= midX).classed('dx-plot-label-details-bottom', meta.y >= midY).add(dx.detached('span')["class"]('dx-plot-label-sparkline-x').add(labelNode));
      return dx.select(element).clear().add(marker).add(details);
    };
    dx.components.clear(selector);
    dx.component.register(selector, this);
    graph = new dx.Graph(selector, {
      redrawOnResize: opts.redrawOnResize
    });
    if (opts.type !== 'bar' && opts.type !== 'line') {
      dx.consoleWarning('options.type can only be "line" or "bar", you supplied "' + opts.type + '"');
...
dx.plot.label = dx_plot_label;

dx.plot.arcCurve = arcCurve;

dx.plot.svgCurve = svgCurve;

dx.Axis = Axis;

dx.Graph = Graph;

dx.graph = function(options) {
  var graph, selection;
  selection = new dx.detached('div');
  graph = new Graph(selection.node(), options);
  return selection;
};

dx.LineSeries = LineSeries;

dx.BandSeries = BandSeries;

dx.ScatterSeries = ScatterSeries;

dx.BarSeries = BarSeries;

dx.StraightLineSeries = StraightLineSeries;

dx.pieChart = function(options) {
...
dx.PieChart = PieChart;

dx.sparkline = function(options) {
  var selection, sparkline;
  selection = dx.detached('div').classed('dx-sparkline', true);
  sparkline = new Sparkline(selection.node(), options);
  sparkline.render();
  return selection;
};

dx.Sparkline = Sparkline;
...
ButtonGroup = (function(superClass) {
  ...
  function ButtonGroup(selector, options) {
    ...
    dx.component.register(selector, this);
    this.options = dx.merge.defined({
      buttonClass: 'dx-complement',
      activeClass: 'dx-contrast',
      fullWidth: false,
      renderer: function(node, data, current) {
        dx.select(node).text(data.value != null ? data.value : data);
      },
      items: [],
      disabled: false
    }, options);
    this.current = void 0;
    group = dx.select(selector).classed('dx-button-group', true).append('div')["class"]('dx-input-group').classed('dx-input-group-full-width', this.options.fullWidth);
    this.view = group.view('button').enter(function() {
      return this.append('button')["class"]('dx-btn').classed('dx-section dx-no-margin', self.options.fullWidth).classed(self.options.buttonClass, true).node();
      ...
      this["class"]('dx-btn').classed('dx-section dx-no-margin', self.options.fullWidth).classed(buttonClass, true).attr('disabled', self.options.disabled ? true : void 0).on('click', 'dx.button-group', function() {
...
dx.buttonGroup = function(options) {
  var selection;
  selection = dx.detached('div');
...
setValue = function(picker, value, items, cause) {
  ...
  for (i = 0, len = items.length; i < len; i++) {
    item = items[i];
    if (item === value || (dx.isObject(item) && (item.value === value || item.value === (value != null ? value.value : void 0)))) {
      newVal = item;
      break;
    }
  }
...
Picker = (function(superClass) {
  ...
  function Picker(selector, options) {
    ...
    resolvedOptions = dx.merge.defined({
      dropdownOptions: {},
      items: [],
      noValueText: dx.userFacingText('picker', 'chooseValue'),
      renderer: void 0,
      value: void 0,
      disabled: false,
      fullWidth: false
    ...
    dx.component.register(selector, this);
    this.selection = dx.select(selector);
    this.selection.classed('dx-picker-full-width', resolvedOptions.fullWidth);
    button = this.selection.classed('dx-picker dx-btn', true).append('span')["class"]('dx-picker-inner').attr('type', 'button');
    selectedText = button.append('span')["class"]('dx-picker-text');
    button.append('span')["class"]('dx-picker-icon').append('i')["class"]('dx-icon dx-icon-caret-down');
    renderWrapper = (function(_this) {
      return function(node, item) {
        return _this._.renderer(node, item);
      };
    })(this);
    menu = new dx.Menu(selector, {
    ...
    menu.pipe(this, '', ['highlight']);
    menu.dropdown.pipe(this, 'dropdown');
    menu.on('change', 'dx.picker', (function(_this) {
      return function(item) {
...
dx.picker = function(options) {
  var selection;
  selection = dx.detached('button');
...
(function(){
var deprecatedWarning, dx_xhr, parsers, performRequest, reshapedRequest, respondToRequest, sendRequest, standardRequest;

deprecatedWarning = (function(_this) {
  return function(fn) {
    return dx.deprecatedWarning(fn, 'fetch (native browser function) - https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API');
  };
})(this);
...
performRequest = function(url, data, callback, options, index) {
  ...
  defaults = {
    requestType: 'GET',
    formatter: dx.identity,
    headers: {}
  };
  options = dx.merge(defaults, options);
  if (options.contentType) {
...
dx_xhr = function(urlType, urls, data, callback, options) {
...
standardRequest = function() {
  ...
  urlType = (function() {
    switch (false) {
      case !dx.isArray(urls):
        return 'array';
      case !dx.isObject(urls):
        return 'object';
      case !dx.isString(urls):
        return 'string';
    }
  })();
  if (!urlType || urlType === 'array' && urls.length === 0) {
    console.error('Incorrect URL passed into dx.request: ', urls);
    return;
  }
  if (dx.isFunction(arguments[1])) {
  ...
  return dx_xhr(urlType, urls, data || null, callback, options);
};

dx.request = function() {
  deprecatedWarning('dx.request');
...
parsers = {
  ...
  'text/html': function(text) {
    return dx.parseHTML.call(dx.request, text);
  },
    ...
    defaults = type ? {
      ...
      formatter: function(xhr) {
        ...
        if (parser) {
          ...
          dx.consoleWarning("Unknown parser for mime type " + mimeType + ", carrying on anyway");
          return xhr;
        }
      }
    };
...
dx.json = reshapedRequest('application/json');

dx.html = reshapedRequest('text/html');

dx.text = reshapedRequest('text/plain');

dx.reshapedRequest = reshapedRequest();
...
updateHeaderPositions = function(container, wrapperNode) {
  ...
  topNode = container.shallowSelect('.dx-sticky-table-header-top');
  if (topNode != null) {
    topNode.select('.dx-table').style('left', leftOffset + 'px');
  }
  leftNode = container.shallowSelect('.dx-sticky-table-header-left');
  if (leftNode != null) {
    return leftNode.select('.dx-table').style('top', topOffset + 'px');
  }
};
...
cloneEvents = function(elem, clone) {
  ...
  if ((elem != null) && (clone != null)) {
    elemData = dx.select.getHexagonElementDataObject(elem);
...
getChildrenFromTable = function(t, body, single) {
  ...
  return dx.flatten(realParents.map(function(parent) {
    return getChildren(dx.select(parent), 'th, td', single);
  }));
};
...
createStickyHeaderNodes = function(real, cloned) {
  ...
  for (i = j = 0, ref = real.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
    ...
    dx.select(real[i]).classed('dx-sticky-table-invisible', true);
    results.push(dx.select(cloned[i]).classed('dx-sticky-table-invisible', false));
  }
  return results;
};
...
cloneTableAndNodeEvents = function(selection, realTable, tableClone, body, single) {
  ...
  innerTableClone.selectAll('th, td').classed('dx-sticky-table-invisible', true);
...
StickyTableHeaders = (function() {
  function StickyTableHeaders(selector, options) {
    ...
    selection = dx.select(selector);
    table = selection.classed('dx-table') || selection.node().nodeName.toLowerCase() === 'table' ? (tableIsRootElement = true, selection) : (tableIsRootElement = false, selection.select('.dx-table'));
    if (table.classed('dx-table-full') && (options.fullWidth == null)) {
      options.fullWidth = true;
    }
    if (resolvedOptions.stickTableHead && table.select('thead').selectAll('tr').empty()) {
      dx.consoleWarning('dx.StickyTableHeaders - ' + selector, 'Sticky table headers initialized with stickTableHead of true without a thead element present');
    ...
    if (resolvedOptions.stickFirstColumn && table.select('tbody').select('tr').selectAll('th, td').empty()) {
      dx.consoleWarning('dx.StickyTableHeaders - ' + selector, 'Sticky table headers initialized with stickFirstColumn of true without any columns to stick');
      resolvedOptions.stickFirstColumn = false;
    }
    container = tableIsRootElement ? table.insertAfter('div')["class"]('dx-sticky-table-headers') : selection.classed('dx-sticky-table-headers', true);
    if (resolvedOptions.containerClass) {
      container.classed(resolvedOptions.containerClass, true);
    }
    wrapper = container.append('div')["class"]('dx-sticky-table-wrapper');
    wrapper.append(table);
    showScrollIndicators = dx.scrollbarSize() === 0;
    if (showScrollIndicators) {
      topIndicator = container.append('div')["class"]('dx-sticky-table-scroll-top');
      rightIndicator = container.append('div')["class"]('dx-sticky-table-scroll-right');
      bottomIndicator = container.append('div')["class"]('dx-sticky-table-scroll-bottom');
      leftIndicator = container.append('div')["class"]('dx-sticky-table-scroll-left');
    }
    wrapper.on('scroll', 'dx.sticky-table-headers', function() {
      if (showScrollIndicators) {
    ...
    if (resolvedOptions.useResponsive) {
      self = this;
      container.on('resize', 'dx.sticky-table-headers', function() {
        self.render();
  ...
  StickyTableHeaders.prototype.render = function() {
    ...
    if (options.stickFirstColumn) {
      offsetWidthElem = dx.select(table.select('tbody').select('tr').select('th, td').nodes[0]);
    ...
    tableBox = table.box();
    hasVerticalScroll = wrapperNode.scrollHeight > (wrapperNode.clientHeight + dx.scrollbarSize());
    hasHorizontalScroll = wrapperNode.scrollWidth > (wrapperNode.clientWidth + dx.scrollbarSize());
    heightScrollbarOffset = hasHorizontalScroll ? dx.scrollbarSize() : 0;
    widthScrollbarOffset = hasVerticalScroll ? dx.scrollbarSize() : 0;
    ...
    if (options.stickTableHead) {
      topHead = container.shallowSelect('.dx-sticky-table-header-top');
      if (topHead.empty()) {
        topHead = container.prepend('div')["class"]('dx-sticky-table-header-top');
    ...
    if (options.stickFirstColumn) {
      leftHead = container.shallowSelect('.dx-sticky-table-header-left');
      if (leftHead.empty()) {
        leftHead = container.prepend('div')["class"]('dx-sticky-table-header-left');
        if (!_.showScrollIndicators && options.fullWidth) {
          background = table.select('th').style('background-color');
    ...
    if (options.stickTableHead && options.stickFirstColumn) {
      topLeftHead = container.shallowSelect('.dx-sticky-table-header-top-left');
      if (topLeftHead.empty()) {
        topLeftHead = container.prepend('div')["class"]('dx-sticky-table-header-top-left');
      }
      topLeftHead.clear();
    ...
    if (_.showScrollIndicators) {
      ...
      topIndicator = container.shallowSelect('.dx-sticky-table-scroll-top').style('top', scrollOffsetHeight + 'px').style('left', scrollOffsetWidth + 'px').style('width', wrapperBox.width + 'px');
      rightIndicator = container.shallowSelect('.dx-sticky-table-scroll-right').style('top', scrollOffsetHeight + 'px').style('height', wrapperBox.height + 'px');
      bottomIndicator = container.shallowSelect('.dx-sticky-table-scroll-bottom').style('left', scrollOffsetWidth + 'px').style('width', wrapperBox.width + 'px');
      leftIndicator = container.shallowSelect('.dx-sticky-table-scroll-left').style('top', scrollOffsetHeight + 'px').style('left', scrollOffsetWidth + 'px').style('height', wrapperBox.height + 'px');
      updateScrollIndicators(wrapper, topIndicator, rightIndicator, bottomIndicator, leftIndicator);
    }
    return updateHeaderPositions(container, wrapper.node());
  };
...
dx.StickyTableHeaders = StickyTableHeaders;
...
Toggle = (function(superClass) {
  ...
  Toggle.prototype.value = function(val) {
    if (val != null) {
      ...
      this.toggle.classed('dx-toggle-box-on', val);
      return this;
    } else {
...
validateItems = function(feed, items) {
  if (!feed.validateItems(items)) {
    dx.consoleWarning("dx.AutocompletePicker: the items was expected to be an array of items or a function, you supplied: " + items);
    return false;
  } else {
    return true;
  }
...
AutocompletePicker = (function(superClass) {
  ...
  function AutocompletePicker(selector, items, options) {
    ...
    defaults = {
      ...
      renderer: void 0,
      value: void 0,
      chooseValueText: dx.userFacingText('autocompletePicker', 'chooseValue'),
      loadingText: dx.userFacingText('autocompletePicker', 'loading'),
      noResultsText: dx.userFacingText('autocompletePicker', 'noResults'),
      otherResultsText: dx.userFacingText('autocompletePicker', 'otherResults')
    };
    if (options.valueLookup) {
      defaults.renderer = function(element, item) {
        return dx.select(element).text(options.valueLookup(item));
      };
    }
    resolvedOptions = dx.merge(defaults, options);
    selection = dx.select(selector).classed('dx-autocomplete-picker dx-btn', true);
    if (resolvedOptions.buttonClass) {
      selection.classed(resolvedOptions.buttonClass, true);
    }
    valueText = selection.append('div')["class"]('dx-autocomplete-picker-text');
    selection.append('span')["class"]('dx-autocomplete-picker-icon').append('i')["class"]('dx-icon dx-icon-caret-down');
    feedOptions = {
    ...
    this._ = {
      ...
      valueText: valueText,
      feed: feed,
      valueLookup: resolvedOptions.valueLookup || dx.identity
    };
    if (validateItems(feed, items)) {
      feed.items(items);
      renderWrapper = (function(_this) {
        return function(element, item) {
          selection = dx.select(element).clear().classed('dx-autocomplete-picker-heading', item.heading);
          if (item.unselectable || item.heading) {
            return dx.select(element).text(item.text).off();
          } else {
      ...
      menu = new dx.Menu(selector, {
        dropdownOptions: {
          ddClass: 'dx-autocomplete-picker-dropdown'
        }
      });
      ...
      debouncedPopulate = dx.debounce(debounceDuration, populateMenu);
      ...
      input = dx.detached('input')["class"]('dx-autocomplete-picker-input').on('input', function(e) {
...
dx.autocompletePicker = function(items, options) {
  var selection;
  selection = dx.detached('div');
...
dx.AutocompletePicker = AutocompletePicker;
...
DateTimePicker = (function(superClass) {
  ...
  function DateTimePicker(selector, options) {
    ...
    this.selection = dx.select(this.selector).classed('dx-date-time-picker', true);
    ...
    this.datePicker = new dx.DatePicker(dtNode, this.options.datePickerOptions);
    this.timePicker = new dx.TimePicker(tpNode, this.options.timePickerOptions);
    ...
    updateDatePicker = (function(_this) {
      return function(data) {
        ...
        _this.datePicker.date(dx.preferences.applyTimezoneOffset(_this.date()));
    ...
    this.datePicker.on('change', 'dx.date-time-picker', updateTimePicker);
    this.timePicker.on('change', 'dx.date-time-picker', updateDatePicker);
  }
...
dx.dateTimePicker = function(options) {
  var selection;
  selection = dx.detached('div');
...
dx.DateTimePicker = DateTimePicker;
...
TagInput = (function(superClass) {
  ...
  function TagInput(selector, options) {
    ...
    this.options = dx.merge.defined({
      ...
      draggable: true,
      items: [],
      placeholder: dx.userFacingText('tagInput', 'placeholder'),
    ...
    dx.component.register(this.selector, this);
    this.selection = dx.select(this.selector).classed('dx-tag-input', true);
    this.tagContainer = this.selection.append('span')["class"]('dx-tags-container');
    if (this.options.draggable) {
      _.dragContainer = new dx.DragContainer(this.tagContainer.node());
    }
    isInsideForm = !this.selection.closest('form').empty();
    inputContainer = this.selection.append(isInsideForm ? 'div' : 'form')["class"]('dx-tag-input-container');
    validationForm = isInsideForm ? this.selection.closest('.dx-form') : inputContainer;
    ...
    if (this.options.autocompleteData != null) {
      ...
      this._.autocomplete = new dx.AutoComplete(this.input.node(), acData, this.options.autocompleteOptions);
      inputMap = ((ref = this._.autocomplete.options) != null ? ref.inputMap : void 0) || dx.identity;
      this._.autocomplete.on('change', 'dx.taginput', (function(_this) {
        return function(value) {
    ...
    validateForm = (function(_this) {
      return function(clear) {
        if (isInsideForm) {
          if (clear) {
            return validationForm.selectAll('.dx-form-error').remove();
          } else {
            return dx.validateForm(validationForm.node()).valid;
          }
        } else {
    ...
    this.input.on('keypress', 'dx.tag-input', (function(_this) {
      return function(event) {
        var name;
        if (event.keyCode === 13) {
          validateForm();
    ...
    this.input.on('input', 'dx.tag-input', hasError);
    this.input.on('keydown', 'dx.tag-input', (function(_this) {
      return function(event) {
        ...
        if (((event.keyCode || event.charCode) === 8) && !backspacedown) {
          ...
          if (_this.input.value() === '') {
            selection = _this.tagContainer.selectAll('.dx-tag');
            if (selection.size() > 0) {
              if ((ref1 = _this._.autocomplete) != null) {
                ref1.hide();
              }
              nodeSelection = dx.select(selection.node(selection.size() - 1));
    ...
    this.input.on('keyup', 'dx.tag-input', function(event) {
    ...
    this.input.on('focus', 'dx.tag-input', (function(_this) {
      return function(event) {
  ...
  addTag = function(tagInput, name, clasz) {
    ...
    tagSelection = tagInput.tagContainer.append('div')["class"]('dx-tag').add(dx.detached('span')["class"]('dx-tag-text').text(name)).add(dx.detached('span')["class"]('dx-tag-remove').add(dx.detached('i')["class"]('dx-icon dx-icon-close')));
    if (tagInput.options.draggable) {
      tagSelection.classed('dx-drag-element', true).select('.dx-tag-text').classed('dx-drag-control', true);
    }
    if (clasz) {
      tagSelection.classed(clasz, true);
    }
    tagSelection.classed('dx-disabled', tagInput._.disabled);
    ...
    tagSelection.select('.dx-tag-remove').on('click', 'dx.tag-input', (function(_this) {
      return function() {
        var value;
        if (!tagInput._.disabled && !tagSelection.classed('dx-disabled')) {
  ...
  TagInput.prototype.add = function(name, cssclass) {
    ...
    if (dx.isArray(name)) {
      ...
      dx.consoleWarning('TagInput.add was passed the wrong argument type', 'TagInput.add accepts an array or string argument, you supplied:', name);
    }
    this.input.value('');
    if (this.options.draggable) {
  ...
  TagInput.prototype.items = function(items, cssclass) {
    if (arguments.length > 0) {
      ...
      if (dx.isArray(items)) {
        ...
        dx.consoleWarning('TagInput.items was passed the wrong argument type', 'TagInput.items only accepts an array argument, you supplied:', items);
      }
      return this;
    } else {
      return this.tagContainer.selectAll('.dx-tag').select('.dx-tag-text').text();
    }
  };
  ...
  TagInput.prototype.disabled = function(disable) {
    if (disable != null) {
      ...
      this.tagContainer.selectAll('.dx-tag').classed('dx-disabled', disable).classed('dx-drag-element', this.options.draggable && !disable);
      if (this.options.draggable) {
        this._.dragContainer.setup();
      }
      return this;
...
MorphSection = (function(superClass) {
  ...
  function MorphSection(selector1, options) {
    ...
    this.expanded = false;
    selection = dx.select(this.selector).classed('dx-openable', true).classed('dx-morph-section', true);
    selection.select('.dx-morph-toggle').on('click', 'dx.morph-section', (function(_this) {
      return function() {
        return _this.toggle();
      };
    })(this));
    selection.select('.dx-morph-content').style('display', 'none');
  }
  ...
  MorphSection.prototype.visible = function(show) {
    ...
    if (show === void 0) {
      ...
      selection = dx.select(this.selector).classed('dx-opened', show);
      toggle = selection.select('.dx-morph-toggle');
      content = selection.select('.dx-morph-content');
...
InlineMorphSection = (function(superClass) {
  ...
  function InlineMorphSection(selector, enterEditMode, exitEditMode, options) {
    ...
    morphSection = this;
    this.on('show', 'dx.morph-section', function(data) {
      morphSection.detector = new dx.ClickDetector(data.content);
      enterEditMode.call(morphSection, data.toggle, data.content);
      dx.select(data.toggle).style('display', 'none');
      morphSection.detector.addException(data.content);
      return morphSection.detector.on('click', 'dx.morph-section', function() {
        morphSection.detector.cleanUp();
        morphSection.detector = void 0;
        return morphSection.hide();
      });
    });
    this.on('hide', 'dx.morph-section', function(data) {
      if (exitEditMode != null) {
        exitEditMode.call(morphSection, data.toggle, data.content);
      }
      return dx.select(data.toggle).style('display', '');
    });
  }
...
dx.MorphSection = MorphSection;

dx.InlineMorphSection = InlineMorphSection;
...
setVisibility = function(show, animate) {
  ...
  animateTitlebar = function(elem) {
    if (show) {
      elem.classed('dx-titlebar-mobile-hide', false);
      if (animate) {
      ...
      if (animate) {
        return elem.morph()["with"]('collapsev', 50).then(function() {
          return elem.classed('dx-titlebar-mobile-hide', true);
...
TitleBar = (function() {
  function TitleBar(selector) {
    ...
    dx.component.register(this.selector, this);
    this._ = {};
    this.isMobileFriendly = dx.select(this.selector).select('.dx-titlebar-menu-icon-mobile').size() > 0;
    hasLinkBar = dx.select(this.selector).select('.dx-titlebar-linkbar').selectAll('.dx-titlebar-link').size() > 0;
    isFixed = dx.select('body').classed('dx-heading-fixed');
    isFullScreen = dx.select('body').classed('dx-full-screen');
    if (this.isMobileFriendly) {
      dx.select(this.selector).select('.dx-titlebar-menu-icon-mobile').on('click', 'dx.titlebar', (function(_this) {
        return function() {
    ...
    if (hasLinkBar && (isFixed || isFullScreen)) {
      dx.select('body').classed('dx-titlebar-link-padding', true);
    }
  }
    ...
    if (arguments.length > 0) {
      if (cls != null) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          ...
          dx.select(this.selector).select('.dx-titlebar').classed(d, cls === d);
        }
      }
      return this;
    } else {
...
dx.TitleBar = TitleBar;

dx.titleBar = function(options) {
  var icon, iconClass, iconLink, ref, ref1, ref2, ref3, ref4, selection, showIcon, subtitle, title;
  if (options == null) {
    options = {};
  }
  title = (ref = options.title) != null ? ref : 'Title', subtitle = (ref1 = options.subtitle) != null ? ref1 : '', showIcon = (ref2 = options.showIcon) != null ? ref2 : true, iconLink = (ref3 = options.iconLink) != null ? ref3 : '#', iconClass = (ref4 = options.iconClass) != null ? ref4 : 'dx-logo';
  icon = showIcon ? dx.detached('a')["class"]('dx-titlebar-icon').attr('href', iconLink).add(dx.detached('img')["class"](iconClass)) : void 0;
  selection = dx.div('dx-heading').add(dx.div('dx-titlebar').add(dx.div('dx-titlebar-container').add(dx.div('dx-titlebar-header').add(icon).add(title ? dx.div('dx-titlebar-title').text(title) : void 0).add(subtitle ? dx.div('dx-titlebar-subtitle').text(subtitle) : void 0))));
  new dx.TitleBar(selection);
  return selection;
};

if (dx.select('.dx-heading').size() > 0) {
  dx.titlebar = new dx.TitleBar('.dx-heading');
}
...
slide = function(e) {
  ...
  dx.select(document).on('pointermove', 'dx.slider', _.pointerMoveHandler);
  dx.select(document).on('pointerup', 'dx.slider', _.pointerUpHandler);
  _.slidableWidth = this.options.type === 'range' ? _.container.width() - _.controlWidth : _.container.width();
  e.event.preventDefault();
  pos = dx.clamp(0, _.container.width(), e.x - _.container.box().left) / _.slidableWidth;
  val = {};
  if (this.options.type === 'range') {
    ...
    if (_.dragging === 'end' || (isEnd && _.dragging !== 'start')) {
      _.dragging = 'end';
      pos -= offset;
      pos = dx.clamp(start, 1, pos);
      val.end = posToValue.call(this, pos);
    } else {
      _.dragging = 'start';
      pos = dx.clamp(0, end, pos);
...
slideEnd = function(e) {
  ...
  _ = this._;
  if (_.mouseDown) {
    dx.select(document).off('pointermove', 'dx.slider', _.pointerMoveHandler);
    dx.select(document).off('pointerup', 'dx.slider', _.pointerUpHandler);
    _.mouseDown = false;
    _.dragging = void 0;
    node = _.range.select('.dx-slider-active');
    node.morph()["with"]('delay', 100).then('fadeout', 200).then(function() {
      node.classed('dx-slider-active', false);
...
renderValues = function() {
  ...
  _ = this._;
  points = _.container.selectAll('.dx-slider-point');
  if (!points.empty()) {
  ...
  if ((node != null) && (val != null)) {
    if (!_.valueVis) {
      _.valueVis = true;
      node.classed('dx-slider-active', true).classed('dx-slider-under', false);
      this.options.renderer(this, node.node(), val);
      box = node.box();
      node.classed('dx-slider-under', box.top - box.height < 0);
...
Slider = (function(superClass) {
  ...
  function Slider(selector, options) {
    ...
    this.options = dx.merge({
      ...
      renderer: function(slider, elem, value) {
        return dx.select(elem).text(dx.fixed(value));
      },
      min: 0,
      max: 1,
      step: void 0,
    ...
    self = this;
    _.container = dx.select(this.selector).classed('dx-slider', true).append('div').classed('dx-slider-inner', true).classed('dx-slider-double', this.options.type === 'range').classed('dx-slider-discrete', _.isDiscrete).on('pointerdown', 'dx.slider', (function(_this) {
      return function(e) {
        if (!_.disabled) {
    ...
    _.range = _.container.append('div')["class"]('dx-slider-range');
    if (_.controlWidth == null) {
      _.controlWidth = Number(window.getComputedStyle(_.range.node(), ':after').width.replace('px', ''));
    }
    _.valueL = _.range.append('div')["class"]('dx-slider-value').style('left', 0);
    _.valueR = _.range.append('div')["class"]('dx-slider-value').style('left', '100%');
    _.slidableWidth = this.options.type === 'range' ? _.container.width() - _.controlWidth : _.container.width();
  ...
  Slider.prototype.discreteValues = function(array, render) {
    ...
    if (array != null) {
      ...
      _ = this._;
      _.isDiscrete = ((ref = this.options.discreteValues) != null ? ref.length : void 0) > 0 || (this.options.step != null);
      _.discreteValues = new dx._.Map();
      _.discreteSpacing = 1 / (array.length - 1);
      _.container.selectAll('.dx-slider-point').remove();
      for (i = j = 0, ref1 = array.length; 0 <= ref1 ? j < ref1 : j > ref1; i = 0 <= ref1 ? ++j : --j) {
        ...
        if (this.options.type === 'range' || i > 0) {
          point = _.range.insertBefore('div')["class"]('dx-slider-point').style('left', pos * 100 + '%');
        }
      }
      if (render) {
      ...
      clamp = (function(_this) {
        return function(val) {
          if (_this._.isDiscrete) {
            ...
            return dx.clamp(_this.options.min, _this.options.max, val);
          }
        };
      })(this);
      if (this.options.type === 'range') {
  ...
  Slider.prototype.disabled = function(disable) {
    if (disable != null) {
      this._.disabled = disable;
      return dx.select(this.selector).classed('dx-disabled', disable);
    } else {
      return !!this._.disabled;
    }
  };
...
div = function(clasz) {
  return dx.detached('div')["class"](clasz);
};

span = function(clasz) {
  return dx.detached('span')["class"](clasz);
};

classContext = function(selection, context) {
  if (context) {
    return selection.classed('dx-' + context, true);
  } else {
    return selection;
  }
};
...
textContext = function(selection, context) {
  if (context) {
    return selection.classed('dx-text-' + context, true);
  } else {
    return selection;
  }
};
...
backgroundContext = function(selection, context) {
  if (context) {
    return selection.classed('dx-background-' + context, true);
  } else {
    return selection;
  }
};
...
dx.card.small = function() {
  return div('dx-card-small dx-card');
};
    ...
    return joints.forEach(function(joint) {
      return directions.forEach(function(direction) {
        return headers.forEach(function(header) {
          var base, types;
          types = [size, fixed, joint, direction, header].filter(dx.defined);
          base = dx.card;
...
defineComponent = function(identifier, setup) {
  ...
  return parent[name] = function(options) {
    ...
    dx.component.register(selection.node(), {
...
dx.card.aligned = function() {
  return dx.detached('div')["class"]('dx-card-aligned');
};
...
textLikeComponent = function(elementType, clasz) {
  return function(options) {
    ...
    selection = textContext(dx.detached(elementType)["class"](clasz).text(options.text), options != null ? options.context : void 0);
...
dx.card.text = textLikeComponent('span', 'dx-card-text');

dx.card.small.text = function(options) {
  return dx.card.text(options).classed('dx-card-small', true);
};

dx.card.large.text = function(options) {
  return dx.card.text(options).classed('dx-card-large', true);
};

dx.card.title = textLikeComponent('span', 'dx-card-title dx-header');

dx.card.small.title = function(options) {
  return dx.card.title(options).classed('dx-card-small', true);
};

dx.card.large.title = function(options) {
  return dx.card.title(options).classed('dx-card-large', true);
};

dx.card.icon = function(options) {
  var selection;
  selection = textContext(dx.detached('i')["class"]('dx-card-icon ' + (options.icon || '')), options != null ? options.context : void 0);
  dx.component.register(selection.node(), {
    icon: function(icon) {
      selection["class"]('dx-card-icon ' + (icon || ''));
      return this;
    },
    context: function(context) {
      textContext(selection, context);
...
dx.card.small.icon = function(options) {
  return dx.card.icon(options).classed('dx-card-small', true);
};

dx.card.large.icon = function(options) {
  return dx.card.icon(options).classed('dx-card-large', true);
};
...
dx.card.action.section = function(options) {
  return dx.card.section(options).add(dx.detached('a').attr('href', options.link)["class"]('dx-card-action').add(dx.card.text(options)));
};

dx.card.action.icon.section = function(options) {
  return dx.card.section(options).add(dx.detached('a').attr('href', options.link)["class"]('dx-card-action').add(dx.card.text(options)).add(dx.card.icon(options)));
};

dx.card.icon.action.section = function(options) {
  return dx.card.section(options).add(dx.detached('a').attr('href', options.link)["class"]('dx-card-action').add(dx.card.icon(options)).add(dx.card.text(options)));
};

dx.card.notice = function(options) {
  if (options == null) {
    options = {};
  }
  return dx.card().classed('dx-card-small', true).add(dx.card.header.section({
    sectionContext: options.context || 'info'
  }).add(dx.card.title({
    titleText: options.title || 'Note'
  }))).add(dx.card.section().add(dx.card.text({
...
ColorPicker = (function(superClass) {
  ...
  function ColorPicker(selector, options) {
    ...
    dx.deprecatedWarning('dx.ColorPicker', 'N/A - Please use a third party colour picker');
    this.options = dx.merge.defined({
      startColor: '#000',
      showInputs: 0,
      align: 'lblt',
      ...
      return function(elem) {
        ...
        getMousePos = function(e) {
          ...
          if (isNaN(e.layerX) && e.offsetX) {
            ...
            elem = dx.select(touches[0].target).box();
        ...
        colorChanged = function(source) {
          ...
          if (source === 'slider' || source === 'box') {
            gridBG = dx.color().hsl([_.currentColor.hue(), 100, 50]);
        ...
        hexBoxChanged = function(e) {
          if (dx.color.isColorString(hexInput.value())) {
            _.currentColor = dx.color(hexInput.value());
        ...
        dragObject = function(elem, parent, min, max, startCallback, moveCallback, endCallback) {
          ...
          dragStart = function(e) {
            ...
            dx.select(document).on('pointermove', 'dx.color-picker', dragGo);
            return dx.select(document).on('pointerup', 'dx.color-picker', dragStopHook);
          };
          dragGo = function(e) {
            var newPos;
            e = e.event;
          ...
          dragStop = function() {
            ...
            dx.select(document).off('pointermove', 'dx.color-picker', dragGo);
            dx.select(document).off('pointerup', 'dx.color-picker', dragStopHook);
          ...
          StartListening = function() {
            ...
            listening = true;
            return parent.on('pointerdown', 'dx.color-picker', dragStart);
          };
          StopListening = function() {
            if (!listening) {
              return;
            }
            parent.off('pointerdown', 'dx.color-picker', dragStart);
        ...
        picker = dx.select(elem).append('div')["class"]('dx-colorpicker');
        pickerInner = picker.append('div')["class"]('dx-colorpicker-inner');
        pickerFields = pickerInner.append('div')["class"]('dx-colorpicker-fields');
        grid = pickerFields.append('div')["class"]('dx-colorpicker-grid');
        circlePicker = grid.append('div')["class"]('dx-colorpicker-picker');
        slider = pickerFields.append('div')["class"]('dx-colorpicker-slider');
        sliderPicker = slider.append('div')["class"]('dx-colorpicker-picker');
        if (_this.options.showInputs) {
          inputs = pickerInner.append('div')["class"]('dx-colorpicker-inputs');
          colorGroup = inputs.append('div')["class"]('dx-colorpicker-input-group');
          quickColorBox = colorGroup.append('div')["class"]('dx-colorpicker-quick-color');
          staticColorBox = colorGroup.append('div')["class"]('dx-colorpicker-static-color');
          makeInput = function(parent, name, text) {
            return parent.append('div')["class"]('dx-colorpicker-input').append('label').attr('for', name).text(text).insertAfter('input').attr('name', name).attr('size', 7).attr('maxlength', 3);
          };
          hexInput = makeInput(inputs, 'hex', 'Hex:');
          rgbGroup = inputs.append('div')["class"]('dx-colorpicker-input-group');
          redInput = makeInput(rgbGroup, 'red', 'R:');
          greenInput = makeInput(rgbGroup, 'green', 'G:');
          blueInput = makeInput(rgbGroup, 'blue', 'B:');
          hslGroup = inputs.append('div')["class"]('dx-colorpicker-input-group');
        ...
        if (_this.options.showInputs) {
          hexInput.on('change', 'dx.color-picker', hexBoxChanged);
          redInput.on('change', 'dx.color-picker', redBoxChanged);
          greenInput.on('change', 'dx.color-picker', greenBoxChanged);
          blueInput.on('change', 'dx.color-picker', blueBoxChanged);
          hueInput.on('change', 'dx.color-picker', hueBoxChanged);
          saturationInput.on('change', 'dx.color-picker', saturationBoxChanged);
          lightnessInput.on('change', 'dx.color-picker', lightnessBoxChanged);
          quickColorBox.style('background', _.currentColor.toString());
          staticColorBox.style('background', _.currentColor.toString());
        }
    ...
    this.dropdown = new dx.Dropdown(selector, buildDropdown, {
  ...
  ColorPicker.prototype.value = function(color) {
    ...
    _ = this._;
    if (color != null) {
      _.currentColor = typeof color === 'string' ? dx.color(color) : color;
  ...
  ColorPicker.prototype.disabled = function(disabled) {
    if (arguments.length > 0) {
      this.options.disabled = disabled;
      dx.select(this._.selector).attr('disabled', disabled ? true : void 0).classed('dx-disabled', disabled);
      if (this.dropdown.isOpen() && disabled === true) {
        this.dropdown.hide();
      }
      return this;
...
dx.colorPicker = function(options) {
  var selection;
  selection = dx.detached('div');
  ...
  ColorScale.prototype.apply = function(v) {
    ...
    switch (false) {
      ...
      default:
        ...
        percentage = dx.clamp(0, 1, mDiff / pointDiff);
        col = dx.color(this.rangeArr[point - 1].color).mix(dx.color(this.rangeArr[point].color), percentage);
    }
    return col.toString();
  };
...
Crumbtrail = (function() {
  function Crumbtrail(selector, options) {
    ...
    dx.component.register(this.selector, this);
    self = this;
    section = dx.select(this.selector);
    section.classed('dx-crumbtrail', true);
    this.options = dx.merge.defined({
      renderer: function(node, data) {
        return dx.select(node).text(data);
      },
      items: [],
      separator: '/'
    }, options);
    update = function(d, element, i) {
      if (i % 2 === 0) {
        this["class"]('dx-crumbtrail-node');
        return self.options.renderer(element, d);
      } else {
        return this["class"]('dx-crumbtrail-separator').text(self.options.separator).node();
      }
    };
  ...
  Crumbtrail.prototype.items = function(data) {
    if (data != null) {
      ...
      this.view.apply(dx.flatten(data.map(function(d) {
        return [d, 0];
      })).slice(0, -1));
      return this;
    } else {
...
dx.crumbtrail = function(options) {
  var selection;
  selection = dx.detached('div');
...
toCriteriaItems = function(list) {
  return dx.unique(list).map(function(item) {
    return {
      value: item,
      text: dx.userFacingText('dataTable', item)
    };
  });
};
...
advancedSearchCriteriaValidate = function(value) {
  ...
  allowedTypes = dx.filter.types();
  if ((dx.isArray(value) && value.every(function(c) {
    return ~allowedTypes.indexOf(c);
  })) || value === void 0) {
    return value || [];
  } else if (dx.isArray(value)) {
    invalidTypes = value.filter(function(c) {
      return !~allowedTypes.indexOf(c);
    });
    dx.consoleWarning('Invalid Filter Criteria Specified:', invalidTypes, '\nPlease select a value from dx.filter.stringTypes()', allowedTypes);
    return [];
  } else {
    dx.consoleWarning('Expected an array of filter criteria but was passed:', value);
    return [];
  }
};
...
createPaginationBlock = function(table) {
  ...
  container = dx.detached('div')["class"]('dx-data-table-paginator');
  pickerNode = container.append('button')["class"]('dx-data-table-paginator-picker dx-btn dx-btn-invisible').node();
  picker = new dx.Picker(pickerNode, {
    dropdownOptions: {
      align: 'rbrt'
    }
  }).on('change', 'dx.data-table', (function(_this) {
    return function(d) {
      if (d.cause === 'user') {
  ...
  totalRows = container.append('span')["class"]('dx-data-table-paginator-total-rows');
  back = container.append('button')["class"]('dx-data-table-paginator-back dx-btn dx-btn-invisible');
  back.append('i')["class"]('dx-icon dx-icon-chevron-left');
  back.on('click', 'dx.data-table', (function(_this) {
    return function() {
      if (!back.classed('dx-data-table-btn-disabled')) {
        return table.page(table.page() - 1);
      }
    };
  })(this));
  forward = container.append('button')["class"]('dx-data-table-paginator-forward dx-btn dx-btn-invisible');
  forward.append('i')["class"]('dx-icon dx-icon-chevron-right');
  forward.on('click', 'dx.data-table', (function(_this) {
    return function() {
      if (!forward.classed('dx-data-table-btn-disabled')) {
        return table.page(table.page() + 1);
      }
    };
  })(this));
...
createPageSizeBlock = function(table, options) {
  ...
  container = dx.detached('div')["class"]('dx-data-table-page-size');
  container.append('span').text(options.rowsPerPageText + ': ');
  node = container.append('button')["class"]('dx-data-table-page-size-picker dx-btn dx-btn-invisible').node();
  picker = new dx.Picker(node, {
    dropdownOptions: {
      align: 'rbrt'
    }
  }).on('change', 'dx.data-table', function(d) {
    if (d.cause === 'user') {
...
spacer = function() {
  return dx.detached('div')["class"]('dx-data-table-spacer');
};
...
createAdvancedSearchView = function(selection, dataTable, options) {
  ...
  advancedSearchRowEnter = function(filterGroup, filterGroupIndex) {
    return function(filterRow, index, trueIndex) {
      ...
      typePickerOptions = {
        items: [
          {
            text: dx.userFacingText('dataTable', 'and'),
            value: 'and'
          }, {
            text: dx.userFacingText('dataTable', 'or'),
            value: 'or'
          }
        ],
        fullWidth: true
      ...
      columnRenderer = function(element, cell) {
        if (cell.anyColumn) {
          return dx.select(element).text(cell.text);
        } else {
      ...
      criteriaAnyPlaceholder = dx.div('dx-data-table-advanced-search-criteria-placeholder dx-text-disabled dx-background-disabled').text(dx.userFacingText('dataTable', 'contains'));
      debouncedInput = dx.debounce(200, function(e) {
        var filter, leftFilterGroups, leftFilters, newFilter, prevFilters, ref, ref1, rightFilterGroups, rightFilters;
        prevFilters = dataTable.advancedSearch();
        ref = splitArray(prevFilters, filterGroupIndex), leftFilterGroups = ref[0], filterGroup = ref[1], rightFilterGroups = ref[2];
        ref1 = splitArray(filterGroup, trueIndex), leftFilters = ref1[0], filter = ref1[1], rightFilters = ref1[2];
      ...
      termInput = dx.detached('input').attr('placeholder', options.advancedSearchPlaceholder)["class"]('dx-data-table-advanced-search-input dx-section').attr('required', 'required').on('input', debouncedInput);
      removeBtn = dx.button({
        context: 'negative'
      }).classed('dx-data-table-advanced-search-remove dx-btn-invert', true).add(dx.icon({
      ...
      return this.append('div')["class"]('dx-data-table-advanced-search-filter dx-section dx-input-group dx-input-group-full-width').add(typePickerSel).add(columnPickerSel).add(criteriaAnyPlaceholder).add(criteriaPickerSel).add(dx.div('dx-data-table-advanced-search-filter-input-container dx-input-group dx-no-pad dx-no-border').add(termInput).add(removeBtn)).node();
    };
  };
  advancedSearchRowUpdate = function(arg, element, index) {
    ...
    filterRowSel = dx.select(element);
    validContext = !term ? 'negative' : void 0;
    filterRowSel.select('.dx-data-table-advanced-search-type').component().value(index === 0 ? 'or' : 'and');
    trueColumn = column || 'any';
    filterRowSel.select('.dx-data-table-advanced-search-column').component().value(trueColumn);
    ...
    filterRowSel.select('.dx-data-table-advanced-search-criteria').style('display', criteriaItems.length === 1 ? 'none' : 'block').component().items(toCriteriaItems(criteriaItems)).value(criteria || 'contains');
    filterRowSel.select('.dx-data-table-advanced-search-criteria-placeholder').style('display', criteriaItems.length === 1 ? 'block' : 'none');
    return filterRowSel.select('.dx-data-table-advanced-search-input').value(term || '');
  };
  advancedSearchGroupEnter = function(filterGroup, index, trueIndex) {
    var filterGroupSel, filterGroupView;
    filterGroupSel = dx.detached('div')["class"]('dx-data-table-advanced-search-filter-group');
    filterGroupView = filterGroupSel.view('.dx-data-table-advanced-search-filter').enter(advancedSearchRowEnter(filterGroup, trueIndex)).update(advancedSearchRowUpdate);
    dx.component.register(filterGroupSel.node(), {
  ...
  advancedSearchGroupUpdate = function(filterGroup, element, index) {
    return dx.component(element).filterGroupView.apply(filterGroup);
  };
  return selection.view('.dx-data-table-advanced-search-filter-group').enter(advancedSearchGroupEnter).update(advancedSearchGroupUpdate);
};
...
DataTable = (function(superClass) {
  ...
  function DataTable(selector, options) {
    ...
    resolvedOptions = dx.merge({
      ...
      cellRenderer: function(element, cell, row) {
        return dx.select(element).text(cell);
      },
      headerCellRenderer: function(element, cell, headers) {
        return dx.select(element).text(cell.name);
      },
      columns: {},
      clearSelectionText: dx.userFacingText('dataTable', 'clearSelection'),
      loadingText: dx.userFacingText('dataTable', 'loading'),
      noDataMessage: dx.userFacingText('dataTable', 'noData'),
      ...
      anyColumnText: dx.userFacingText('dataTable', 'anyColumn'),
      advancedSearchText: dx.userFacingText('dataTable', 'advancedSearch'),
      advancedSearchPlaceholder: dx.userFacingText('dataTable', 'search')
    }, options);
    ...
    selection = dx.select(selector).classed('dx-data-table', true);
    content = dx.detached('div')["class"]('dx-data-table-content');
    loadingDiv = dx.detached('div')["class"]('dx-data-table-loading').add(dx.detached('div')["class"]('dx-data-table-loading-inner').add(dx.detached('div')["class"]('dx-spinner')).add(dx.detached('span').text(' ' + resolvedOptions.loadingText)));
    statusBar = dx.detached('div')["class"]('dx-data-table-status-bar');
    statusBarText = dx.detached('span')["class"]('dx-data-table-status-bar-text');
    statusBarClear = dx.detached('span')["class"]('dx-data-table-status-bar-clear').text(" (" + resolvedOptions.clearSelectionText + ")").on('click', 'dx.data-table', (function(_this) {
      return function() {
        _this._.selectedRows.clear();
        selection.select('.dx-data-table-content').selectAll('.dx-data-table-row-selected').classed('dx-data-table-row-selected', false);
        _this.updateSelected();
        return _this.emit('selectedrowsclear');
      };
    })(this));
    controlPanelCompact = dx.detached('div')["class"]('dx-data-table-control-panel-compact');
    controlPanelCompactToggle = dx.button().classed('dx-data-table-control-panel-compact-toggle dx-btn-invisible', true).add(dx.icon({
      "class": 'dx-icon dx-icon-bars'
    })).on('click', function() {
      var toggleElem;
      toggleElem = controlPanel;
      if (toggleElem.classed('dx-data-table-compact-hide')) {
        return toggleElem.classed('dx-data-table-compact-hide', false).style('height', '0px').morph()["with"]('expandv', 150).then(function() {
          return controlPanelCompact.classed('dx-data-table-control-panel-compact-open', true);
        }).go();
      } else {
        return toggleElem.morph()["with"]('collapsev', 50).then(function() {
          toggleElem.classed('dx-data-table-compact-hide', true);
          return controlPanelCompact.classed('dx-data-table-control-panel-compact-open', false);
        }).thenStyle('display', '').go();
      }
    });
    controlPanel = dx.detached('div')["class"]('dx-data-table-control-panel dx-data-table-compact-hide');
    controlPanelInner = dx.detached('div')["class"]('dx-data-table-control-panel-inner');
    compactSort = dx.detached('div')["class"]('dx-data-table-sort').classed('dx-data-table-sort-visible', resolvedOptions.sortEnabled).add(dx.detached('span').text(resolvedOptions.sortByText + ': '));
    sortColPicker = new dx.Picker(compactSort.append('button')["class"]('dx-btn dx-btn-invisible').node());
    sortColPicker.on('change', 'dx.data-table', (function(_this) {
      return function(d) {
        if (d.cause === 'user') {
          return _this.sort({
    ...
    filterContainer = dx.detached('div')["class"]('dx-data-table-filter-container');
    onInput = dx.debounce(200, (function(_this) {
      return function() {
        return _this.filter(filterInput.value(), void 0, 'user');
      };
    })(this));
    filterInput = dx.detached('input')["class"]('dx-data-table-filter').attr('placeholder', resolvedOptions.searchPlaceholder).classed('dx-data-table-filter-visible', resolvedOptions.filterEnabled).on('input', 'dx.data-table', onInput);
    advancedSearchContainer = dx.detached('div')["class"]('dx-data-table-advanced-search-container');
    advancedSearchToggle = dx.button()["class"]('dx-data-table-advanced-search-toggle dx-btn dx-btn-invisible').text(resolvedOptions.advancedSearchText);
    advancedSearchToggleButton = new dx.Toggle(advancedSearchToggle.node());
    advancedSearchToggleButton.on('change', (function(_this) {
      return function(data) {
        return _this.advancedSearchEnabled(data);
      };
    })(this));
    advancedSearch = dx.detached('div')["class"]('dx-data-table-advanced-search');
    advancedSearchView = createAdvancedSearchView(advancedSearch, this, resolvedOptions);
    advancedSearchButtons = dx.detached('div')["class"]('dx-data-table-advanced-search-buttons');
    ...
    advancedSearchAddFilterButton = dx.button({
      context: 'positive'
    }).classed('dx-data-table-advanced-search-add-filter dx-data-table-advanced-search-button dx-btn-invert', true).add(dx.icon({
      "class": 'dx-data-table-advanced-search-icon dx-icon dx-icon-plus dx-text-positive'
    })).add(dx.detached('span').text(resolvedOptions.addFilterText)).on('click', addFilter);
    advancedSearchClearFilterButton = dx.button({
      context: 'negative'
    }).classed('dx-data-table-advanced-search-clear-filters dx-data-table-advanced-search-button dx-btn-invert', true).add(dx.icon({
      "class": 'dx-data-table-advanced-search-icon dx-icon dx-icon-close dx-text-negative'
    })).add(dx.detached('span').text(resolvedOptions.clearFiltersText)).on('click', clearFilters);
    ...
    pagination.classed('dx-data-table-compact-hide', true);
    controlPanelBottom = dx.detached('div')["class"]('dx-data-table-control-panel-bottom');
    ...
    this._ = {
      ...
      selectedRows: new dx._.Set(resolvedOptions.selectedRows),
      expandedRows: new dx._.Set(resolvedOptions.expandedRows),
    ...
    selection.on('resize', 'dx.data-table', (function(_this) {
      return function() {
        var state;
        selection.selectAll('.dx-data-table-collapsible-content-container').map(function(e) {
        ...
        selection.classed('dx-data-table-compact', state);
    ...
    randomId = dx.randomId();
    dx.select('body').on('keydown', 'dx.data-table.shift.' + randomId, (function(_this) {
      return function(e) {
        if (e.shiftKey && _this.selectEnabled()) {
          return selection.classed('dx-data-table-disable-text-selection', true);
        }
      };
    })(this));
    dx.select('body').on('keyup', 'dx.data-table.shift.' + randomId, (function(_this) {
      return function(e) {
        if (!e.shiftKey && _this.selectEnabled()) {
          return selection.classed('dx-data-table-disable-text-selection', false);
        }
      };
    })(this));
  }
  ...
  columnOption = function(name) {
    return function(columnId, value, cb) {
      var base, options;
      options = this._.options;
      if (arguments.length > 1 && dx.isString(columnId)) {
        ...
        if (dx.isString(columnId) && options.columns[columnId]) {
  ...
  DataTable.prototype.selectedRows = function(value, cb) {
    var newSelectedRows;
    if (arguments.length > 0 && !dx.isFunction(value)) {
      if (this.singleSelection() && dx.isArray(value) && value.length) {
        value = [value[0]];
      }
      this._.selectedRows = new dx._.Set(value);
  ...
  DataTable.prototype.expandedRows = function(value, cb) {
    if (arguments.length > 0 && !dx.isFunction(value)) {
      this._.expandedRows = new dx._.Set(value);
      this.render(cb);
  ...
  DataTable.prototype.render = function(cb) {
    ...
    if (feed === void 0 || (feed.headers === void 0 || feed.totalCount === void 0 || feed.rows === void 0)) {
      dx.consoleWarning('No feed specified when rendering data table');
      return;
    }
    ...
    selection.select('.dx-data-table-loading').style('display', '');
    advancedSearchVisibleAndEnabled = (!options.filterEnabled || options.showAdvancedSearch) && options.advancedSearchEnabled;
    filterSel = selection.select('.dx-data-table-filter').classed('dx-data-table-filter-visible', options.filterEnabled && !advancedSearchVisibleAndEnabled);
    ...
    selection.select('.dx-data-table-advanced-search-toggle').classed('dx-data-table-advanced-search-visible', options.filterEnabled && options.showAdvancedSearch);
    selection.select('.dx-data-table-advanced-search-container').classed('dx-data-table-advanced-search-visible', advancedSearchVisibleAndEnabled);
    selection.select('.dx-data-table-control-panel').classed('dx-data-table-filter-enabled', options.filterEnabled);
    ...
    selection.select('.dx-data-table-control-panel-compact-toggle').classed('dx-data-table-control-panel-compact-toggle-visible', showCompactControlPanelToggle);
    feed.headers((function(_this) {
      return function(headers) {
        ...
        selection.select('.dx-data-table-sort').classed('dx-data-table-sort-visible', options.sortEnabled || headers.some(function(header) {
        ...
        return feed.totalCount(function(totalCount) {
          ...
          return feed.rows(range, function(arg) {
            ...
            if (options.displayMode === 'paginate') {
              multiPage = false;
              selection.classed('dx-data-table-infinite', filteredCount === void 0);
              ...
              selection.selectAll('.dx-data-table-paginator').classed('dx-data-table-paginator-visible', multiPage);
              selection.selectAll('.dx-data-table-paginator-total-rows').text(numText || '');
              selection.selectAll('.dx-data-table-paginator-back').classed('dx-data-table-btn-disabled', _this.page() === 1);
              selection.selectAll('.dx-data-table-paginator-forward').classed('dx-data-table-btn-disabled', _this.page() === _this._.numPages);
            }
            selection.select('.dx-data-table-control-panel-compact').classed('dx-data-table-control-panel-compact-visible', multiPage || showCompactControlPanelToggle);
            selection.select('.dx-data-table-control-panel-bottom').classed('dx-data-table-control-panel-bottom-visible', multiPage || ((ref1 = options.pageSizeOptions) != null ? ref1.length : void 0));
            selection.select('.dx-data-table-control-panel').classed('dx-data-table-control-panel-visible', multiPage || showCompactControlPanelToggle);
            if (headers.some(function(header) {
              return getColumnOption('sortEnabled', header.id);
            })) {
              currentSort = _this.sort() || {};
              sortColumns = dx.flatten(headers.map(function(header) {
              ...
              _this._.sortColPicker.renderer(function(element, option) {
                if (option.value) {
                  ...
                  return dx.select(element).append('i')["class"]('dx-data-table-compact-sort-arrow dx-icon dx-icon-chevron-' + (option.direction === 'asc' ? 'up' : 'down'));
                } else {
                  return dx.select(element).text(option.text);
                }
              }).items([
                {
            ...
            selection.selectAll('.dx-data-table-page-size').classed('dx-data-table-page-size-visible', selectPageSize);
            if (selectPageSize) {
              if (options.pageSizeOptions.indexOf(options.pageSize) === -1) {
                options.pageSizeOptions.push(options.pageSize);
              }
              pageSizeOptions = options.pageSizeOptions.sort(dx.sort.compare).map(function(item) {
                return {
                  text: item,
                  value: item
                };
            ...
            if (headers.some(function(header) {
              ...
              for (row = j = ref2 = maxHeaderDepth - 1; j >= 0; row = j += -1) {
                ...
                if (options.selectEnabled || (options.collapsibleRenderer != null)) {
                  groupedRow.append('th')["class"]('dx-data-table-control');
                }
                count = 1;
                for (column = l = 1, ref3 = headerGroups.length; l <= ref3; column = l += 1) {
                  ...
                  if (column === headerGroups.length || col[row] !== prevCol[row] || parent !== prevParent) {
                    groupedRow.append('th').attr('colspan', count)["class"]('dx-data-table-cell-grouped').text(prevCol[row]);
                    count = 0;
                  }
                  count++;
                }
            ...
            if (options.selectEnabled || (options.collapsibleRenderer != null)) {
              headerControlBox = headerRow.append('th')["class"]('dx-data-table-control dx-table-head-no-border');
            }
            if (options.selectEnabled && !options.singleSelection) {
              headerCheckBox = headerControlBox.append('div')["class"]('dx-data-table-checkbox').on('click', 'dx.data-table', function() {
                var enabledRows;
                if (rows.length > 0) {
              ...
              headerCheckBox.append('i')["class"]('dx-icon dx-icon-check');
            }
            headers.forEach(function(header, i) {
              var cellDiv, cellDivContent, dirClass;
              cellDiv = headerRow.append('th')["class"]('dx-data-table-cell').classed('dx-table-header-allow-wrap', getColumnOption('allowHeaderWrap', header.id));
              cellDivContent = cellDiv.append('div')["class"]('dx-data-table-cell-inner');
              getColumnOption('headerCellRenderer', header.id)(cellDivContent.append('span')["class"]('dx-data-table-title').node(), header, headers);
              if (getColumnOption('sortEnabled', header.id)) {
                cellDiv.classed('dx-data-table-cell-sort-enabled', true);
                currentSort = _this.sort();
                dirClass = currentSort && currentSort.column === header.id ? 'dx-icon-sort-' + currentSort.direction + ' dx-data-table-sort-on' : 'dx-icon-sort';
                cellDivContent.append('i')["class"]('dx-icon ' + dirClass + ' dx-data-table-sort-icon');
                return cellDiv.on('click', 'dx.data-table', function() {
                  var direction;
                  currentSort = _this.sort() || {};
            ...
            _this.updateSelected = function() {
              ...
              getSelectableRows = function(parent) {
                return parent.selectAll('.dx-data-table-row').filter(parentFilter(parent)).classed('dx-data-table-row-selected', false);
              };
              rowDivs = getSelectableRows(tbody);
              leftHeaderBody = container.select('.dx-sticky-table-header-left').select('tbody');
              ...
              if (_this._.selectedRows.size > 0) {
                for (rowIndex = m = 0, len = rows.length; m < len; rowIndex = ++m) {
                  ...
                  if (_this._.selectedRows.has(options.rowIDLookup(row))) {
                    dx.select(rowDivs.nodes[rowIndex]).classed('dx-data-table-row-selected', true);
                    if (checkBoxDivs.nodes[rowIndex] != null) {
                      dx.select(checkBoxDivs.nodes[rowIndex]).classed('dx-data-table-row-selected', true);
                    }
                  }
                }
              }
              pageHasSelection = tbody.selectAll('.dx-data-table-row-selected').size() > 0;
            ...
            buildCollapsible = function() {
              ...
              contentRow = dx.detached('tr')["class"]('dx-data-table-collapsible-content-row');
              hiddenRow = dx.detached('tr')["class"]('dx-data-table-collapsible-row-spacer');
              contentRow.append('td')["class"]('dx-data-table-collapsible-cell dx-data-table-collapsible-cell-empty');
              contentDiv = contentRow.append('td')["class"]('dx-data-table-collapsible-cell').attr('colspan', fullWidthColSpan).append('div')["class"]('dx-data-table-collapsible-content-container').append('div')["class"]('dx-data-table-collapsible-content');
              return {
            ...
            toggleCollapsible = function(node, row, force) {
              ...
              currentVis = force != null ? force : !cc.contentRow.classed('dx-data-table-collapsible-row-visible');
              cc.contentRow.classed('dx-data-table-collapsible-row-visible', currentVis);
              node.classed('dx-data-table-collapsible-row-visible', currentVis);
              node.select('.dx-data-table-collapsible-toggle').select('i')["class"](currentVis ? 'dx-icon dx-icon-minus' : 'dx-icon dx-icon-plus');
              if (currentVis) {
              ...
              _this._.collapsibleSizeDiff = parseInt(selection.style('width')) - parseInt(dx.select(cc.contentDiv.node().parentNode).style('width'));
              return currentVis;
            };
            if (filteredCount === void 0 || filteredCount > 0) {
              rows.forEach(function(row, rowIndex) {
                ...
                tr = tbody.append('tr')["class"]('dx-data-table-row').classed('dx-data-table-row-selected', _this._.selectedRows.has(options.rowIDLookup(row))).classed('dx-data-table-row-disabled', !options.rowEnabledLookup(row));
                tr.on('click', 'dx.data-table', function(e) {
                ...
                tr.classed('dx-data-table-row-select-enabled', options.selectEnabled);
                if (options.selectEnabled || (options.collapsibleRenderer != null)) {
                  controlDiv = tr.append('th')["class"]('dx-data-table-control');
                  if (options.selectEnabled) {
                    checkbox = controlDiv.append('div')["class"]('dx-data-table-checkbox');
                    checkbox.append('i')["class"]('dx-icon dx-icon-check');
                    if (options.rowEnabledLookup(row)) {
                      checkbox.on('click', 'dx.data-table', function(e) {
                        e.stopPropagation();
                  ...
                  if (options.collapsibleRenderer != null) {
                    collapsibleControl = controlDiv.append('div')["class"]('dx-data-table-collapsible-toggle').classed('dx-data-table-collapsible-disabled', !rowIsCollapsible);
                    collapsibleControl.append('i')["class"]('dx-icon dx-icon-plus');
                  }
                  if (rowIsCollapsible) {
                    if (_this._.expandedRows.has(options.rowIDLookup(row))) {
                      toggleCollapsible(tr, row, true);
                    }
                    collapsibleControl.on('click', 'dx.data-table.collapse-row', function(e) {
                      var currentVis;
                ...
                for (columnIndex = m = 0, len = ref4.length; m < len; columnIndex = ++m) {
                  ...
                  keyDiv = dx.detached('div')["class"]('dx-data-table-cell-key');
                  getColumnOption('headerCellRenderer', headers[columnIndex].id)(keyDiv.node(), headers[columnIndex], headers);
                  cellElem = tr.append('td')["class"]('dx-data-table-cell');
                  ...
                  cellDiv = cellElem.add(keyDiv).append('div')["class"]('dx-data-table-cell-value').node();
                  results.push(getColumnOption('cellRenderer', headers[columnIndex].id)(cellDiv, cell, row));
                }
                return results;
              });
            } else {
              tbody.append('tr')["class"]('dx-data-table-row-no-data').append('td').attr('colspan', fullWidthColSpan).text(options.noDataMessage);
            }
            _this.updateSelected();
            if (_this.page() === _this._.oldPage) {
              wrapperNode = selection.select('.dx-data-table-content > .dx-sticky-table-wrapper').node();
            ...
            selection.select('.dx-data-table-content').insertAfter(container);
            selection.select('.dx-data-table-content').remove();
            selection.classed('dx-data-table-compact', ((options.compact === 'auto') && (selection.width() < collapseBreakPoint)) || (options.compact === true)).classed('dx-data-table-show-search-above-content', options.showSearchAboveTable);
            ...
            _this._.stickyHeaders = new dx.StickyTableHeaders(container.node(), stickyOpts);
            if (scrollLeft != null) {
              selection.select('.dx-data-table-content > .dx-sticky-table-wrapper').node().scrollLeft = scrollLeft;
            }
            if (scrollTop != null) {
              selection.select('.dx-data-table-content > .dx-sticky-table-wrapper').node().scrollTop = scrollTop;
            }
            selection.shallowSelect('.dx-data-table-loading').style('display', 'none');
            _this.emit('render');
            return typeof cb === "function" ? cb() : void 0;
          });
        });
...
defaultTermLookup = function(term, rowSearchTerm, criteria) {
  ...
  lookupArr = dx.isString(rowSearchTerm) ? [rowSearchTerm] : rowSearchTerm;
  arr = term.replace(stripLeadingAndTrailingWhitespaceRegex, '').split(whitespaceSplitRegex);
  validPart = dx.find(arr, function(part) {
    return dx.filter[criteria](lookupArr, part.toLowerCase()).length;
  });
  return dx.defined(validPart);
};

getAdvancedSearchFilter = function(cellValueLookup, termLookup) {
  if (cellValueLookup == null) {
    cellValueLookup = dx.identity;
  }
  if (termLookup == null) {
  ...
  return function(filters, row) {
    ...
    validFilters = dx.find(filters, function(groupedFilters) {
      var invalidFilter;
      invalidFilter = dx.find(groupedFilters, function(filter) {
        var searchTerm;
      ...
      return !dx.defined(invalidFilter);
    });
    return dx.defined(validFilters);
  };
};
...
dx.DataTable = DataTable;

dx.dataTable = function(options) {
...
dx.dataTable.objectFeed = objectFeed;

dx.dataTable.urlFeed = urlFeed;

dx.dataTable.getAdvancedSearchFilter = getAdvancedSearchFilter;
...
DrawingColor = (function() {
  ...
  DrawingColor.prototype.set = function(name, value) {
    switch (name) {
      case 'color':
        return this.c = dx.color(value);
      case 'color.red':
        return this.c.red(dx.clamp(0, 255, value));
      case 'color.green':
        return this.c.green(dx.clamp(0, 255, value));
      case 'color.blue':
        return this.c.blue(dx.clamp(0, 255, value));
      case 'color.alpha':
        return this.c.alpha(dx.clampUnit(value));
    }
  };
...
DrawingObject = (function() {
  function DrawingObject(id) {
    this.id = id != null ? id : dx.randomId();
    this.attr = new dx._.Map;
    this.properties = new dx._.Map;
  ...
  DrawingObject.prototype.set = function(name, value, duration, callback) {
    ...
    } else if (dx.startsWith(name, 'attr.')) {
  ...
  DrawingObject.prototype.drawBoundingBox = function(ctx, drawing) {
    ...
    if (bb) {
      drawing.setStrokeColor(dx.color('rgba(64, 64, 64, 1)'));
      padding = 2;
      ...
      drawing.setFillColor(dx.color('rgba(128, 128, 128, 0.1)'));
...
ColorProperty = (function() {
  ...
  ColorProperty.prototype.set = function(value, duration, callback) {
    ...
    if (dx.defined(this.stop)) {
      this.stop();
      this.stop = void 0;
    }
    if (dx.defined(duration)) {
      sr = this.value.red();
      sg = this.value.green();
      sb = this.value.blue();
      sa = this.value.alpha();
      ...
      return this.stop = dx.transition(duration, update, dx.ease.linear, cb);
    } else {
      return this.value = dx.color(value);
    }
  };
...
StringProperty = (function() {
  ...
  StringProperty.prototype.set = function(value, duration, callback) {
    ...
    if (dx.defined(duration)) {
      interpolater = dx.interpolate(this.value, value);
      cb = callback ? (function(_this) {
        return function(cancelled) {
          _this.stop = void 0;
          return callback(!cancelled, _this.value);
      ...
      return this.stop = dx.transition(duration, update, dx.ease.linear);
    } else {
      return this.value = value;
    }
  };
...
Camera = (function(superClass) {
  ...
  function Camera(drawing) {
    ...
    mouseControlState = {
      selection: dx.select(this.drawing.canvasNode),
    ...
    namespace = 'drawing-' + dx.randomId();
    dx.select(document).on('mouseup', namespace, mouseUpHandler);
    dx.select(document).on('touchend', namespace, touchUpHandler);
    dx.select(this.drawing.canvasNode).on('mousemove', 'dx.drawing', (function(_this) {
      return function(e) {
        return onMouseMove.call(_this, e, mouseControlState);
      };
    })(this));
    mouseControlState.selection.on('mousedown', 'dx.drawing', (function(_this) {
      return function(e) {
        return onMouseDown.call(_this, e, mouseControlState);
      };
    })(this));
    mouseControlState.selection.on('wheel', 'dx.drawing', (function(_this) {
      return function(e) {
        return onMouseWheel.call(_this, e, mouseControlState);
      };
    })(this));
    dx.select(this.drawing.canvasNode).on('touchstart', 'dx.drawing', (function(_this) {
      return function(e) {
        return onTouchStart.call(_this, e, mouseControlState);
      };
    })(this));
    mouseControlState.selection.on('touchmove', 'dx.drawing', (function(_this) {
      return function(e) {
        return onTouchMove.call(_this, e, mouseControlState);
      };
    })(this));
    mouseControlState.selection.on('touchcancel', 'dx.drawing', (function(_this) {
      return function(e) {
        return onTouchEnd.call(_this, e, mouseControlState);
      };
    })(this));
  ...
  Camera.prototype.update = function() {
    ...
    if (this.smoothZoom) {
      return this.actualZoom = dx.tween(this.actualZoom, this.zoom, this.zoomSmoothingFactor);
    } else {
...
Drawing = (function(superClass) {
  ...
  function Drawing(selector, autoStart) {
    ...
    dx.deprecatedWarning('dx.Drawing', 'N/A - This module will be removed in the next major release');
    dx.component.register(selector, this);
    container = dx.select(selector);
    container.classed('dx-drawing', true);
    this.canvas = container.append('canvas');
    this.overlay = container.append('div')["class"]('dx-drawing-overlay');
    node = container.node();
    ...
    this.globalAlpha = 1;
    this.stats = null;
    this.selectedObjects = new dx._.List;
    this.frame = 0;
    if (autoStart) {
      this.resume();
    }
  ...
  Drawing.prototype.enableSearchbox = function() {
    var input;
    this.searchbox = this.overlay.append('div').classed('dx-drawing-searchbox', true);
  ...
  Drawing.prototype.select = function(drawingObject, append) {
    ...
    if (append) {
      ...
      this.selectedObjects = new dx._.List([drawingObject]);
    }
...
Layer = (function() {
  ...
  Layer.prototype.render = function() {
    ...
    if (this.alphaCurveFunction) {
      this.alpha = dx.clampUnit(this.alphaCurveFunction(this.drawing.camera.actualZoom));
...
Sidebar = (function(superClass) {
  ...
  function Sidebar(container, pos, togglePos, populate) {
    ...
    this.visible = false;
    this.selection = container.append('div')["class"]('dx-drawing-sidebar dx-drawing-sidebar-' + pos);
    switch (pos) {
      case 't':
      case 'b':
    ...
    if (togglePos) {
      togglePos.split('');
      this.toggleBtn = container.append('div')["class"]('dx-btn dx-drawing-sidebar-toggle').classed('dx-drawing-sidebar-toggle-' + togglePos[0], true).classed('dx-drawing-sidebar-toggle-' + togglePos[1], true).on('click', (function(_this) {
        return function() {
          return _this.toggle();
        };
      })(this)).append('i')["class"]('dx-icon dx-icon-bars');
    }
...
PerformanceGauge = (function() {
  ...
  PerformanceGauge.prototype.update = function() {
    ...
    if (!isNaN(thisFrameFps)) {
      if (this.fps) {
        this.fps = dx.tween(this.fps, thisFrameFps, 0.05);
      } else {
    ...
    if (this.ms) {
      return this.ms = dx.tween(this.ms, thisFrameMs, 0.05);
    } else {
...
Composite = (function(superClass) {
  ...
  function Composite() {
    ...
    this.objectList = new dx._.List;
    this.objectMap = new dx._.Map;
  }

  Composite.prototype.set = function(name, value, duration, callback) {
    var components, obj, objectName, property;
    if (this.properties.has(name) || dx.startsWith(name, 'attr.')) {
...
FileInput = (function(superClass) {
  ...
  function FileInput(selector, options) {
    ...
    if (resolvedOptions.multiple && !options.buttonText) {
      resolvedOptions.buttonText = dx.userFacingText('fileInput', 'chooseFiles');
    }
    selection = dx.select(this.selector).classed('dx-file-input', true).classed('dx-file-input-full-width', resolvedOptions.fullWidth);
    ...
    input = dx.detached('input')["class"]('dx-file-input-hidden').attr('type', 'file').attr('accept', acceptedExtensionsString).attr('multiple', resolvedOptions.multiple ? 'multiple' : void 0);
    group = dx.detached('div')["class"]("dx-input-group dx-input-group-full-width dx-no-margin");
    button = dx.detached('button').attr('type', 'button')["class"]("dx-file-input-button dx-btn dx-no-margin " + resolvedOptions.buttonClass).on('click', function() {
      return input.node().click();
    }).add(dx.detached('i')["class"]('dx-file-input-icon dx-icon dx-icon-upload')).add(dx.detached('span').text(resolvedOptions.buttonText));
    noFilesTextDiv = dx.section().text(resolvedOptions.noFilesText);
    selectedFiles = dx.section().classed('dx-file-input-selected', true).add(noFilesTextDiv);
    imageType = /^image\//;
    filePreview = function(file) {
      var container, image, img, reader, remove, text;
      if (imageType.test(file.type)) {
        img = dx.detached('img').node();
        img.file = file;
        ...
        image = dx.section.fixed().classed('dx-file-input-preview-image', true).add(img);
      }
      container = dx.group().classed('dx-file-input-preview', true);
      text = dx.section().classed('dx-file-input-preview-text', true).text(file.name);
      remove = dx.section.fixed().classed('dx-file-input-preview-remove dx-text-negative', true).add(dx.icon({
        "class": 'dx-icon dx-icon-close'
      })).on('click', function() {
    ...
    handleFiles = (function(_this) {
      return function(fileMap) {
        ...
        selectedFiles.clear().classed('dx-btn', false).off('click', 'dx.file-input');
        length = fileMap.size;
        if (length) {
          if (length === 1) {
            if (dropdown.isOpen()) {
    ...
    dropdownDiv = dx.detached('div');
    setupDropdown = function(element) {
      var sel;
      sel = dx.select(element);
      return self._.fileMap.values().map(function(file) {
        return sel.append(filePreview(file));
      });
    };
    dropdown = new dx.Dropdown(dropdownDiv.node(), setupDropdown, {
      ddClass: 'dx-file-input-dropdown'
    });
  ...
  FileInput.prototype.disabled = function(disabled) {
    ...
    if (arguments.length) {
      ...
      this._.selectedFiles.classed('dx-text-disabled dx-background-disabled', disabled);
  ...
  FileInput.prototype.value = function(value) {
    if (arguments.length) {
      if (value != null) {
        dx.consoleWarning('dx.FileInput.value: It is not possible to set the value of a file input for security reasons. The value can only be cleared by passing in "undefined"');
      } else {
        this._.input.value('');
        this._.selectedFiles.clear().classed('dx-btn', false).off('click', 'dx.file-input');
        this._.selectedFiles.append(this._.noFilesTextDiv);
        this._.fileMap = new dx._.Map;
      }
      return this;
    } else {
...
dx.FileInput = FileInput;

dx.fileInput = function(options) {
  var selection;
  selection = dx.detached('div');
...
getButtons = function(form) {
  ...
  selection = dx.select(form.selector);
  sel = selection.select('.dx-form-buttons');
  return buttons = sel.empty() ? selection.append('div')["class"]('dx-form-buttons') : sel;
};
...
Form = (function(superClass) {
  ...
  function Form(selector) {
    ...
    dx.component.register(this.selector, this);
    this.formId = "form-" + dx.randomId() + '-';
    this.properties = new dx._.Map;
    dx.select(this.selector).classed('dx-form', true).on('keypress', 'dx.form-builder', function(e) {
      var target;
      target = dx.select(e.target || e.srcElement);
  ...
  Form.prototype.add = function(name, type, nodeType, f) {
    ...
    formSel = dx.select(this.selector);
  ...
  Form.prototype.addText = function(name, options) {
    ...
    return this.add(name, 'text', 'input', function() {
      ...
      if ((options.autoCompleteData != null) && options.type !== "password" && options.type !== "number") {
        new dx.AutoComplete(selection.node(), options.autoCompleteData, options.autoCompleteOptions != null ? options.autoCompleteOptions : options.autoCompleteOptions = void 0);
      }
  ...
  Form.prototype.addPicker = function(name, values, options) {
    ...
    return this.add(name, 'select', 'div', function() {
      ...
      picker = new dx.Picker(elem.node(), pickerOptions);
      input = this.append('input')["class"]('dx-hidden-form-input').attr('size', 0);
  ...
  Form.prototype.addAutocompletePicker = function(name, values, options) {
    ...
    return this.add(name, 'select', 'div', function() {
      ...
      autocompletePicker = new dx.AutocompletePicker(elem.node(), values, autocompletePickerOptions);
      input = this.append('input')["class"]('dx-hidden-form-input').attr('size', 0);
      this.style('position', 'relative');
      if (typeof options.required !== 'boolean') {
  ...
  Form.prototype.addRadio = function(name, values, options) {
    ...
    return this.add(name, 'radio', 'div', function() {
      ...
      for (i = 0, len = values.length; i < len; i++) {
        value = values[i];
        selection = this.append('div')["class"]('dx-radio-container');
  ...
  Form.prototype.addDatePicker = function(name, options) {
    ...
    return this.add(name, 'datepicker', 'div', function() {
      ...
      datepicker = new dx.DatePicker(elem, options.datePickerOptions);
  ...
  Form.prototype.addTimePicker = function(name, options) {
    ...
    return this.add(name, 'timepicker', 'div', function() {
      ...
      elem = this.append('div').node();
      timepicker = new dx.TimePicker(elem, options.timePickerOptions);
      getValue = function() {
  ...
  Form.prototype.addDateTimePicker = function(name, options) {
    ...
    return this.add(name, 'datetimepicker', 'div', function() {
      ...
      elem = this.append('div').node();
      datetimepicker = new dx.DateTimePicker(elem, options.dateTimePickerOptions);
      getValue = function() {
  ...
  Form.prototype.addTagInput = function(name, options) {
    ...
    return this.add(name, 'tagInput', 'div', function() {
      ...
      tagInput = new dx.TagInput(elem, options.tagInputOptions);
      if (options.required) {
        input = this.select('input');
        setValidity = function() {
          return input.node().setCustomValidity(dx.userFacingText('form', 'pleaseAddAValue'));
        };
        change = function() {
          var value;
        ...
        setValidity();
        tagInput.on('add', 'dx.form-builder', change);
        tagInput.on('remove', 'dx.form-builder', change);
      }
      return {
        required: options.required,
        key: options.key,
  ...
  Form.prototype.addFileInput = function(name, options) {
    ...
    return this.add(name, 'fileInput', 'div', function() {
      var elem, fileInput;
      elem = this.append('div').node();
      fileInput = new dx.FileInput(elem, options.fileInputOptions);
      return {
        key: options.key,
        componentNode: elem,
        hidden: options.hidden,
  ...
  Form.prototype.addButton = function(text, action, opts) {
    ...
    button = dx.detached('button').attr('type', options.buttonType).attr('id', id)["class"]("dx-btn dx-" + options.context).add(options.icon ? dx.detached('i')["class"](options.icon) : void 0).add(dx.detached('span').text(" " + text)).on('click', 'dx.form-builder', (function(_this) {
      return function(e) {
        e.preventDefault();
  ...
  Form.prototype.submit = function() {
    ...
    ref = dx.validateForm(this.selector), valid = ref.valid, errors = ref.errors;
    if (valid) {
  ...
  Form.prototype.hidden = function(property, hidden) {
    ...
    if (dx.isArray(property)) {
      ...
      if (hidden != null) {
        prop.hidden = hidden;
        dx.select(prop.node.parentNode).style('display', hidden ? 'none' : '');
  ...
  Form.prototype.disabled = function(property, disabled) {
    ...
    if (dx.isArray(property)) {
      ...
      if (disabled != null) {
        ...
        prop.extras.disable(dx.select(prop.node), disabled);
        this.properties.set(property, prop);
        return this;
      } else {
    ...
    if (arguments.length > 1) {
      ...
      if (node) {
        if (node.setCustomValidity) {
          ...
          dx.select(node).selectAll('input').nodes.map(function(e) {
      ...
      if (node) {
        if (node.setCustomValidity) {
          ...
          return dx.select(node).select('input').node.validationMessage;
        }
      }
    }
  };
  ...
  Form.prototype.value = function(property, value) {
    ...
    if (this.properties.has(property)) {
      ...
      if (arguments.length > 1) {
        switch (it.type) {
          case 'checkbox':
            return dx.select(node).prop('checked', value);
          case 'radio':
            return dx.select(node).selectAll('input').filter(function(d) {
              return d.value() === value;
            }).prop('checked', true);
          case 'fileInput':
            fileInput = dx.component(it.extras.componentNode || node);
          ...
          default:
            return dx.select(node).value(value);
        }
      } else {
        if (!it.hidden) {
          value = (function() {
            switch (it.type) {
              case 'checkbox':
                return dx.select(it.node).prop('checked');
              case 'radio':
                return dx.select(it.node).select('input:checked').value();
              case 'tagInput':
                return dx.component(it.extras.componentNode || it.node).items();
              case 'fileInput':
              ...
              default:
                return dx.select(it.node).value();
            }
          })();
          return value;
        }
...
InlineEditable = (function(superClass) {
  ...
  enterEditMode = function(options) {
    return function(toggle, content) {
      var inputSel, value;
      value = dx.select(toggle).text();
      inputSel = dx.select(content).select('.dx-name');
  ...
  function InlineEditable(selector, opts) {
    ...
    this.selector = selector;
    selection = dx.select(this.selector).classed('dx-inline-editable', true);
    defaultOptions = {
      enterValueText: dx.userFacingText('inlineEditable', 'enterValue'),
      value: selection.text()
    };
    options = dx.merge(defaultOptions, opts);
    selection.text('');
    this.textSelection = selection.append('a')["class"]('dx-morph-toggle').text(options.value || options.enterValueText);
    input = dx.detached('input')["class"]('dx-name').attr('placeholder', options.enterValueText);
    confirm = dx.detached('button')["class"]('dx-btn dx-positive dx-confirm').add(dx.detached('i')["class"]('dx-icon dx-icon-check'));
    setValue = (function(_this) {
      return function() {
        var value;
        value = input.value();
        _this.textSelection.text(value || options.enterValueText).classed('dx-inline-editable-no-value', !value.length);
        _this.emit('change', {
          cause: 'user',
          value: value
        });
    ...
    confirm.on('click', 'dx.inline-editable', setValue);
    input.on('keydown', 'dx.inline-editable', function(e) {
      if (e.key === 'Enter' || e.keyCode === 13 || e.which === 13) {
        return setValue();
      }
    });
    selection.append('div')["class"]('dx-morph-content dx-input-group').add(input).add(confirm);
...
})(dx.InlineMorphSection);

dx.inlineEditable = function(options) {
  var selection;
  selection = dx.detached('div');
...
dx.InlineEditable = InlineEditable;
...
InlinePicker = (function(superClass) {
  ...
  enterEditMode = function(toggle, content) {
    ...
    return dx.select(content).select('.dx-confirm').on('click', 'dx.inline-picker', (function(_this) {
      return function() {
  ...
  function InlinePicker(selector, options) {
    ...
    resolvedOptions = dx.merge({
      renderer: void 0,
      items: [],
      contextClass: 'dx-complement',
      ddClass: void 0,
      noValueText: void 0,
      value: void 0
    }, options);
    selection = dx.select(selector).classed('dx-inline-picker', true);
    pickerNode = dx.detached('button')["class"]('dx-btn ' + resolvedOptions.contextClass).node();
    selectedText = selection.append('a')["class"]('dx-morph-toggle');
    selection.append('div')["class"]('dx-morph-content dx-input-group').add(pickerNode).add(dx.detached('button')["class"]('dx-btn dx-positive dx-confirm').add(dx.detached('i')["class"]('dx-icon dx-icon-check')));
    picker = new dx.Picker(pickerNode, {
      renderer: resolvedOptions.renderer,
      items: resolvedOptions.items,
    ...
    picker._.menu.dropdown.on('showstart', 'dx.inline-picker', (function(_this) {
      return function() {
...
dx.inlinePicker = function(options) {
  var selection;
  selection = dx.detached('div');
...
Meter = (function(superClass) {
  ...
  function Meter(selector, options) {
    ...
    this.options = dx.merge({
      ...
      arcPadding: 0.02,
      markerPadding: 0.1,
      progressBackgroundCol: dx.color(dx.theme.palette.contrastCol).alpha(0.05).toString(),
      progressCol: dx.theme.plot.positiveCol,
      trackerCol: dx.color(dx.theme.plot.positiveCol).alpha(0.5).toString(),
      trackerBackgroundCol: dx.color(dx.theme.palette.contrastCol).alpha(0.05).toString(),
      markerCol: dx.theme.palette.contrastCol,
      valueFormatter: function(value, isTotal) {
        if (isTotal) {
          return (dx.userFacingText('meter', 'of')) + " " + value;
        } else {
          return value;
        }
      },
    ...
    randomId = dx.randomId();
    _.selection = selection = dx.select(selector).classed('dx-meter', true).on('resize', (function(_this) {
      return function() {
    ...
    _.container = container = selection.append('div')["class"]('dx-meter-container');
    ...
    _.markerText = svg.append('g').append('text')["class"]('dx-meter-marker-text').append('textPath').attr('xlink:href', '#' + randomId);
    innerText = container.append('div')["class"]('dx-meter-inner-text');
    _.completedText = innerText.append('div')["class"]('dx-meter-completed');
    _.totalText = innerText.append('div')["class"]('dx-meter-total');
    _.typeText = innerText.append('div')["class"]('dx-meter-type');
  }

  Meter.prototype.value = function(data) {
    if (arguments.length > 0) {
      this._.data = dx.merge(this._.data, data);
      this.render();
      return this;
    } else {
      return this._.data;
  ...
  Meter.prototype.render = function(cause) {
    ...
    if (size > 0) {
      ...
      updateArc = function(selection, start, end, r1, r2, col) {
        ...
        padding = 0;
        path = dx.plot.arcCurve(x, y, innerRadius, outerRadius, startRadians, endRadians, padding);
      ...
      if (options.useMarker) {
        ...
        path = dx.plot.svgCurve(points, false);
...
Paginator = (function(superClass) {
  ...
  function Paginator(selector, options) {
    ...
    this._ = dx.merge({
      ...
      currentPageAria: dx.userFacingText('paginator', 'currentPageAria'),
      gotoPageAria: dx.userFacingText('paginator', 'gotoPageAria'),
      prevPageAria: dx.userFacingText('paginator', 'prevPageAria'),
      nextPageAria: dx.userFacingText('paginator', 'nextPageAria'),
...
createTableView = function(table, head, body) {
  ...
  cellViewEnter = function(data, index, isHead) {
    ...
    type = cellIsHead || dx.isFunction(data) ? 'th' : 'td';
    cell = this.append(type)["class"]('dx-pivot-table-cell');
    if (cellIsHead) {
      cell.classed('dx-pivot-table-head-cell', true);
    }
    if (cellIsTopLeft) {
      cell.classed('dx-table-head-no-border', true);
    }
    return cell.node();
  };
  ...
  rowViewEnter = function(data, isHead) {
    ...
    rowView = dx.select(rowNode).view('.dx-pivot-table-cell').enter(function(datum) {
      var index;
    ...
    dx.component.register(rowNode, {
      view: rowView
    });
    return rowNode;
  };
  rowViewUpdate = function(data, node, index, isHead) {
    return dx.component(node).view.apply(data);
  };
  ...
  function PivotTable(selector, options) {
    ...
    this.options = dx.merge.defined({
      ...
      cellRender: function(data, element, isHead, column) {
        return dx.select(element).text(data);
      },
      useResponsive: true,
      data: void 0,
      fullWidth: void 0,
    ...
    if (data != null) {
      ...
      if (topData.length > 0 && bodyData.length > 0) {
        if (topData.length !== bodyData[0].length) {
          dx.consoleWarning('dx.PivotTable - ' + this.selector, 'The number of columns in the dataset is not equal to the number of headers provided in data.topHead');
        }
      }
      if (leftData.length > 0) {
        if (leftData.length !== bodyData.length) {
          dx.consoleWarning('dx.PivotTable - ' + this.selector, 'The number of rows in the dataset is not equal to the number of headers provided in data.leftHead');
        }
      ...
      this.table.classed('dx-table-full', false);
      if (this.options.stickyHeaders) {
        if (!this.stickyTableHeaders) {
          this.stickyTableHeaders = new dx.StickyTableHeaders(this.selector, {
        ...
        this.table.classed('dx-table-full', this.options.fullWidth);
      }
      return this;
    } else {
      return this._.data;
...
dx.pivotTable = function(options) {
  var selection;
  selection = dx.detached('div');
...
SideCollapsible = (function(superClass) {
  ...
  function SideCollapsible(selector, options) {
    ...
    this.selection = dx.select(selector).classed('dx-side-collapsible', true);
    this.openHeading = this.selection.select('.dx-side-collapsible-heading-open').classed('dx-side-collapsible-heading', true);
    this.closedHeading = this.selection.select('.dx-side-collapsible-heading-closed').classed('dx-side-collapsible-heading', true).classed('dx-side-collapsible-heading-' + this.options.position, true).classed('dx-side-collapsible-heading-no-rotate', !this.options.rotateHeading);
    this.content = this.selection.select('.dx-side-collapsible-content');
    ...
    if ((closedToggle = this.closedHeading.select('.dx-side-collapsible-toggle')).empty()) {
      this.closedHeading.on('click', 'dx.side-collapsible', (function(_this) {
        return function() {
          return _this.show();
        };
      })(this));
    } else {
      this.closedHeading.classed('dx-side-collapsible-heading-no-hover', true);
      closedToggle.on('click', 'dx.side-collapsible', (function(_this) {
        return function() {
          return _this.show();
        };
      })(this));
    }
    if ((openToggle = this.openHeading.select('.dx-side-collapsible-toggle')).empty()) {
      this.openHeading.on('click', 'dx.side-collapsible', (function(_this) {
        return function() {
          return _this.hide();
        };
      })(this));
    } else {
      this.openHeading.classed('dx-side-collapsible-heading-no-hover', true);
      openToggle.on('click', 'dx.side-collapsible', (function(_this) {
        return function() {
          return _this.hide();
        };
      })(this));
...
dx.SideCollapsible = SideCollapsible;
...
Sidebar = (function(superClass) {
  ...
  function Sidebar(selector, options) {
    ...
    dx.component.register(selector, this);
    options = dx.merge.defined({
      headerSelector: '.dx-titlebar-header',
      contentSelector: '.dx-content',
      autoAddSidebarClass: true
    }, options);
    this.selection = dx.select(selector);
    opened = dx.select('body').width() > 320 + 240;
    this.selection.classed('dx-sidebar', true).classed('dx-openable', true).classed('dx-opened', opened);
    dx.select('body').classed('dx-sidebar-opened', opened).node().offsetHeight;
    if (options.autoAddSidebarClass) {
      dx.select('body').classed('dx-sidebar-page', true);
    }
    this.selection.classed('dx-animate', true);
    dx.select(options.contentSelector).classed('dx-animate', true);
    btn = dx.select(options.headerSelector).prepend('button').attr('type', 'button')["class"]('dx-titlebar-sidebar-button').add(dx.detached('i')["class"]('dx-icon dx-icon-bars'));
    btn.on('click', 'dx.sidebar', (function(_this) {
      return function() {
        return _this.toggle();
      };
    })(this));
  ...
  Sidebar.prototype.toggle = function() {
    if (this.selection.classed('dx-opened')) {
      this.hide();
    } else {
      this.show();
    }
  ...
  Sidebar.prototype.hide = function() {
    this.selection.classed('dx-opened', false);
    dx.select('body').classed('dx-sidebar-opened', false);
    this.emit('hide');
    return this;
  };

  Sidebar.prototype.show = function() {
    this.selection.classed('dx-opened', true);
    dx.select('body').classed('dx-sidebar-opened', true);
    this.emit('show');
    return this;
  };
...
onTabSelected = function(tabs, element, i, cause) {
  ...
  tabs.selected = i;
  selection = dx.select(tabs.selector);
  selection.selectAll('.dx-tab').classed('dx-tab-active', false);
  dx.select(element).classed('dx-tab-active', true);
  context = dx.palette.context(element);
  tabsContent = selection.select('.dx-tabs-content');
  dx.palette.borderContext(tabsContent, context);
  rootSelection = dx.select(tabs.selector);
  rootSelection.selectAll('.dx-tab-content').classed('dx-tab-content-hidden', true);
  item = tabs.items()[i];
  if (item != null) {
    tabsContent.clear();
    tabs._.options.contentRenderer(tabsContent.node(), item.content);
  } else {
    tabToSelectSelector = '#' + dx.select(element).attr('data-content');
    tabToSelect = rootSelection.select(tabToSelectSelector);
    tabToSelect.classed('dx-tab-content-hidden', false);
  }
...
Tabs = (function(superClass) {
  ...
  function Tabs(selector, options) {
    ...
    defaultRenderer = function(node, value) {
      return dx.select(node).text(value);
    };
    defaultOpts = {
      items: [],
    ...
    resolvedOptions = dx.merge(defaultOpts, options);
    this._ = {};
    this._.options = resolvedOptions;
    dx.component.register(this.selector, this);
    rootSel = dx.select(this.selector);
    rootSel.classed('dx-tabs', true);
    titleRenderer = resolvedOptions.titleRenderer;
    tabsContent = rootSel.select('.dx-tabs-content');
    if (tabsContent.empty()) {
      tabsContent = rootSel.append('div')["class"]('dx-tabs-content');
    }
    this.selected = -1;
    self = this;
    if (resolvedOptions.items.length) {
      ...
      rootSel.selectAll('.dx-tab').forEach(function(node, i) {
        return node.on('click', 'dx.tabs', function() {
  ...
  Tabs.prototype.items = function(newItems) {
    ...
    if (arguments.length) {
      if (newItems.length === 0) {
        dx.consoleWarning('Setting items to empty array');
      } else {
        root = dx.select(this.selector);
        tabsContent = root.select('.dx-tabs-content');
        root.selectAll('.dx-tab').remove();
        tabsContent.clear();
        titleBarsToAdd = newItems.map((function(_this) {
          return function(arg, i) {
            ...
            tab = dx.detached('div')["class"]('dx-tab');
            _this._.options.titleRenderer(tab.node(), title);
            dx.palette.context(tab.node(), context);
            tab.on('click', 'dx.tabs', function() {
  ...
  Tabs.prototype.select = function(i, force) {
    ...
    if (this.selected !== i || force) {
      tab = dx.select(this.selector).selectAll('.dx-tab').nodes[i];
...
dx.tabs = function(opts) {
  var ret, tabs;
  ret = dx.detached('div');
  tabs = new dx.Tabs(ret.node(), opts);
  return ret;
};
...
TimeSlider = (function(superClass) {
  ...
  function TimeSlider(selector, opts) {
    ...
    options = dx.merge({
      renderer: function(slider, elem, value) {
        return dx.select(elem).text(slider.options.formatter(new Date(value)));
      }
    }, opts);
    options.min = min;
    options.max = max;
    if (typeof moment !== "undefined" && moment !== null) {
      ...
      if (options.formatter == null) {
        options.formatter = function(date) {
          return dx.zeroPad(date.getHours()) + ':' + dx.zeroPad(date.getMinutes());
        };
      }
    }
...
dx.timeSlider = function(options) {
  var selection;
  selection = dx.detached('div');
...
createNodeView = function(node, renderer, lazy) {
  return node.view('.dx-tree-node').enter(function(d) {
...
createTreeNode = function(data, renderer, lazy) {
  ...
  treeNode = dx.detached('div')["class"]('dx-tree-node');
  nodeContent = dx.detached('div').attr('class', 'dx-tree-node-content');
  renderer(nodeContent.node(), data);
  if ((data.children != null) && data.children.length > 0) {
    childContainer = dx.detached('div')["class"]('dx-tree-node-children').style('display', 'none');
    if (lazy) {
      childView = createNodeView(childContainer, renderer, lazy);
      dx.component.register(treeNode.node(), {
    ...
    return treeNode.add(dx.detached('div')["class"]('dx-tree-node-parent').add(nodeContent)).add(childContainer);
  } else {
...
formatIcon = (function(_this) {
  return function(node, iconElement, animate) {
    var children, open;
    children = dx.select(node).shallowSelect('.dx-tree-node-children');
    if (!children.selectAll('.dx-tree-node').empty()) {
      open = children.style('display') === 'block';
      dx.select(node).classed('dx-tree-node-open', open);
      if (animate) {
        return dx.select(iconElement).select('i').morph().then(open ? 'rotate-90' : 'rotate-0').go(true);
      }
    }
  };
})(this);
...
format = function(tree, element, animate) {
  ...
  treeNode = dx.select(element);
  treeNodeComponent = treeNode.component();
  renderLazyChildren = function(rootNode, recursive) {
    if ((rootNode.component() != null) && rootNode.selectAll('.dx-tree-node').empty()) {
      rootNode.component().renderChildren();
      formatChildren(tree, rootNode.selectAll('.dx-tree-node'), animate);
    }
    if (recursive) {
      return rootNode.selectAll('.dx-tree-node').map(function(sel) {
  ...
  toggle = (function(_this) {
    return function(iconElement) {
      ...
      selection = treeNode.select('.dx-tree-node-children');
  ...
  openAllOrToggle = (function(_this) {
    return function(iconElement) {
      ...
      treeNode.selectAll('.dx-tree-node-children').style('display', 'block');
      if (treeNode.shallowSelect('.dx-tree-node-children').style('display') === 'block') {
        return formatChildren(tree, treeNode.selectAll('.dx-tree-node'), animate);
      } else {
        return toggle(iconElement);
      }
    };
  })(this);
  siblings = dx.select(element.parentNode).shallowSelectAll('.dx-tree-node');
  siblingsHaveChildren = siblings.shallowSelect('.dx-tree-node-children').shallowSelectAll('.dx-tree-node').size() > 0;
  siblingsHaveLazyChildren = siblingsHaveChildren || siblings.nodes.map(function(node) {
    return dx.component(node) != null;
  }).some(dx.identity);
  showDisabled = tree.options.hideDisabledButtons ? false : siblingsHaveLazyChildren;
  childTreeNodes = treeNode.select('.dx-tree-node-children');
  if ((treeNodeComponent != null) || childTreeNodes.selectAll('.dx-tree-node').size() > 0 || showDisabled) {
    innerElem = dx.select(element).select('.dx-tree-node-parent');
    if (innerElem.size() > 0) {
      return innerElem.view('.dx-tree-node-parent-icon').enter(function() {
        var selection;
        selection = this.append('div');
        selection.attr('class', 'dx-tree-node-parent-icon').append('i').attr('class', 'dx-icon dx-icon-chevron-right dx-tree-node-state-icon');
        return selection.node();
      ...
      parent = dx.select(element);
      elem = parent.select('.dx-tree-node-content').node();
      newElem = parent.append('div').attr('class', 'dx-tree-node-parent').node();
      newElem.appendChild(elem);
      selection = dx.select(newElem).append('div');
      return selection.attr('class', 'dx-tree-node-parent-icon dx-tree-node-parent-icon-disabled').append('i').attr('class', 'dx-icon dx-icon-chevron-right');
    }
  }
};
...
recurseUpTree = function(node) {
  ...
  nodeSel = dx.select(node);
  if (nodeSel.classed('dx-tree-node')) {
    node = node.parentNode;
    nodeSel.select(':scope > .dx-tree-node-children').style('display', 'block');
  }
...
Tree = (function() {
  function Tree(selector1, options) {
    ...
    this.options = dx.merge.defined({
      ...
      renderer: function(elem, data) {
        return dx.select(elem).node().innerHTML = data.name || data;
      },
      items: [],
      lazy: false
    }, options);
    this.selection = dx.select(this.selector).classed('dx-tree dx-openable', true);
    if ((this.options.items != null) && this.options.items.length > 0) {
      this.items(this.options.items);
    }
    this.refresh(false);
  ...
  Tree.prototype.refresh = function(animate) {
    animate = animate != null ? animate : this.options.animate;
    formatChildren(this, this.selection.selectAll('.dx-tree-node'), animate);
    return this;
  };
  ...
  Tree.prototype.show = function(animate, node) {
    ...
    if (node != null) {
      ...
      this.selection.selectAll('.dx-tree-node-children').style('display', 'block');
    }
    this.selection.classed('dx-opened', true);
  ...
  Tree.prototype.hide = function(animate, node) {
    ...
    if (node != null) {
      nodeSel = dx.select(node);
      if (!nodeSel.classed('dx-tree-node')) {
        nodeSel = nodeSel.closest('.dx-tree-node');
      }
      nodeSel.select(':scope > .dx-tree-node-children').style('display', 'none');
      childArr = this.selection.selectAll('.dx-tree-node-children').style('display');
      ...
      this.selection.classed('dx-opened', false);
      this.selection.selectAll('.dx-tree-node-children').style('display', 'none');
    }
    animate = animate != null ? animate : this.options.animate;
    this.refresh(animate);
    return this;
...
dx.initializeTrees = function(selector) {
  return dx.selectAll(selector).nodes.map(function(d) {
    return new Tree(d);
  });
};
