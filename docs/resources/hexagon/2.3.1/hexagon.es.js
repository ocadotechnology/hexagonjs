...
var argmax, argmin, clamp, clampUnit, clone, cycle, debounce, defined, endsWith, extend, find, flatten, groupBy, hash, hashList, identity, isArray, isBoolean, isFunction, isNumber, isObject, isPlainObject, isString, max, maxBy, merge, mergeDefined, mergeImpl, min, minBy, randomId, range, shallowClone, shallowMerge, shallowMergeDefined, startsWith, sum, supports, supportsDate, supportsTouch, transpose, tween, unique, vendor, vendorPrefixes, zip;

// consistent string to int hashing
hash = function(str, maxValue) {
  var i, l, len, ref, res;
  res = 0;
  len = str.length - 1;
...
clampUnit = function(value) {
  return clamp(0, 1, value);
};

randomId = function(size, alphabet) {
...
min = function(values) {
  return Math.min.apply(null, values != null ? values.filter(defined) : void 0);
};

minBy = function(values, f) {
...
max = function(values) {
  return Math.max.apply(null, values != null ? values.filter(defined) : void 0);
};

maxBy = function(values, f) {
...
cycle = function(list, i) {
  return list[i % list.length];
};

hashList = function(list, str) {
  return list[hash(str, list.length)];
};
...
tween = function(start, end, amount) {
...
zip = function(arrays) {
  ...
  if (arrays) {
    if (arrays.length > 0) {
      length = min(arrays.map(function(d) {
        return d.length || 0;
      }));
      if (length > 0) {
        results = [];
...
mergeImpl = function(deep, retainUndefined, objects) {
...
merge = function() {
  ...
  return mergeImpl(true, true, objects);
};
...
mergeDefined = function() {
  ...
  return mergeImpl(true, false, objects);
};
...
shallowMerge = function() {
  ...
  return mergeImpl(false, true, objects);
};

shallowMergeDefined = function() {
  var objects = [], len = arguments.length;
  while ( len-- ) objects[ len ] = arguments[ len ];

  return mergeImpl(false, false, objects);
};
...
clone = function(obj) {
  ...
  if (isArray(obj)) {
    ...
    return new List(obj.entries().map(clone));
  } else if (obj instanceof Map) {
    return new Map(obj.entries().map(function(ref) {
      var k = ref[0];
      var v = ref[1];

      return [clone(k), clone(v)];
    }));
  } else if (obj instanceof Set) {
    return new Set(obj.keys().map(clone));
...
var EventEmitter = (function() {
  ...
  EventEmitter.prototype.off = function off (name, namespace, callback) {
    ...
    if (isString(namespace)) {
      ...
      if (!callback && !isString(namespace)) {
        callback = namespace;
      }
...
Color = (function() {
  ...
  Color.prototype.range = function range$1 (numLight, numDark, maxRange, outputFormat) {
    ...
    self = this;
    light = range(numLight + 1).map(function(i) {
      return self.clone().lighten(step * i);
    });
    dark = range(numDark).reverse().map(function(i) {
    ...
    return function(value) {
      if (arguments.length > 0) {
        if ((value != null) && !isNaN(value)) {
          this._[prop] = clamp(min, max, value);
          if (!isAlphaValue) {
            update(this._, isHSL);
          }
        }
...
getMatches = function(node) {
  return node.matches || vendor(node, 'matchesSelector') || matchPolyfill;
};
...
var Selection = (function() {
  ...
  attachSingle = function(selection, element, attacher) {
    ...
    //   return []
    if (!(isString(element) || (element instanceof Selection) || (element instanceof Element))) {
  ...
  attach = function(selection, name, attacher, reverse) {
    ...
    newNodes = isArray(name) ? (singleSelection = false, dir = reverse ? -1 : 1, ns = (function() {
      var j, len, ref, results;
      ref = dir;
      results = [];
      ...
      return results;
    })(), flatten(reverse ? ns.reverse() : ns)) : attachSingle(selection, name, attacher);
    s = new Selection(newNodes);
...
var select = function(selector) {
  if (selector instanceof Selection) {
    return selector;
  } else if (!((selector instanceof HTMLElement) || (selector instanceof SVGElement) || isString(selector) || selector === document || selector === window)) {
    // XXX Breaking: Selection select error
...
var Animation, Morph, animate, animateState, initAnimate, morph, registerMorph;
...
initAnimate = function() {
  Selection.prototype.animate = function(ease) {
    return animate(this.nodes[0], ease);
  };
...
var initMorphs = function() {
  ...
  registerMorph('fadeout', function(node, duration) {
    ...
    return animate(node).style('opacity', 0, duration);
  });
  registerMorph('fadein', function(node, duration) {
    if ( duration === void 0 ) duration = 100;

    return animate(node).style('opacity', 1, duration);
  });
  ...
  registerMorph('collapse', function(node, duration) {
    ...
    return animate(node).style('height', '0px', duration).style('padding-top', '0px', duration).style('padding-bottom', '0px', duration).style('margin-top', '0px', duration).style('margin-bottom', '0px', duration).style('width', '0px', duration).style('padding-left', '0px', duration).style('padding-right', '0px', duration).style('margin-left', '0px', duration).style('margin-right', '0px', duration).on('end', 'hx.morphs', function(e) {
  ...
  registerMorph('collapsev', function(node, duration) {
    ...
    var selection;
    selection = select(node).classed('hx-morph-hidden', true);
    return animate(node).style('height', '0px', duration).style('padding-top', '0px', duration).style('padding-bottom', '0px', duration).style('margin-top', '0px', duration).style('margin-bottom', '0px', duration).on('end', 'hx.morphs', function(e) {
  ...
  registerMorph('collapseh', function(node, duration) {
    ...
    var selection;
    selection = select(node).classed('hx-morph-hidden', true);
    return animate(node).style('width', '0px', duration).style('padding-left', '0px', duration).style('padding-right', '0px', duration).style('margin-left', '0px', duration).style('margin-right', '0px', duration).on('end', 'hx.morphs', function(e) {
  ...
  registerMorph('rotate-90', function(node, duration) {
    ...
    return animate(node).style('-webkit-transform', 'matrix(1, 0, 0, 1, 0, 0)', 'matrix(0, 1, -1, 0, 0, 0)', duration).style('transform', 'matrix(1, 0, 0, 1, 0, 0)', 'matrix(0, 1, -1, 0, 0, 0)', duration).on('end', 'hx.morphs', function() {
  ...
  registerMorph('rotate-0', function(node, duration) {
    ...
    return animate(node).style('-webkit-transform', 'matrix(0, 1, -1, 0, 0, 0)', 'matrix(1, 0, 0, 1, 0, 0)', duration).style('transform', 'matrix(0, 1, -1, 0, 0, 0)', 'matrix(1, 0, 0, 1, 0, 0)', duration).on('end', 'hx.morphs', function() {
      return select(node).style('transform', '').style('-webkit-transform', '');
    }).on('cancel', 'hx.morphs', function() {
      return select(node).style('transform', '').style('-webkit-transform', '');
    });
...
var TitleBar = /*@__PURE__*/(function () {
  ...
  TitleBar.prototype.active = function active (id) {
    ...
    if (arguments.length > 0) {
      ...
      if (id != null) {
        this._.active = isString(id) || isElement(id) || isSelection(id) ? select(id).classed('hx-selected', true) : (node = selection.node(id), node != null ? select(node).classed('hx-selected', true) : void 0);
        return this;
      }
    } else {
...
var collatorFn, compare, compareNullsLast, defaultCollator, hasCollator, localeCollatorFn, localeCompare, nullsLastCollator, sort, sortBy, state;
...
ClickDetector = /*@__PURE__*/(function (EventEmitter) {
...
var buildFilter, filter, filterCaseModifier, filterContains, filterExact, filterExcludes, filterFuzzy, filterGreater, filterLess, filterMatch, filterNumberTypes, filterRegex, filterStartsWith, filterStringTypes, filterTypes;
...
// of the match.
filterExact = buildFilter(function(term) {
...
filterStartsWith = buildFilter(function(term) {
  return function(item) {
    if (startsWith(item, term)) {
      return term.length;
    } else {
      return -1;
    }
...
filterContains = buildFilter(function(term) {
  return function(item) {
    var index;
...
filterExcludes = buildFilter(function(term) {
  return function(item) {
    var index;
    index = item.indexOf(term);
    if (index === -1) {
...
filterGreater = buildFilter(function(term) {
  return function(item) {
    var val;
...
filterLess = buildFilter(function(term) {
  return function(item) {
    var val;
...
filterFuzzy = buildFilter(function(term) {
...
filterRegex = buildFilter(function(term) {
  return function(item) {
    var match;
...
var filter$1 = /*#__PURE__*/Object.freeze({
  ...
  get filterExcludes () { return filterExcludes; },
  get filterGreater () { return filterGreater; },
  get filterLess () { return filterLess; },
  get filterFuzzy () { return filterFuzzy; },
  get filterRegex () { return filterRegex; },
  get filterStringTypes () { return filterStringTypes; },
  get filterNumberTypes () { return filterNumberTypes; },
  get filterTypes () { return filterTypes; },
  get filter () { return filter; }
});

var defaultReplacer, format, getValue, isCorrectlyFormattedArray, isNullOrNumber, isStringWithLength, lookupPlural, paramRegex, setValue, setWholeObject, toMultilineSelection, userFacingText, userFacingTextDefaults;
...
getValue = function(module, key, parseLater, params) {
  ...
  val = isArray(valueToGet) ? (n = !params || isNaN(params.n) ? 1 : params.n, lookupPlural(valueToGet, n)) : valueToGet;
...
isCorrectlyFormattedArray = function(valueToSet) {
  return (isArray(valueToSet) && valueToSet.every(function(item) {
    return isArray(item) && item.length === 3;
...
userFacingText = function() {
  ...
  if (!arguments.length) {
    ...
    return clone(state$1.localisedText);
  }
  if (arguments.length === 1) {
...
userFacingTextDefaults = function() {
  return clone(state$1.initialValues);
};
...
var deprecatedWarning, exp, fixed, format$1, formatExp, formatFixed, formatRound, formatSI, formatZeroPad, precision, round, roundPrecision, si, siSuffixes, strictCheck, zeroPad;
...
format$1 = {
  round: function(sf, strict) {
    ...
    return function(number) {
      return round(number, {strict: strict, sf: sf});
    };
  },
  si: function(sf, strict) {
    deprecatedWarning('si');
    return function(number) {
      return si(number, {strict: strict, sf: sf});
    };
  },
  exp: function(sf, strict) {
    deprecatedWarning('exp');
    return function(number) {
      return exp(number, {strict: strict, sf: sf}).replace('+', '');
    };
  },
  fixed: function(digits, strict) {
    deprecatedWarning('fixed');
    return function(number) {
      return fixed(number, {digits: digits, strict: strict});
    };
  },
  zeroPad: function(length, strict) {
    deprecatedWarning('zeroPad');
    return function(number) {
      return zeroPad(number, {length: length, strict: strict});
    };
  }
};
...
var Modal$1, ModalBase, getHeaderRender, getTitleRender, makeButtons, modal, modalDialog, modalInput;
...
Modal$1 = /*@__PURE__*/(function (ModalBase) {
...
var Notification, inbuiltNotificationManager, nextId, notify, notifyDefaultTimeout, notifyInfo, notifyInfoPlain, notifyLoading, notifyLoadingPlain, notifyNegative, notifyNegativePlain, notifyPlain, notifyPositive, notifyPositivePlain, notifyWarning, notifyWarningPlain, redraw, removeNotification, setupNotification, startTimeout, togglePin, updatePinnedStatus, wrapDeprecated$1;
...
setupNotification = function(notification, selection) {
  ...
  msgIsString = isString(msg) || isNumber(msg);
  msgIsNode = !msgIsString && ((msg instanceof Selection) || (msg instanceof HTMLElement));
  msgIsArrayOfNodes = msgIsNode || isArray(msg) && msg.every(function(item) {
    return (item instanceof Selection) || (item instanceof HTMLElement);
  });
  msgIsObject = !msgIsString && !msgIsNode && !msgIsArrayOfNodes && isObject(msg);
  if (msgIsString) {
    content.text(msg);
...
var NotificationManager = /*@__PURE__*/(function () {
  ...
  if ( selector === void 0 ) selector = 'body';
    ...
    this.currentId = 0;
    this.notifications = [];
    this.uniqueId = 'hx-notify-' + randomId();
    this._ = {
      defaultTimeout: 5
    };
  }
  ...
  NotificationManager.prototype.info = function info (message, options) {
    ...
    return this.notify(message, merge({
...
notifyDefaultTimeout = function(timeout) {
...
// XXX Deprecated: Remove in next major
notify = wrapDeprecated$1('notify', notifyPlain);

notifyInfo = wrapDeprecated$1('notifyInfo', notifyInfoPlain);

notifyPositive = wrapDeprecated$1('notifyPositive', notifyPositivePlain);

notifyWarning = wrapDeprecated$1('notifyWarning', notifyWarningPlain);

notifyNegative = wrapDeprecated$1('notifyNegative', notifyNegativePlain);

notifyLoading = wrapDeprecated$1('notifyLoading', notifyLoadingPlain);

notify.info = wrapDeprecated$1('hx.notify.info', notifyInfoPlain);

notify.positive = wrapDeprecated$1('hx.notify.positive', notifyPositivePlain);

notify.warning = wrapDeprecated$1('hx.notify.warning', notifyWarningPlain);

notify.negative = wrapDeprecated$1('hx.notify.negative', notifyNegativePlain);

notify.loading = wrapDeprecated$1('hx.notify.loading', notifyLoadingPlain);
...
dropdownContentToSetupDropdown = function(dropdownContent) {
  switch (false) {
    ...
    case !isFunction(dropdownContent):
      return function(node) {
...
var Dropdown = /*@__PURE__*/(function (EventEmitter) {
  ...
  Dropdown.prototype.dropdownContent = function dropdownContent (dropdownContent$1) {
    ...
    if (arguments.length) {
      ...
      this._ = shallowMerge(this._, {setupDropdown: setupDropdown, dropdownContent: dropdownContent$1});
      this.render();
      return this;
    } else {
...
var Menu, MenuBase, MenuItem, addItem, checkEvent, dealWithEvent, emitItem, getAllItems, moveSelectionDown, moveSelectionUp, populateNode, setActive, setupInner, toggleCollapsible;
    ...
    selection.on('click', 'hx.menu', function () {
      ...
      if (!this$1.options.disabled) {
        if (this$1.dropdown.isOpen()) {
          ...
          if (!this$1.loading) {
            if ((this$1.data != null) && isFunction(this$1.data)) {
              this$1.loading = true;
...
Menu = /*@__PURE__*/(function (MenuBase) {
...
var Autocomplete, autocomplete, buildAutocomplete, findTerm, showAutocomplete, sortActive;
...
sortActive = function(items) {
  ...
  groupedActive = new Map(groupBy(items, function(i) {
    return !i.disabled;
  }));
...
findTerm = function(term, forceMatch) {
  ...
  if (this.options.showOtherResults && !forceMatch && !dataMatches) {
    ...
    remainingResults = filteredData.length === 0 ? allData : (data = allData.filter(function(d) {
      ...
      return sort.compare(self.options.inputMap(a), self.options.inputMap(b));
    }) : data.sort(sort.compare) : void 0, data);
...
buildAutocomplete = function(searchTerm, fromCallback, loading) {
  ...
  if ((_.callback != null) && !fromCallback) {
    ...
    if (filteredData == null) {
      ...
      message.text = userFacingText.format(this.options.pleaseEnterMinCharactersMessage, {
...
Autocomplete = /*@__PURE__*/(function (EventEmitter) {
  function Autocomplete(selector, data1, opts) {
    ...
    if (!isArray(this.data) && !isFunction(this.data)) {
      ...
      this.options.filterOptions = merge({}, _filterOpts, opts.filterOptions);
      if ((base = this.options).filter == null) {
        base.filter = function (arr, term) {
          ...
          filtered = filter$1[filterName](arr, term, self.options.filterOptions);
      ...
      if ((base2 = this.options).placeholder == null) {
        base2.placeholder = this.options.minLength > 0 ? userFacingText.format(this.options.minCharactersMessage, {
          minLength: this.options.minLength
        }) : void 0;
      }
  ...
  Autocomplete.prototype.clearCache = function clearCache () {
    this._.data = new Map();
    if ((this.data != null) && !isFunction(this.data)) {
...
var Preferences = /*@__PURE__*/(function (EventEmitter) {
  ...
  Preferences.prototype.setup = function setup (options) {
    ...
    var setupModal = function (element, thisModal) {
      ...
      var locAutocomplete = new Autocomplete(
      ...
      var localeSection = div().add(detached('label').text(userFacingText('preferences', 'locale'))).add(localeAutocompleteElement);

      var timezoneAutocompleteElement = detached('input').class('hx-preferences-timezone');
      var tzAutocomplete = new Autocomplete(
      ...
      var timezoneSection = div().add(detached('label').text(userFacingText('preferences', 'timezone'))).add(timezoneAutocompleteElement);
    ...
    var modal = new ModalBase(userFacingText('preferences', 'preferences'), setupModal);
    this._ = {
...
var DateTimeLocalizer = /*@__PURE__*/(function (PreferencesHandler) {
  ...
  DateTimeLocalizer.prototype.date = function date (date$1, useInbuilt) {
    if (useInbuilt) {
      return ((date$1.getFullYear()) + "-" + (zeroPad(date$1.getMonth() + 1)) + "-" + (zeroPad(date$1.getDate())));
    }
    return ((zeroPad(date$1.getDate())) + "/" + (zeroPad(date$1.getMonth() + 1)) + "/" + (date$1.getFullYear()));
  };
...
var IntlDateTimeLocalizer = /*@__PURE__*/(function (PreferencesHandler) {
  ...
  IntlDateTimeLocalizer.prototype.setupFormatters = function setupFormatters () {
    ...
    var months = range(12).map(function (_, i) { return month.format(new Date(Date.UTC(2019, i, 1))); });
    var fullMonths = range(12).map(function (_, i) { return fullMonth.format(new Date(Date.UTC(2019, i, 1))); });
    this._.formatters = {
      year: year,
      date: date,
      time: time,
...
sortItems = function(valueLookup) {
  if ( valueLookup === void 0 ) valueLookup = identity;

  return function(a, b) {
    return compare(valueLookup(a), valueLookup(b));
  };
};
...
filterNested = function(items, term, filterName, filterOptions) {
  if (items.some(function(item) {
    ...
    return items.reduce(function(acc, item) {
      ...
      filterSingle = filter$1[filterName]([item], term, filterOptions);
      return acc.concat( filterSingle);
    }, []);
  } else {
    return filter$1[filterName](items, term, filterOptions);
  }
};

AutocompleteFeed = /*@__PURE__*/(function () {
  ...
  AutocompleteFeed.prototype.filter = function filter (term, callback) {
    ...
    if (term === void 0) {
      ...
      } else if (_.options.matchType === 'external' && isFunction(_.items)) {
  ...
  AutocompleteFeed.prototype.validateItems = function validateItems (items) {
    return Array.isArray(items) || isFunction(items);
  };
...
ButtonGroup = /*@__PURE__*/(function (EventEmitter) {
...
var Picker, PickerBase, picker, pickerBase, setValue$1;
...
setValue$1 = function(picker, value, items, cause) {
  ...
  for (j = 0, len = items.length; j < len; j++) {
    item = items[j];
    if (item === value || (isObject(item) && (item.value === value || item.value === (value != null ? value.value : void 0)))) {
      newVal = item;
      break;
    }
  }
...
Picker = /*@__PURE__*/(function (PickerBase) {
...
var DragContainer, containerChildren, drag, dragContainer, endDrag, getGrid, startDrag;
...
containerChildren = function(container) {
  ...
  return items.filter(defined);
};

DragContainer = /*@__PURE__*/(function (EventEmitter) {
  ...
  DragContainer.prototype.order = function order (order$1) {
    ...
    if (arguments.length > 0) {
      ...
      return containerChildren(this).map(this.lookup()).filter(defined);
    }
  };
...
dragContainer = function(options) {
  var selection;
  selection = div();
...
Toggle = /*@__PURE__*/(function (EventEmitter) {
...
var Crumbtrail, crumbtrail;

Crumbtrail = /*@__PURE__*/(function () {
  ...
  Crumbtrail.prototype.items = function items (data) {
    if (data != null) {
      ...
      this.view.apply(flatten(data.map(function(d) {
        return [d, 0];
      })).slice(0, -1));
      return this;
    } else {
...
var AutocompletePicker, AutocompletePickerBase, autocompletePicker, debounceDuration, enterKeyCode, setPickerValue, validateItems;
...
AutocompletePicker = /*@__PURE__*/(function (AutocompletePickerBase) {
...
autocompletePicker = function(items, options) {
  var selection;
  selection = div();
...
getChildrenFromTable = function(t, body, single) {
  ...
  return flatten(realParents.map(function(parent) {
...
StickyTableHeaders = /*@__PURE__*/(function () {
...
doCollisionDetection = function(nodesRaw) {
  ...
  reductor = function(oldDistance, ref) {
    ...
    tuple = find(previousNodes, function(ref) {
...
populateLegendSeries = function(selection, series) {
  ...
  selection.view('.hx-legend-entry', 'g').enter(function() {
    ...
    this.select('text').text(isFunction(s.title) ? s.title() : s.name).attr('y', i * 20 + 10).attr('x', 15);
    return this.select('rect').text(isFunction(s.title) ? s.title() : s.name).attr('y', i * 20).attr('x', 0).attr('width', 10).attr('height', 10).attr('fill', s.legendColor ? s.legendColor() : s.fillColor); // XXX: the else s.fillColor is there for PieCharts... which should be fixed when pie charts are refactored
  }).apply(series);
  width = max(selection.selectAll('text').nodes.map(function(node) {
...
var plotLabelBasic, plotLabelStandard;

plotLabelStandard = function(element, meta) {
...
plotLabelBasic = function(element, meta) {
  ...
  var LineSeries = /*@__PURE__*/(function (Series) {
    ...
    LineSeries.prototype.updateSvg = function updateSvg (fillLayer, sparseLayer) {
      ...
      if (this.markersEnabled()) {
        preparedData = this.axis.y.scaleType() !== 'discrete' ? applyStack(flatten(featheredData)) : flatten(featheredData);
  ...
  var BarSeries = /*@__PURE__*/(function (Series) {
    ...
    BarSeries.prototype.getLabelDetails = function getLabelDetails (x, y) {
      ...
      if (this.labelsEnabled()) {
        ...
        barData = find(this.data(), function(d) {
          return d.x === xx;
        });
        ...
        if (barData) {
          ...
          if ((xx != null) && (yy != null) && (barData != null)) { // and barX <= x < barX + width
            ...
            yy = clamp(min, max, this.axis.yScale.apply(yy));
            meta = {
              series: this,
              title: this.title(),
              x: barX + width / 2,
...
var Axis, dimension, supportsGroup;
...
Axis = (function() {
  ...
  Axis.prototype.preupdateXAxisSvg = function preupdateXAxisSvg (element) {
    ...
    if (this.x.scaleType() === 'discrete') {
      ...
      xmin = min(xs.map(function(d) {
        return d[0];
      }));
      xmax = max(xs.map(function(d) {
        return d[1];
      }));
  ...
  Axis.prototype.getLabelDetails = function getLabelDetails (x, y) {
    var labels;
    labels = flatten(this.series().map(function(series) {
  ...
  Axis.prototype.getYStack = function getYStack (type, group, x, seriesId, start) {
    ...
    if (group) {
      ...
      return yStack + sum(maybeys.filter(identity));
    } else {
      return Math.max(start, 0);
    }
  };
  ...
  Axis.prototype.calculateYBounds = function calculateYBounds (yMinMightBeAuto, yMaxMightBeAuto) {
    ...
    if ('auto' === yMaxMightBeAuto || 'auto' === yMinMightBeAuto) {
      ...
      types = groupBy(allSeries, function(d) {
        return d._.type;
      });
      stackGroups = types.map(function(ref) {
        ...
        return {
          type: type,
          group: groupBy(series, function(s) {
            if (supportsGroup(s)) {
              return s.group();
            } else {
              return void 0;
      ...
      typeGroupReductor = function(type) {
        return function(ref, ref$1) {
          ...
          ((assign = seriesGroup === void 0 ? (maybeys = allSeries.map(function(s) {
            ...
            yymin: min(ys.map(function(d) {
              return d[0];
            })),
            yymax: max(ys.map(function(d) {
              return d[1];
            }))
          }) : (allX = unique(flatten(series.map(function(s) {
            ...
            return sum(maybeys.filter(identity));
          }), {
...
Graph = (function() {
    ...
    function Graph(selector, options) {
      ...
      this.svgTarget.on('touchmove', 'hx.plot', function (e) {
        ...
        if (e.targetTouches.length > 1 && this$1.zoomEnabled()) {
          ...
          this$1.zoomRangeStart(clampUnit(xhat + z * startFactor));
          this$1.zoomRangeEnd(clampUnit(xhat + z * endFactor));
          this$1.emit('zoom', {
    ...
    Graph.prototype.render = function render () {
      ...
      hasData = this.axes().some(function(axis) {
        return axis.series().some(function(series) {
          var data;
          data = series.data();
          return isObject(data) || data.length > 0;
        });
      });
      self = this;
      ...
      if (this.legendEnabled()) {
        ...
        populateLegendSeries(legendContainer, flatten(this.axes().map(function(axis) {
          return axis.series();
        })));
  ...
  getClosestMeta = function(graph, x, y) {
    ...
    labels = flatten(graph.axes().map(function(axis) {
...
PieChart = (function() {
  ...
  var PieChart = /*@__PURE__*/(function (EventEmitter) {
    ...
    PieChart.prototype.render = function render () {
      ...
      if (this.legendEnabled()) {
        ...
        populateLegendSeries(legendContainer, flatten(data.map(function(d) {
          return d.segments;
        })));
        switch (this.legendLocation()) {
          case 'top-left':
  ...
  defaultSegmentTextFormatter = function(segment, segments) {
    if (segment.size / sum(segments.map(function(s) {
      return s.size;
    })) > 0.05) {
      return segment.size;
    } else {
  ...
  calculateTotal = function(segments) {
    ...
    allZero = false;
    preTotal = sum(segments.map(function(x) {
      return x.size;
    }));
    total = preTotal === 0 ? (allZero = true, segments.length) : sum(segments.map(function(x) {
...
var Slider, discreteToPos, posToDiscrete, posToValue, renderValues, slide, slideEnd, slider;
...
Slider = /*@__PURE__*/(function (EventEmitter) {
  function Slider(selector, options) {
    ...
    this.options = merge({
      ...
      renderer: function(slider, elem, value) {
        return select(elem).text(fixed(value));
      },
      min: 0,
      max: 1,
      step: void 0,
    ...
    if (arguments.length > 0) {
      clampValue = function (val) {
        if (this$1._.isDiscrete) {
          ...
          return clamp(this$1.options.min, this$1.options.max, val);
        }
      };
      if (this.options.type === 'range') {
...
var backgroundContext, card, defineComponent, directions, fixes, getComponentParent, headers, i$1, joints, len, ref, size, sizes, textContext, textLikeComponent;
...
card.small.text = function(options) {
  return card.text(options).classed('hx-card-small', true);
};

card.large.text = function(options) {
  return card.text(options).classed('hx-card-large', true);
};

card.title = textLikeComponent('span', 'hx-card-title hx-header');

card.small.title = function(options) {
  return card.title(options).classed('hx-card-small', true);
};

card.large.title = function(options) {
  return card.title(options).classed('hx-card-large', true);
};

card.icon = function(options) {
  var selection;
...
card.small.icon = function(options) {
  return card.icon(options).classed('hx-card-small', true);
};

card.large.icon = function(options) {
  return card.icon(options).classed('hx-card-large', true);
};
...
card.action.section = function(options) {
  return card.section(options).add(detached('a').attr('href', options.link).class('hx-card-action').add(card.text(options)));
};

card.action.icon.section = function(options) {
  return card.section(options).add(detached('a').attr('href', options.link).class('hx-card-action').add(card.text(options)).add(card.icon(options)));
};

card.icon.action.section = function(options) {
  return card.section(options).add(detached('a').attr('href', options.link).class('hx-card-action').add(card.icon(options)).add(card.text(options)));
};

// Complete cards
card.notice = function(options) {
  if ( options === void 0 ) options = {};

  return card().classed('hx-card-small', true).add(card.header.section({
    sectionContext: options.context || 'info'
  }).add(card.title({
    titleText: options.title || 'Note'
  }))).add(card.section().add(card.text({
    text: options.message
  })));
};

var Paginator, getPageItems, getRange, makeItem, makeRange, paginator;
...
makeRange = function(first, last) {
  return range(last - first + 1).map(function(_, index) {
    return index + first;
  });
};
...
Paginator = (function() {
      ...
      data = this._.v2Features.useAccessibleRendering ? getPageItems(currentPage, currentPageCount, this._.v2Features.padding).map(function (item) {
        ...
        if (item === 'prev') {
          return {
            ...
            aria: userFacingText.format(this$1.prevPageAria(), {
              page: currentPage - 1
            }),
            onClick: function () {
              return selectPage.call(this$1, 'user', currentPage - 1);
        ...
        if (item === 'next') {
          return {
            ...
            aria: userFacingText.format(this$1.nextPageAria(), {
              page: currentPage + 1
            }),
            onClick: function () {
              return selectPage.call(this$1, 'user', currentPage + 1);
        ...
        return {
          text: numericItem,
          aria: userFacingText.format(aria, {
            page: numericItem
          }),
          selected: selected,
          onClick: function () {
  ...
  selectPage = function(cause, value) {
    ...
    currentPageCount = this.pageCount();
    currentPage = this.page();
    newPage = currentPageCount === void 0 ? Math.max(value, 1) : clamp(1, currentPageCount, value);
...
SideCollapsible = /*@__PURE__*/(function (EventEmitter) {
...
var DatePicker, buildCalendar, buildDatepicker, calendarGridRowUpdate, calendarGridUpdate, datePicker, getCalendarDecade, getCalendarMonth, getCalendarYear, isBetweenDates, isSelectable, isSelected, isToday, setupInput, toggleInputValidity, updateDatepicker, validateDates;
...
DatePicker = /*@__PURE__*/(function (EventEmitter) {
  function DatePicker(selector, options) {
    ...
    if (this.options.selectRange) {
      ...
      _.useInbuilt = this.options.allowInbuiltPicker ? (typeof moment === "undefined" || moment === null) && supports('date') && supports('touch') : false;
...
var PivotTable, createTableView, pivotTable;
...
createTableView = function(table, head, body) {
  ...
  cellViewEnter = function(data, index, isHead) {
    ...
    type = cellIsHead || isFunction(data) ? 'th' : 'td';
...
PivotTable = /*@__PURE__*/(function (EventEmitter) {
    ...
    if (data$1 != null) {
      ...
      if (this.options.stickyHeaders) {
        if (!this.stickyTableHeaders) {
          this.stickyTableHeaders = new StickyTableHeaders(this.selector, {
...
var TimePicker, setupInput$1, setupTimepicker, timePicker, updateTimePicker;
...
TimePicker = /*@__PURE__*/(function (EventEmitter) {
  function TimePicker(selector, options) {
    ...
    timeout = void 0;
    if (supports('date') && supports('touch')) {
...
Tabs = /*@__PURE__*/(function (EventEmitter) {
...
var Tree, createChildren, createNodeView, createTreeNode, format$2, formatChildren, formatIcon, initializeTrees, recurseUpTree, tree;
...
formatChildren = function(tree, children, animate) {
  return children.forEach(function (d) {
    return format$2(tree, d.node(), animate);
  });
};
...
format$2 = function(tree, element, animate) {
...
initializeTrees = function(selector) {
  return selectAll(selector).nodes.map(function(d) {
    return new Tree(d);
  });
};
...
var json = reshapedRequest('application/json');

var html = reshapedRequest('text/html');

var text = reshapedRequest('text/plain');

var reshapedRequest = reshapedRequest();
...
*/
var DataTable, advancedSearchCriteriaValidate, capitalize, collapseBreakPoint, columnOptionLookup, createAdvancedSearchView, createPageSizeBlock, createPaginationBlock, dataTable, defaultTermLookup, fullWidthColSpan, getAdvancedSearchFilter, getFiltered, getRowSearchTerm, objectFeed, spacer, splitArray, stripLeadingAndTrailingWhitespaceRegex, toCriteriaItems, urlFeed, whitespaceSplitRegex,
  splice = [].splice;
...
toCriteriaItems = function(list) {
  return unique(list).map(function(item) {
    return {
      value: item,
...
advancedSearchCriteriaValidate = function(value) {
  ...
  if ((isArray(value) && value.every(function(c) {
...
createAdvancedSearchView = function(selection, dataTable, options) {
  ...
  advancedSearchRowEnter = function(filterGroup, filterGroupIndex) {
    return function(filterRow, index, trueIndex) {
      ...
      criteriaAnyPlaceholder = div('hx-data-table-advanced-search-criteria-placeholder hx-text-disabled hx-background-disabled').text(userFacingText('dataTable', 'contains'));
      debouncedInput = debounce(200, function(e) {
        var assign, assign$1;
...
DataTable = (function() {
  ...
  var DataTable = /*@__PURE__*/(function (EventEmitter) {
    ...
    DataTable.prototype.selectedRows = function selectedRows (value, cb) {
      var newSelectedRows;
      if (arguments.length > 0 && !isFunction(value)) {
        // Deal with single select mode when setting the selected rows
        if (this.singleSelection() && isArray(value) && value.length) {
          value = [value[0]];
        }
        ...
        if (advancedSearchVisibleAndEnabled) {
          ...
          this$1._.advancedSearchView.apply(currentFilters.filter(function(x) {
            ...
            return filterGroup.map(function(filterRow) {
              return merge(filterRow, {headers: headers, getColumnOption: getColumnOption});
            });
          }));
        }
        ...
        return feed.totalCount(function (totalCount) {
          ...
          return feed.rows(range, function (ref) {
            ...
            if (headers.some(function(header) {
              ...
              sortColumns = flatten(headers.map(function(header) {
            ...
            if (selectPageSize) {
              ...
              pageSizeOptions = options.pageSizeOptions.sort(compare).map(function(item) {
                return {
                  text: item,
                  value: item
                };
            ...
            this$1.updateSelected = function () {
              ...
              if (totalCount !== void 0) {
                return this$1._.statusBar.select('.hx-data-table-status-bar-text').text(userFacingText.format(options.selectedRowsText, {
            ...
            if (options.useStickyHeaders) {
              ...
              this$1._.stickyHeaders = new StickyTableHeaders(container.node(), stickyOpts);
            }
            if (scrollLeft != null) {
  ...
  columnOption = function(name) {
    return function(columnId, value, cb) {
      var base, options;
      options = this._.options;
      if (arguments.length > 1 && isString(columnId)) {
        ...
        if (isString(columnId) && options.columns[columnId]) {
...
defaultTermLookup = function(term, rowSearchTerm, criteria) {
  ...
  lookupArr = isString(rowSearchTerm) ? [rowSearchTerm] : rowSearchTerm;
  arr = term.replace(stripLeadingAndTrailingWhitespaceRegex, '').split(whitespaceSplitRegex);
  validPart = find(arr, function(part) {
    return filter$1["filter" + capitalize(criteria)](lookupArr, part.toLowerCase()).length;
  });
  return defined(validPart);
};

getAdvancedSearchFilter = function(cellValueLookup, termLookup) {
  if ( cellValueLookup === void 0 ) cellValueLookup = identity;
  ...
  return function(filters, row) {
    ...
    validFilters = find(filters, function(groupedFilters) {
      var invalidFilter;
      invalidFilter = find(groupedFilters, function(currFilter) {
        var searchTerm;
      ...
      return !defined(invalidFilter);
    });
    return defined(validFilters);
  };
};
...
getValidationMessage = function(message, type) {
  switch (message.toLowerCase()) {
    case 'value missing':
      if (type === 'radio') {
        return userFacingText('form', 'missingRadioValue');
      } else {
...
var TagInput, createFilteredData, tagInput;
  ...
  var TagInput = /*@__PURE__*/(function (EventEmitter) {
    function TagInput(selector, options) {
      ...
      if (this.options.autocompleteData != null) {
        ...
        this._.autocomplete = new Autocomplete(this.input.node(), acData, this.options.autocompleteOptions);
        inputMap = ((ref = this._.autocomplete.options) != null ? ref.inputMap : void 0) || identity;
...
var DateTimePicker = /*@__PURE__*/(function (EventEmitter) {
  function DateTimePicker(selector, options) {
    ...
    this.datePicker = new DatePicker(dtNode, this.options.datePickerOptions);
    this.timePicker = new TimePicker(tpNode, this.options.timePickerOptions);
...
var SingleSelect = /*@__PURE__*/(function (EventEmitter) {
  function SingleSelect(selector, items, options) {
    ...
    var resolvedOptions = merge({}, defaultOptions, options);
    ...
    var shouldDebounce = isFunction(items);

    var feed = new AutocompleteFeed({
      filter: filter,
    ...
    var debouncedPopulate = debounce(debounceDuration$1, filterAndRenderMenu);
...
var Form = /*@__PURE__*/(function (EventEmitter) {
  ...
  Form.prototype.addDatePicker = function addDatePicker (name, options) {
    ...
    return this.add(name, 'date-picker', function() {
      ...
      dpOpts = this.options.featureFlags.useUpdatedStructure ? merge({}, options.datePickerOptions, {
        v2Features: {
  ...
  Form.prototype.addTagInput = function addTagInput (name, options) {
    ...
    return this.add(name, 'tag-input', function() {
      ...
      componentElem = div();
      tiOpts = this.options.featureFlags.useUpdatedStructure ? merge({}, options.tagInputOptions, {
        featureFlags: {
          useInputClass: true
        }
  ...
  Form.prototype.addSingleSelect = function addSingleSelect (name, items, options) {
    ...
    return this.add(name, 'select', function() {
      ...
      singleSelectOptions = merge({required: required}, options.singleSelectOptions);
...
var Meter = /*@__PURE__*/(function (EventEmitter) {
  function Meter(selector, options) {
    ...
    this.options = merge({
      ...
      valueFormatter: function(value, isTotal) {
        if (isTotal) {
          return ((userFacingText('meter', 'of')) + " " + value);
        } else {
          return value;
        }
      },
  ...
  Meter.prototype.value = function value (data) {
    if (arguments.length > 0) {
      this._.data = merge(this._.data, data);
      this.render();
      return this;
    } else {
      return this._.data;
...
var Camera, Circle, ColorProperty, Composite, DiscreteProperty, Drawing, DrawingColor, DrawingObject, Grid, Layer, Line, NumberProperty, PerformanceGauge, Point, Rectangle, Shape, Sidebar$1, StringProperty, Text, drawing, horizontalLineIntersection, pointRectangleIntersection, verticalLineIntersection;
...
DrawingColor = /*@__PURE__*/(function () {
  ...
  DrawingColor.prototype.set = function set (name, value) {
    switch (name) {
      ...
      case 'color.red':
        return this.c.red(clamp(0, 255, value));
      case 'color.green':
        return this.c.green(clamp(0, 255, value));
      case 'color.blue':
        return this.c.blue(clamp(0, 255, value));
      case 'color.alpha':
        return this.c.alpha(clampUnit(value));
    }
  };
...
Composite = /*@__PURE__*/(function (DrawingObject) {
  ...
  Composite.prototype.set = function set (name, value, duration, callback) {
    ...
    if (this.properties.has(name) || startsWith(name, 'attr.')) {
...
PerformanceGauge = /*@__PURE__*/(function () {
  ...
  PerformanceGauge.prototype.update = function update () {
    ...
    if (!isNaN(thisFrameFps)) {
      if (this.fps) {
        this.fps = tween(this.fps, thisFrameFps, 0.05);
      } else {
    ...
    if (this.ms) {
      return this.ms = tween(this.ms, thisFrameMs, 0.05);
    } else {
...
Layer = (function() {
  ...
  Layer.prototype.render = function render () {
    ...
    if (this.alphaCurveFunction) {
      this.alpha = clampUnit(this.alphaCurveFunction(this.drawing.camera.actualZoom));
...
Camera = (function() {
  ...
  var Camera = /*@__PURE__*/(function (EventEmitter) {
    ...
    Camera.prototype.update = function update () {
      ...
      if (this.smoothZoom) {
        return this.actualZoom = tween(this.actualZoom, this.zoom, this.zoomSmoothingFactor);
      } else {
