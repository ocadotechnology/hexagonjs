import { card, div, range, select, theme, sparkline } from 'hexagon-js'


function titleText (title, text) {
  return card.section()
    .add(div().add(card.title({text: title})))
    .add(div().add(card.text({text: text})))
}

function sparklineExample (options) {
  return card.group()
    .add(card.fixed.section().add(card.text({text: options.text})))
    .add(card.section().add(sparkline({strokeColor: options.sparklineColor, data: options.sparklineData})))
}
...
function statusCard (name, context) {
  return card()
    .add(card.header.section({ context: context})
