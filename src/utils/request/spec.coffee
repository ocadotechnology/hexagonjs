...
describe 'Request API', ->
  ...
  describe 'request', ->
    ...
    it 'should use POST by default as the request type when data is provided', ->
      request 'test.file', {some: 'data'}, defaultCb
    ...
    it 'should correctly set the requestType', ->
      ...
      request 'test.file', defaultCb, options
    ...
    it 'should thrown an error when an incorrect url is passed in', ->
      ...
      request defaultCb
      console.error.should.have.been.called.with('Incorrect URL passed into request: ', defaultCb)

      arr = []
      request arr
      console.error.should.have.been.called.with('Incorrect URL passed into request: ', arr)
  ...
  describe 'html', ->
    ...
    it 'should pass the data through', ->
      data = {some: 'data'}
      html 'test.html', data, defaultCb

      requests[0].requestBody.should.equal(JSON.stringify data)

  describe 'text', ->
    it 'should correctly  set the mimeType to text/plain', ->
      text 'test.txt', defaultCb
    ...
    it 'should pass the data through', ->
      data = {some: 'data'}
      text 'test.txt', data, defaultCb
      requests[0].requestBody.should.equal(JSON.stringify data)

  describe 'reshapedRequest', ->
    ...
    it 'should log a warning when the content type defined is not text, json or plain', (done) ->
      ...
      cb = (error, result, source) ->
        should.not.exist(error)
        should.exist(result)
        logger.warn.should.have.been.called.with("Unknown parser for mime type application/octet-stream, carrying on anyway")
        done()

      reshapedRequest 'test.bin', cb
      requests[0].method.should.equal('GET')
