...
import { isString, isArray, isObject, randomId, vendor, flatten } from 'utils/utils'
...
export class Selection
  ...
  api: (name, apiObject) ->
    ...
    if arguments.length == 0
      ...
      # XXX Deprecated: Component
      if not hedo.components.indexOf(apiObject) is -1
...
export component = (selector) ->
  ...
  getHexagonElementDataObject(select(selector).node()).components[0]

# lists the components for an element
export components = (selector) ->
  ...
  (getHexagonElementDataObject(select(selector).node()).components or []).slice()
