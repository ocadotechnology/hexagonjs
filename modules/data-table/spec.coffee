import { DataTable, dataTable, objectFeed, getAdvancedSearchFilter } from 'data-table'
import { detached, select, Selection, div, span } from 'selection'
...
export default () ->
  describe 'data-table', ->
    ...
    emulatePickerValueChange = (pickerNode, dt, valueIndex, done, expectation) ->
      ...
      emit(pickerNode, 'click', fakeEvent)
      ...
      emit(dropdown.node(), 'click', pickerEvent)
      ...
      describe name, ->
        ...
        it 'should call callback if passed in', ->
          for value in valuesToCheck
            ...
            options[name] = value
            dt = new DataTable(div(), options)
            dt.feed(objectFeed(threeRowsData))
            cb = chai.spy()
            dt[name](value, cb)
    ...
    testTable = (options, done, spec) ->
      ...
      dt = new DataTable(container, tableOptions)
      dt.feed objectFeed(data), ->
        spec(container, dt, options, data)
        fixture.clear()
        done?()

    it 'should have user facing text defined', ->
      userFacingText('dataTable', 'addFilter').should.equal('Add Filter')
      userFacingText('dataTable', 'advancedSearch').should.equal('Advanced Search')
      userFacingText('dataTable', 'and').should.equal('and')
      userFacingText('dataTable', 'anyColumn').should.equal('Any column')
      userFacingText('dataTable', 'clearFilters').should.equal('Clear Filters')
      userFacingText('dataTable', 'clearSelection').should.equal('clear selection')
      userFacingText('dataTable', 'loading').should.equal('Loading')
      userFacingText('dataTable', 'noData').should.equal('No Data')
      userFacingText('dataTable', 'noSort').should.equal('No Sort')
      userFacingText('dataTable', 'or').should.equal('or')
      userFacingText('dataTable', 'rowsPerPage').should.equal('Rows Per Page')
      userFacingText('dataTable', 'search').should.equal('Search')
      userFacingText('dataTable', 'selectedRows').should.equal('$selected of $total selected.')
      userFacingText('dataTable', 'sortBy').should.equal('Sort By')

    describe 'global options', ->
      checkOption('collapsibleRenderer', [((d) -> span().text(d)), ((d) -> span().text(d*2)), ((d) -> span().text(d+''))])
      checkOption('compact', ['auto', true, false])
      checkOption('displayMode', ['paginate', 'all'])
      checkOption('filter', ['bob', 'aaaa', undefined])
      checkOption('feed', [objectFeed(noData), objectFeed(threeRowsData)])
    ...
    describe 'column options', ->
      ...
      checkColumnOption('cellRenderer', [((d) -> span().text(d)), ((d) -> span().text(d*2)), ((d) -> span().text(d+''))])
      checkColumnOption('headerCellRenderer', [((d) -> span().text(d)), ((d) -> span().text(d*2)), ((d) -> span().text(d+''))])
    ...
    describe 'render', ->
      ...
      describe 'default options', ->
        testTable {}, undefined, (container, dt, options, data) ->
          ...
          it 'should have the correct rows', ->
            ...
            select(rows[0]).selectAll('td').selectAll('.hx-data-table-cell-key').text().should.eql(headers)
            select(rows[0]).selectAll('td').selectAll('.hx-data-table-cell-value').text().should.eql(['Bob', '25', 'Developer'])
            select(rows[1]).selectAll('td').selectAll('.hx-data-table-cell-key').text().should.eql(headers)
            select(rows[1]).selectAll('td').selectAll('.hx-data-table-cell-value').text().should.eql(['Jan', '41', 'Artist'])
            select(rows[2]).selectAll('td').selectAll('.hx-data-table-cell-key').text().should.eql(headers)
            select(rows[2]).selectAll('td').selectAll('.hx-data-table-cell-value').text().should.eql(['Dan', '41', 'Builder'])
      ...
      describe 'collapsibleRenderer', ->
        ...
        testTable {tableOptions}, undefined, (container, dt, options, data) ->
          ...
          it 'should render collapsible rows correctly when toggling with the button', ->
            ...
            emit(handlers.node(0), 'click', fakeEvent)
            ...
            emit(handlers.node(1), 'click', fakeEvent)
      ...
      describe 'expandedRows', ->
        ...
        it "should get the correct row id's", (done) ->
          ...
          testTable {tableOptions}, done, (container, dt, options, data) ->
            ...
            emit(clickHandlers.forEach (h) -> emit(h.node(), 'click', fakeEvent)) # open all collapsibles
            dt.expandedRows().should.eql(['0', '2'])

            emit(clickHandlers.node(0), 'click', fakeEvent) # close first collapsible
            dt.expandedRows().should.eql(['2'])

            emit(clickHandlers.node(2), 'click', fakeEvent) # close second collapsible
            emit(clickHandlers.node(0), 'click', fakeEvent) # open first collapsible
        ...
        describe 'paginate', ->
          ...
          it 'should change the page when the back button is clicked', (done) ->
            testTable {tableOptions: {displayMode: 'paginate', pageSize: 1}}, undefined, (container, dt, options, data) ->
              ...
              emit(container.select('.hx-data-table-paginator-back').node(), 'click', fakeEvent)
          ...
          it 'should change the page when the forward button is clicked', (done) ->
            testTable {tableOptions: {displayMode: 'paginate', pageSize: 1}}, undefined, (container, dt, options, data) ->
              ...
              emit(container.select('.hx-data-table-paginator-forward').node(), 'click', fakeEvent)
      ...
      describe 'filter', ->
        it 'should not update the cursor position when rendering if the filter is unchanged', (done) ->
          testTable {tableOptions: {filterEnabled: true, filter: 'Bob'}}, undefined, (container, dt, options, data) ->
            ...
            # when
            container.api('data-table').render ->
              # then
              container.select('.hx-data-table-filter').node().selectionStart.should.equal(1)
              done()
        ...
        it 'changing the page should change the visible data', (done) ->
          testTable {tableOptions: {pageSize: 1}}, done, (container, dt, options, data) ->
            ...
            dt.page 2, ->
              rows = container.select('.hx-sticky-table-wrapper').select('tbody').selectAll('.hx-data-table-row').nodes
              select(rows[0]).selectAll('td').selectAll('.hx-data-table-cell-key').text().should.eql(headers)
              select(rows[0]).selectAll('td').selectAll('.hx-data-table-cell-value').text().should.eql(['Jan', '41', 'Artist'])
        ...
        it 'should allow all rows to be selected by default', (done) ->
          testTable {tableOptions: {selectEnabled: true}}, done, (container, dt, options, data) ->
            container.select('.hx-sticky-table-header-left').selectAll('.hx-data-table-checkbox')
              .forEach (h) -> emit(h.node(), 'click', fakeEvent)
      ...
      describe 'selectedRows', ->
        ...
        it 'should emit an event when the value is changed via the api', (done) ->
          dt = new DataTable(div(), {selectEnabled: true, feed: objectFeed(threeRowsData)})
      ...
      describe 'selectEnabled', ->
        ...
        it 'should select all the rows when clicking the checkbox in the top left', (done) ->
          testTable {tableOptions: selectEnabled: true}, done, (container, dt, options, data) ->
            selectAllNode = container.select('.hx-sticky-table-header-top-left').select('thead').select('.hx-data-table-checkbox').node()
            ...
            emit(selectAllNode, 'click', fakeEvent)
        ...
        it 'should clear the selection when the (clear selection) link is clicked', (done) ->
          testTable {tableOptions: selectEnabled: true}, done, (container, dt, options, data) ->
            ...
            clearSelectionNode = container.select('.hx-data-table-status-bar').select('.hx-data-table-status-bar-clear').node()

            emit(clearSelectionNode, 'click', fakeEvent)
            container.classed('hx-data-table-has-page-selection').should.equal(false)
            container.classed('hx-data-table-has-selection').should.equal(false)
            container.select('.hx-sticky-table-wrapper').selectAll('.hx-data-table-row-selected').size().should.equal(0)
            container.select('.hx-sticky-table-header-left').selectAll('.hx-data-table-row-selected').size().should.equal(0)
        ...
        it 'should not break when clicking the top left tick with no data', (done) ->
          testTable {data: noData, tableOptions: selectEnabled: true}, done, (container, dt, options, data) ->
            selectAllNode = container.select('.hx-sticky-table-header-top').select('thead').select('.hx-data-table-checkbox').node()
        ...
        it 'should not allow disabled rows to be selected', ->
          it 'should select the correct range from top to bottom', (done) ->
            testTable {tableOptions: selectEnabled: true, rowEnabledLookup: (row) -> !row.collapsible}, done, (container, dt, options, data) ->
              ...
              emit(clickHandlers.node(0), 'click', fakeEvent)
              container.select('.hx-sticky-table-wrapper').selectAll('.hx-data-table-row-selected').size().should.equal(0)
              container.select('.hx-sticky-table-header-left').selectAll('.hx-data-table-row-selected').size().should.equal(0)
        ...
        describe 'shift selection', ->
          it 'should select the correct range from top to bottom', (done) ->
            testTable {tableOptions: selectEnabled: true}, done, (container, dt, options, data) ->
              ...
              emit(clickHandlers.node(2), 'click', fakeShiftEvent)
              container.select('.hx-sticky-table-wrapper').selectAll('.hx-data-table-row-selected').size().should.equal(3)
              container.select('.hx-sticky-table-header-left').selectAll('.hx-data-table-row-selected').size().should.equal(3)
          ...
          it 'should select the correct range from bottom to top', (done) ->
            testTable {tableOptions: selectEnabled: true}, done, (container, dt, options, data) ->
              ...
              emit(clickHandlers.node(2), 'click', fakeEvent)
              container.select('.hx-sticky-table-wrapper').selectAll('.hx-data-table-row-selected').size().should.equal(1)
              container.select('.hx-sticky-table-header-left').selectAll('.hx-data-table-row-selected').size().should.equal(1)

              emit(clickHandlers.node(0), 'click', fakeShiftEvent)
              container.select('.hx-sticky-table-wrapper').selectAll('.hx-data-table-row-selected').size().should.equal(3)
              container.select('.hx-sticky-table-header-left').selectAll('.hx-data-table-row-selected').size().should.equal(3)
          ...
          it 'should class the container correctly when pressing shift to prevent text selection', (done) ->
            testTable {tableOptions: selectEnabled: true}, done, (container, dt, options, data) ->
              ...
              emit(select('body').node(), 'keydown', fakeShiftEvent)
              container.classed('hx-data-table-disable-text-selection').should.equal(true)
              emit(select('body').node(), 'keyup', fakeEvent)
              container.classed('hx-data-table-disable-text-selection').should.equal(false)
      ...
      describe 'singleSelection', ->
        testTable {tableOptions: selectEnabled: true, singleSelection: true}, undefined, (container, dt, options, data) ->
          ...
          it 'should only allow one row to be selected', ->
            ...
            emit(clickHandlers.node(1), 'click', fakeEvent)
            container.select('.hx-sticky-table-wrapper').selectAll('.hx-data-table-row-selected').size().should.equal(1)
        ...
        it 'should only select the new row on shift selection', (done) ->
          testTable {tableOptions: selectEnabled: true, singleSelection: true}, done, (container, dt, options, data) ->
            ...
            emit(clickHandlers.node(1), 'click', fakeShiftEvent)
            container.select('.hx-sticky-table-wrapper').selectAll('.hx-data-table-row-selected').size().should.equal(1)
      ...
      describe 'filter', ->
        ...
        it 'should call filter when changing the filter input', (done) ->
          container = div()
          dt = new DataTable(container.node())
          ...
          dt.feed objectFeed(threeRowsData), ->
            ...
            filterInput.value('a')
            emit(filterInput.node(), 'input', fakeEvent)
        ...
        describe 'without data', ->
          ...
          it 'should render correctly', (done) ->
            testTable {data: groupedHeaderData, tableOptions: {}}, done, (container, dt, options, data) ->
              ...
              firstRow = select(rows.nodes[0])
              secondRow = select(rows.nodes[1])
              thirdRow = select(rows.nodes[2])
          ...
          it 'should render correctly with selection enabled', (done) ->
            testTable {data: groupedHeaderData, tableOptions: {selectEnabled: true}}, done, (container, dt, options, data) ->
              ...
              select(rows.nodes[0]).selectAll('th').text().should.eql(['', 'TH', ''])
              select(rows.nodes[1]).selectAll('th').text().should.eql(['', 'MH1', 'MH2', 'MH2', 'MH1', ''])
              select(rows.nodes[2]).selectAll('th').text().should.eql(['', 'BH1', 'BH2', 'BH2', 'BH', 'BH', 'BH', '', 'BH'])
        ...
        describe 'with data', ->
          ...
          it 'should render correctly with selection enabled', (done) ->
            testTable {data: groupedHeaderData, tableOptions: {selectEnabled: true}}, done, (container, dt, options, data) ->
              ...
              rows.selectAll('.hx-data-table-control').size().should.equal(4)
              rows.selectAll('.hx-data-table-checkbox').size().should.equal(1)
              select(rows.nodes[0]).select('th').text().should.eql('')
              select(rows.nodes[1]).select('th').text().should.eql('')
              select(rows.nodes[2]).select('th').text().should.eql('')
        ...
        describe 'advancedSearchEnabled', ->
          ...
          it 'should toggle the advanced and regular filters correctly', (done) ->
            ...
            testTable {tableOptions}, done, (container, dt, options, data) ->
              ...
              emit(container.select('.hx-data-table-advanced-search-toggle').node(), 'click', fakeEvent)
        ...
        describe 'advancedSearch', ->
          it 'should render the advanced search correctly',(done) ->
            ...
            testTable {tableOptions}, done, (container, dt, options, data) ->
              ...
              checkValue = (node, typePickerVal, columnPickerVal, termVal) ->
                elem = select(node)
                if typePickerVal
                  elem.select('.hx-data-table-advanced-search-type').api('picker').value().value.should.equal(typePickerVal)
                elem.select('.hx-data-table-advanced-search-column').api('picker').value().value.should.equal(columnPickerVal)
          ...
          it 'should make a new advancedSearch group when changing the type picker for a filter', (done) ->
            ...
            testTable {tableOptions}, undefined, (container, dt, options, data) ->
              ...
              pickerNode = select(filterRows.node(1))
          ...
          it 'should update the advancedSearch when changing the term for a filter', (done) ->
            ...
            testTable {tableOptions}, undefined, (container, dt, options, data) ->
              ...
              termNode = select(filterRows.node(2)).select('.hx-data-table-advanced-search-input')

              termNode.value('d')
              emit(termNode.node(), 'input', {target: {value: 'd'}})
          ...
          it 'should default to "any" for the column value', (done) ->
            ...
            testTable {tableOptions}, done, (container, dt, options, data) ->
              ...
              pickerVal = select(filterRows.node(0)).select('.hx-data-table-advanced-search-column').api('picker').value()
          ...
          it 'should add a filter correctly', (done) ->
            ...
            testTable {tableOptions}, done, (container, dt, options, data) ->
              ...
              emit(clearNode, 'click', fakeEvent)
          ...
          describe 'removing filters', ->
            ...
            it 'should remove a single filter', (done) ->
              ...
              testTable {tableOptions}, done, (container, dt, options, data) ->
                ...
                removeNode = select(filterRows.node(1)).select('.hx-data-table-advanced-search-remove').node()

                emit(removeNode, 'click', fakeEvent)
            ...
            it 'should remove the first filter in a group', (done) ->
              ...
              testTable {tableOptions}, done, (container, dt, options, data) ->
                ...
                removeNode = select(filterRows.node(2)).select('.hx-data-table-advanced-search-remove').node()
            ...
            it 'should remove the last filter', (done) ->
              ...
              testTable {tableOptions}, done, (container, dt, options, data) ->
                ...
                removeNode = select(filterRows.node(0)).select('.hx-data-table-advanced-search-remove').node()

                emit(removeNode, 'click', fakeEvent)
                should.not.exist(dt.advancedSearch())


        describe 'getAdvancedSearchFilter', ->
          it 'should use the defaults if provided', ->
            advancedSearchFilter = getAdvancedSearchFilter()
          ...
          it 'should use the provided cellValueLookup', ->
            ...
            advancedSearchFilter = getAdvancedSearchFilter(cellValueLookup)
          ...
          it 'should use the provided termLookup', ->
            ...
            advancedSearchFilter = getAdvancedSearchFilter(undefined, termLookup)
      ...
      describe 'advancedSearchCriteria', ->
        it 'should show a warning when setting an invalid type on creation', (done) ->
          ...
          testTable {tableOptions}, done, (container, dt, options, data) ->
            logger.warn.should.have.been.called.with('Invalid Filter Criteria Specified:', ['something'], '\nPlease select a value from filterStringTypes()', filterTypes())
        ...
        it 'should show a warning when setting an invalid type on creation', (done) ->
          ...
          testTable {tableOptions}, done, (container, dt, options, data) ->
            logger.warn.should.have.been.called.with('Expected an array of filter criteria but was passed:', 'something')
        ...
        it 'should show a warning when setting an invalid type on creation for a column', (done) ->
          ...
          testTable {tableOptions}, done, (container, dt, options, data) ->
            logger.warn.should.have.been.called.with('Expected an array of filter criteria but was passed:', invalidValue)
        ...
        describe 'compact', ->
          ...
          it 'should toggle the control panel visibility when the toggle is clicked', (done) ->
            ...
            testTable {tableOptions}, done, (container, dt, options, data) ->
              ...
              emit(toggleButton.node(), 'click', fakeEvent)
              clock.tick(animationCompletedDelay)
              # XXX: Calling this twice triggers the morphs, calling once does not (regardless of what value passed in)
              clock.tick(animationCompletedDelay)
    ...
    describe 'renderSuppressed', ->
      it 'should prevent render from doing anything when true', ->
        container = div().style('width', '1000px')
        dt = new DataTable(container.node())
        f = chai.spy()
        dt.renderSuppressed(true)
        dt.feed objectFeed(threeRowsData), f
    ...
    describe 'events', ->
      ...
      describe 'sortchange', ->
        it 'should emit an event when sort clicked with cause: user', (done) ->
          ...
          dt.feed objectFeed(threeRowsData), ->
            emit(selection.select('.hx-sticky-table-header-top').select('.hx-data-table-cell').node(), 'click')
      ...
      describe 'filterchange', ->
        it 'should emit an event when filter changed with cause: user', (done) ->
          ...
          dt.feed objectFeed(threeRowsData), ->
            ...
            emit(selection.select('.hx-data-table-filter').node(), 'input')
            clock.tick(inputDebounceDelay)
      ...
      describe 'selectedrowschange', ->
        it 'should emit an event when rows are selected/deselected', (done) ->
          ...
          dt.feed objectFeed(threeRowsData), ->
            emit(selection.select('.hx-sticky-table-header-left').select('tbody').selectAll('.hx-data-table-checkbox').node(0), 'click', fakeEvent)
            emit(selection.select('.hx-sticky-table-header-left').select('tbody').selectAll('.hx-data-table-checkbox').node(2), 'click', fakeEvent)
            emit(selection.select('.hx-sticky-table-header-left').select('tbody').selectAll('.hx-data-table-checkbox').node(0), 'click', fakeEvent)
      ...
      describe 'selectedrowsclear', ->
        it 'should emit an event when the selected rows is cleared', (done) ->
          ...
          dt.on 'selectedrowschange', (d) ->
            if n is 1
              emit(selection.select('.hx-data-table-status-bar-clear').node(0), 'click', fakeEvent)
            n++
      ...
      describe 'expandedrowschange', ->
        it 'should emit an event when row is collapsed/expanded', (done) ->
          ...
          dt.feed objectFeed(threeRowsData), ->
            emit(selection.select('.hx-sticky-table-header-left').select('.hx-data-table-collapsible-toggle').node(), 'click')
            emit(selection.select('.hx-sticky-table-header-left').select('.hx-data-table-collapsible-toggle').node(), 'click')

      describe 'rowclick', ->
        it 'should emit an event when the row is clicked', (done) ->
          selection = div()
          dt = new DataTable(selection, {pageSizeOptions: [5, 10, 15]})
          ...
          dt.feed objectFeed(threeRowsData), ->
            emit(selection.select('.hx-sticky-table-wrapper').select('.hx-data-table-body').select('.hx-data-table-row').node(), 'click')
      ...
      describe 'pagechange', ->
        ...
        describe 'should emit an event when the page changes', ->
          ...
          it 'should work when setting the page to 2', (done) ->
            selection = div()
            dt = new DataTable(selection, {pageSize: 2, pageSizeOptions: [5, 10, 15]})
            dt.on 'pagechange', (d) ->
              d.value.should.eql(2)
              d.cause.should.equal('user')
              done()
            dt.feed objectFeed(threeRowsData), ->
              selection.select('.hx-data-table-paginator-picker').api('picker').emit('change', {value: {value: 2}, cause: 'user'})
          ...
          it 'should work when setting the page to 1', (done) ->
            ...
            dt.feed objectFeed(threeRowsData), ->
              selection.select('.hx-data-table-paginator-picker').api('picker').emit('change', {value: {value: 1}, cause: 'user'})

      describe 'compactchange', ->
        it 'should emit an event when changing from full to compact', (done) ->

          container = fixture.append('div').style('width', '1000px')
        ...
        it 'should emit an event when changing from compact to full', (done) ->
          container = fixture.append('div').style('width', '100px')
    ...
    describe 'data feeds', ->
      describe 'object data feed', ->
        ...
        it 'should get the number of rows in the data set', (done) ->
          objectFeed(data).totalCount (count) ->
            count.should.equal(3)
            done()

        it 'should get the headers from the data set', (done) ->
          objectFeed(data).headers (headers) ->
            headers.should.eql([
              { name: 'Name', id: 'name' },
              { name: 'Age', id: 'age' },
              { name: 'Profession', id: 'profession' }
        ...
        it 'should get the rows from the data set 1', (done) ->
          objectFeed(data).rows {start: 0, end: 0}, (data) ->
        ...
        it 'should get the rows from the data set 2', (done) ->
          objectFeed(data).rows {start: 1, end: 2}, (data) ->
            data.rows.should.eql([
              {
                id: 1,
                cells: { 'name': 'Jan', 'age': 41, 'profession': 'Artist' }
        ...
        it 'should get the rows from the data set with filtering', (done) ->
          objectFeed(data).rows {start: 0, end: 2, filter: '41'}, (data) ->
            data.rows.should.eql([
              {
                id: 1,
                cells: { 'name': 'Jan', 'age': 41, 'profession': 'Artist' }
        ...
        it 'should get the rows from the data set with filtering and limited range', (done) ->
          objectFeed(data).rows {start: 1, end: 1, filter: '41'}, (data) ->
            data.rows.should.eql([
              {
                id: 2,
                cells: { 'name': 'Dan', 'age': 41, 'profession': 'Builder' }
        ...
        it 'should get the rows from the data set with sorting (asc)', (done) ->
          objectFeed(data).rows {start: 0, end: 2, sort: {column: 'name', direction: 'asc'}}, (data) ->
            data.rows.should.eql([
              {
                id: 0, # hidden details can go here (not in the cells object)
                cells: { 'name': 'Bob', 'age': 25, 'profession': 'Developer' }
        ...
        it 'should get the rows from the data set with sorting (desc)', (done) ->
          objectFeed(data).rows {start: 0, end: 2, sort: {column: 'name', direction: 'desc'}}, (data) ->
            data.rows.should.eql([
              {
                id: 1,
                cells: { 'name': 'Jan', 'age': 41, 'profession': 'Artist' }
        ...
        it 'should get the rows from the data set with sorting and limited range', (done) ->
          objectFeed(data).rows {start: 1, end: 2, sort: {column: 'name', direction: 'asc'}}, (data) ->
            data.rows.should.eql([
              {
                id: 2,
                cells: { 'name': 'Dan', 'age': 41, 'profession': 'Builder' }
        ...
        it 'should get the rows from the data set with sorting and filtering', (done) ->
          objectFeed(data).rows {start: 0, end: 2, sort: {column: 'name', direction: 'asc'}, filter: '41'}, (data) ->
            data.rows.should.eql([
              {
                id: 2,
                cells: { 'name': 'Dan', 'age': 41, 'profession': 'Builder' }
        ...
        it 'should get the rows from the data set with sorting and filtering and limited range', (done) ->
          objectFeed(data).rows {start: 0, end: 0, sort: {column: 'name', direction: 'asc'}, filter: '41'}, (data) ->
            data.rows.should.eql([
              {
                id: 2,
                cells: { 'name': 'Dan', 'age': 41, 'profession': 'Builder' }
        ...
        it 'should get the rows for ids correctly 1', (done) ->
          lookup = (row) -> row.id
          objectFeed(data).rowsForIds [0, 1], lookup, (rows) ->
            rows.should.eql([
              {
                id: 0, # hidden details can go here (not in the cells object)
                cells: { 'name': 'Bob', 'age': 25, 'profession': 'Developer' }
        ...
        it 'should get the rows for ids correctly 2', (done) ->
          lookup = (row) -> row.id
          objectFeed(data).rowsForIds [2, 0], lookup, (rows) ->
            rows.should.eql([
              {
                id: 2,
                cells: { 'name': 'Dan', 'age': 41, 'profession': 'Builder' }
        ...
        it 'should use the provided filter function', ->
          ...
          objectFeed(data, options).rows {filter: 'Bob', start: 0, end: 2}, (data) ->
            data.rows.should.eql([
              {
                id: 0,
                cells: { 'name': 'Bob', 'age': 25, 'profession': 'Developer' }
        ...
        it 'should use the provided advanced search function', ->
          ...
          objectFeed(data, options).rows {useAdvancedSearch: true, advancedSearch: [[{column: 'name', term: 'Bob'}]], start: 0, end: 2}, (data) ->
            data.rows.should.eql([
              {
                id: 0,
                cells: { 'name': 'Bob', 'age': 25, 'profession': 'Developer' }
        ...
        describe 'advanced search', ->
          ...
          it 'should return the complete dataset when the filter is not defined', (done) ->
            filter = undefined
            objectFeed(advancedSearchData).rows {start: 0, end: 5, useAdvancedSearch: true, advancedSearch: filter}, (data) ->
          ...
          it 'should filter multiple times on the same data', (done) ->
            ...
            feed = objectFeed(advancedSearchData)
