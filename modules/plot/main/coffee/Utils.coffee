  ...
  selection.view('.hx-legend-entry', 'g')
    ...
    .update (s, e, i) ->
      ...
      @select('rect')
        ...
        .attr('width', 10)
        .attr('height', 10)
        .attr('fill', if s.legendColor then s.legendColor() else s.fillColor) # XXX: the else s.color is there for PieCharts... which should be fixed when pie charts are refactored
    .apply(series)
