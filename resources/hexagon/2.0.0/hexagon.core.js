...
var Modal, getHeaderRender, getTitleRender, makeButtons, modal, modalDialog, modalInput;
...
var Notification, inbuiltNotificationManager, nextId, notify, notifyDefaultTimeout, notifyInfo, notifyLoading, notifyNegative, notifyPositive, notifyWarning, redraw, removeNotification, setupNotification, startTimeout, togglePin, updatePinnedStatus, wrapDeprecated$1;
...
notifyPositive = function(message, options) {
...
notifyNegative = function(message, options) {
  return inbuiltNotificationManager.negative(message, options);
};

notifyLoading = function(message) {
...
// XXX Deprecated: Remove in next major
notify.info = wrapDeprecated$1('hx.notify.info', notifyInfo);

notify.positive = wrapDeprecated$1('hx.notify.positive', notifyPositive);

notify.warning = wrapDeprecated$1('hx.notify.warning', notifyWarning);

notify.negative = wrapDeprecated$1('hx.notify.negative', notifyNegative);

notify.loading = wrapDeprecated$1('hx.notify.loading', notifyLoading);

notify.defaultTimeout = wrapDeprecated$1('hx.notify.defaultTimeout', notifyDefaultTimeout);
...
var LocalStoragePreferencesStore, Preferences, defaultTimezoneList, defaultTimezoneLookup, localeList, lookupLocale, preferences,
  indexOf$1 = [].indexOf;
  ...
  var Preferences = /*@__PURE__*/(function (EventEmitter) {
    function Preferences() {
      ...
      setupModal = function (element) {
        ...
        localeSection = div().add(detached('label').text(userFacingText('preferences', 'locale'))).add(localeAutocompleteElement);
        // Timezone Stuff
        timezoneAutocompleteElement = detached('input');
        timezoneAutocompleteElement.value(timezone);
        new Autocomplete(timezoneAutocompleteElement.node(), this$1._.supportedTimezones, {
          showOtherResults: true,
          mustMatch: true
        }).on('change', function(value) {
          return timezone = value;
        });
        timezoneSection = div().add(detached('label').text(userFacingText('preferences', 'timezone'))).add(timezoneAutocompleteElement);
        saveButton = detached('button').class('hx-btn hx-positive').add(detached('i').class('hx-icon hx-icon-check')).add(detached('span').text(' ' + userFacingText('preferences', 'save'))).on('click', function () {
      ...
      if (!(isString(defaultLocaleId) && lookupLocale(defaultLocaleId))) {
        defaultLocaleId = 'en';
      }
    ...
    Preferences.prototype.isTimezoneSupported = function isTimezoneSupported (timezone) {
      return isString(timezone) && this._.supportedTimezones.indexOf(timezone) !== -1;
    };

    Preferences.prototype.isLocaleSupported = function isLocaleSupported (locale) {
      return isString(locale) && lookupLocale(locale);
    };
...
DateTimeLocalizer = /*@__PURE__*/(function (PreferencesHandler) {
  ...
  DateTimeLocalizer.prototype.time = function time (date, showSeconds) {
    var timeString;
    date = preferences.applyTimezoneOffset(date, this.timezone());
...
var AutocompleteFeed, sortActive$1, sortItems, trimTrailingSpaces;
  ...
  if ( options === void 0 ) options = {};
    ...
    if (resolvedOptions.filter == null) {
      ...
      var assign;
        ...
        filtered = filter$1[filterName](items, term, resolvedOptions.filterOptions);
...
var AutocompletePicker, autocompletePicker, debounceDuration, enterKeyCode, setPickerValue, validateItems;
...
AutocompletePicker = /*@__PURE__*/(function (EventEmitter) {
...
var DateTimePicker = /*@__PURE__*/(function (EventEmitter) {
  function DateTimePicker(selector, options) {
    ...
    updateDatePicker = function (data) {
      ...
      this$1.datePicker.date(preferences.applyTimezoneOffset(this$1.date()));
